...
use actix::{Actor, ActorContext, AsyncContext, Running, SpawnHandle};
    ...
    A::Context: AsyncContext<A>,
{
    ...
    fn poll(
        ...
        let perf = crate::stats::MeasurePerf::new("FramedWrite", "WriterFut");
        ...
        let result = (|| {
            ...
            while !inner.buffer.is_empty() {
                match unsafe { Pin::new_unchecked(io.deref_mut()) }.poll_write(task, &inner.buffer)
                {
                    Poll::Ready(Ok(n)) => {
                        if n == 0
                            && act.error(
    ...
    A::Context: AsyncContext<A>,
{
    ...
    fn poll(
        ...
        let perf = crate::stats::MeasurePerf::new("FramedWrite", "ActorFuture");
...
impl<I: 'static, S: Sink<I> + Unpin + 'static> SinkWrite<I, S> {
        ...
        C: AsyncContext<A>,
    {
        ...
        let handle = ctxt.spawn(SinkWriteFuture { inner: inner.clone(), _actor: PhantomData });
    ...
    A::Context: AsyncContext<A>,
{
    ...
    fn poll(
        ...
        let perf = crate::stats::MeasurePerf::new("FramedWrite", "SinkWriteFuture");
