    ...
    D: Decoder,
{
    ...
    pub fn new(inner: T, decoder: D) -> FramedRead<T, D> {
        FramedRead { inner: framed_read2(Fuse { io: inner, codec: decoder }) }
    }
}
    ...
    D: Decoder,
{
    ...
    fn poll_next(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        let perf =
            near_performance_metrics::stats::MeasurePerf::new("FramedRead", "stream_poll_next1");
    ...
    T: Sink<I>,
{
    ...
    fn poll_ready(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Result<(), Self::Error>> {
        self.project().inner.project().inner.project().io.poll_ready(cx)
    }

    fn start_send(self: Pin<&mut Self>, item: I) -> Result<(), Self::Error> {
        self.project().inner.project().inner.project().io.start_send(item)
    }

    fn poll_flush(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Result<(), Self::Error>> {
        self.project().inner.project().inner.project().io.poll_flush(cx)
    }

    fn poll_close(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Result<(), Self::Error>> {
        self.project().inner.project().inner.project().io.poll_close(cx)
    }
}
...
pub(crate) fn framed_read2_with_buffer<T>(inner: T, mut buf: BytesMut) -> FramedRead2<T> {
    ...
    FramedRead2 { inner, eof: false, is_readable: !buf.is_empty(), buffer: buf }
}
    ...
    T::Codec: Decoder,
{
    ...
    fn poll_next(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        let perf = near_performance_metrics::stats::MeasurePerf::new("FramedRead", "fr_poll_next");
        ...
        let result = (|| {
            loop {
                ...
                if *pinned.is_readable {
                    if *pinned.eof {
                        let frame =
                            pinned.inner.as_mut().project().codec.decode_eof(&mut pinned.buffer)?;
                    ...
                    if let Some(frame) =
                        pinned.inner.as_mut().project().codec.decode(&mut pinned.buffer)?
                    {
                ...
                let bytect = match pinned.inner.as_mut().poll_read_buf(cx, &mut pinned.buffer)? {
                    Poll::Ready(ct) => ct,
