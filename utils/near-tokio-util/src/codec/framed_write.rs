    ...
    E: Encoder,
{
    ...
    pub fn new(inner: T, encoder: E) -> FramedWrite<T, E> {
        FramedWrite { inner: framed_write2(Fuse { io: inner, codec: encoder }) }
    }
}
    ...
    T: Stream,
{
    ...
    fn poll_next(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        self.project().inner.project().inner.project().io.poll_next(cx)
    }
}
...
pub(crate) fn framed_write2<T>(inner: T) -> FramedWrite2<T> {
    FramedWrite2 { inner, buffer: BytesMut::with_capacity(INITIAL_CAPACITY) }
}
    ...
    T::Codec: Encoder<Item = I>,
{
    ...
    fn start_send(self: Pin<&mut Self>, item: I) -> Result<(), Self::Error> {
        ...
        pinned.inner.project().codec.encode(item, &mut pinned.buffer)?;
        Ok(())
    }
