...
impl<T, D> NearFramedRead<T, D>
where
    T: AsyncRead,
    D: Decoder,
{
    /// Creates a new `NearFramedRead` with the given `decoder`.
    pub fn new(inner: T, decoder: D) -> NearFramedRead<T, D> {
        NearFramedRead {
            inner: FramedImpl {
                inner,
                codec: decoder,
    ...
    /// Creates a new `NearFramedRead` with the given `decoder` and a buffer of `capacity`
    /// initial size.
    pub fn with_capacity(inner: T, decoder: D, capacity: usize) -> NearFramedRead<T, D> {
        NearFramedRead {
            inner: FramedImpl {
                inner,
                codec: decoder,
...
impl<T, D> NearFramedRead<T, D> {
    ...
    /// Consumes the `NearFramedRead`, returning its underlying I/O stream.
    ///
    /// Note that care should be taken to not tamper with the underlying stream
    /// of data coming in as it may corrupt the stream of frames otherwise
    /// being worked with.
...
impl<T, D> Stream for NearFramedRead<T, D>
where
    T: AsyncRead,
    D: Decoder,
{
...
impl<T, I, D> Sink<I> for NearFramedRead<T, D>
where
    T: Sink<I>,
{
    type Error = T::Error;
...
impl<T, D> fmt::Debug for NearFramedRead<T, D>
where
    T: fmt::Debug,
    D: fmt::Debug,
{
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("NearFramedRead")
