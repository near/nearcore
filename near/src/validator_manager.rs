...
mod test {
    ...
    fn test_stake_validator() {
        ...
        vm.add_proposals(h0, h1, 1, vec![stake("test2", amount_staked)], vec![], vec![], 0, 0)
            .unwrap()
            .commit()
            .unwrap();
    ...
    fn test_fork_finalization() {
        ...
        vm.add_proposals(h0, h1, 1, vec![stake("test4", amount_staked)], vec![], vec![], 0, 0)
            .unwrap()
            .commit()
            .unwrap();
        ...
        vm.add_proposals(h1, h4, 4, vec![], vec![], vec![], 0, 0).unwrap().commit().unwrap();
        vm.add_proposals(h3, h5, 5, vec![], vec![], vec![], 0, 0).unwrap().commit().unwrap();
        vm.finalize_epoch(&h3, &h5, &h6).unwrap();
        vm.add_proposals(h5, h6, 6, vec![], vec![], vec![], 0, 0).unwrap().commit().unwrap();
        ...
        vm.add_proposals(h4, h7, 7, vec![], vec![], vec![], 0, 0).unwrap().commit().unwrap();
        assert_eq!(
        ...
        vm.add_proposals(h6, h8, 8, vec![], vec![], vec![], 0, 0).unwrap().commit().unwrap();
    ...
    fn test_validator_change_of_stake() {
        ...
        vm.add_proposals(h0, h1, 1, vec![stake("test1", 10)], vec![], vec![], 0, 0)
            .unwrap()
            .commit()
            .unwrap();
        vm.finalize_epoch(&h0, &h1, &h2).unwrap();
