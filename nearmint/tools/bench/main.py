...
def block_time_to_timestamp(t):
    if isinstance(t, str):
        ...
        return t // 1000000000 # datetime.timestamp(datetime.fromtimestamp(t // 100000000))

def benchmark(rpc, users, duration):
    start_status = rpc.status()
    start_height = int(start_status['sync_info']['latest_block_height'])
    ...
    end_height = 0
    end_height = int(end_status['sync_info']['latest_block_height'])

    index = end_height
    num_blocks_per_sec = collections.defaultdict(int)
    num_tx_per_sec = collections.defaultdict(int)
    start_block_time = block_time_to_timestamp(start_status['sync_info']['latest_block_time'])
    end_block_time = block_time_to_timestamp(end_status['sync_info']['latest_block_time'])
    print(start_block_time)
    while index > start_height:
        block = rpc.get_block(index)
        index -= 1
        block_time = block_time_to_timestamp(block['header']['timestamp'])
        num_txs = len(block['transactions'])
        print(block_time - start_block_time, num_txs, block_time)
...
if __name__ == "__main__":
    ...
    alice = User(rpc, "test.near")
    bob = User(rpc, "bob.near")

    benchmark(rpc, [alice, bob], 1)
