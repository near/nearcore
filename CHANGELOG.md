...
* Extended `EXPERIMENTAL_receipt_to_tx` with optional `(block_height, shard_id, window)` hint parameters. `block_height` enables a best-effort hinted fallback scan for callers that supply an approximate execution or creation height; receipt-id-only queries against periods where `save_receipt_to_tx=false` remain unsupported. `shard_id` narrows any hint scan that runs before the walk has crossed a `FromReceipt` arm; once a `FromReceipt` arm fires the handler overwrites `current_shard` via predecessor-account derivation (`shard_for_account_at_height`), so this field does not narrow the rest of the walk. If the very first walker step is a column hit returning `FromReceipt`, the caller's shard hint is discarded without being used. Omitting `shard_id` makes any scan that runs before a `FromReceipt` arm has derived a shard enumerate all tracked shards at the hint height. The wire response is unchanged. The `window` request field controls scan width on any `CenterOut` scan: the first scan against the caller's hint, and any subsequent scan that follows a column hit (column hits do not refresh `current_height`). The width cap on caller-supplied `window` is operator-tunable via `receipt_to_tx_max_hint_window` (default 20). Scans that immediately follow a scan-resolved hop use a separate anchor-inclusive backward iterator (`h, h-1, ..., h-distance`) capped by the new `receipt_to_tx_max_hop_distance` node config (default 10), not by `window`. The selector tracks only the immediately preceding hop, so a column hit between scans flips the next column-miss scan back to `CenterOut + window`. Callers that previously bumped `window` expecting it to widen the post-scan ancestor scan are silently downgraded to `max_hop_distance` on that scan — raise the new config knob if your workload shows ancestor misses. The hint walker performs one scan per hop instead of two (the prior boundary-refresh pre-scan is now folded into the next hop's column-miss branch); no observable change in outcomes.
