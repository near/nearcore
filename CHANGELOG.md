...
* Extended `EXPERIMENTAL_receipt_to_tx` with optional `(block_height, shard_id, window)` hint parameters. `block_height` enables a best-effort hinted fallback scan for callers that supply an approximate execution or creation height; receipt-id-only queries against periods where `save_receipt_to_tx=false` remain unsupported. `shard_id` narrows any hint scan that runs before the walk has crossed a `FromReceipt` arm; once a `FromReceipt` arm fires the handler overwrites `current_shard` via predecessor-account derivation (`shard_for_account_at_height`), so this field does not narrow the rest of the walk. If the very first walker step is a column hit returning `FromReceipt`, the caller's shard hint is discarded without being used. Omitting `shard_id` makes any scan that runs before a `FromReceipt` arm has derived a shard enumerate all tracked shards at the hint height. The wire response is unchanged. The `window` request field controls scan width on the pre-first-scan `CenterOut` scan against the caller's literal hint. The width cap on caller-supplied `window` is operator-tunable via `receipt_to_tx_max_hint_window` (default 20). The hint walker performs at most one scan per hop. Once any scan resolves a hop in the walk, `current_height` carries an upper-bound provenance via causality (receipts emit before they execute); all subsequent column-miss scans use `Ancestor + receipt_to_tx_max_hop_distance` regardless of intervening column hits. The `receipt_to_tx_max_hop_distance` default rises from 10 to 20 to match `receipt_to_tx_max_hint_window`, so default-cap callers retain or expand backward reach (`window=5` callers gain 5 → 20). Operators running with custom `receipt_to_tx_max_hint_window > 20` who walk through scan-resolved-then-column-hit paths should raise `receipt_to_tx_max_hop_distance` to match — without the bump, backward reach narrows from the caller's `window` to the new `max_hop_distance` default in that workload class.
