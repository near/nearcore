...
* A gas key nonce in a `DelegateActionV2` must now be exactly the selected nonce sequence's current value plus one; a delegate action with a nonce gap fails with `DelegateActionInvalidNonce` instead of advancing the sequence. Previously any nonce above the current one was accepted, so a single delegate action could skip a sequence forward and invalidate every queued transaction on it in between. A plain (non-gas-key) nonce in a `DelegateActionV2` keeps the monotonic rule, matching `DelegateAction`.
