...
* `view_access_key` queries now report a missing *account* rather than a missing *key* when the queried account does not exist, so callers such as NEP-641 off-chain resolvers no longer need a second `view_account` request to tell a not-yet-created implicit account apart from a deleted key. `EXPERIMENTAL_view_access_key` returns the structured `UNKNOWN_ACCOUNT` error instead of `UNKNOWN_ACCESS_KEY`. The `query` method with `request_type: view_access_key` returns the structured `UNKNOWN_ACCOUNT` error for a missing account instead of its backward-compatible flat response (`{"error": ..., "logs": [], "block_height": ..., "block_hash": ...}`); the flat shape is kept for an account that exists but does not hold the key. Clients recognize the flat shape by matching its message text, so a new message there would have been read as a contract execution error, while the structured error deserializes into the `UnknownAccount` variant those clients already carry. `view_access_key_list` (both the `query` request type and `EXPERIMENTAL_view_access_key_list`) likewise now returns `UNKNOWN_ACCOUNT` for an account that does not exist, instead of an empty key list; an empty list now unambiguously means the account exists and holds no access keys. `view_gas_key_nonces` (both the `query` request type and `EXPERIMENTAL_view_gas_key_nonces`) returns `UNKNOWN_ACCOUNT` instead of `UNKNOWN_GAS_KEY` for an account that does not exist. ([#16185](https://github.com/near/nearcore/issues/16185))
...
* Added a new `DelegateV2` meta-transaction action that lets gas keys ([NEP-611](https://github.com/near/NEPs/pull/611)) sign relayed (meta) transactions. Unlike the existing `Delegate` action, whose `nonce` is the signing access key's plain nonce, `DelegateV2` carries a `TransactionNonce` that selects either the access key's nonce or one of a gas key's parallel nonce sequences by index. The signed payload is versioned so future variants can be added without a new action type. ([#15904](https://github.com/near/nearcore/pull/15904), [#15905](https://github.com/near/nearcore/pull/15905), [#15906](https://github.com/near/nearcore/pull/15906))
...
* Stabilized continuous epoch sync: epoch sync proofs are maintained incrementally at each epoch boundary, and block headers are garbage-collected on non-archival nodes. Enabling this feature performs a DB migration (48 → 49) to store the compressed proof. ([#15476](https://github.com/near/nearcore/pull/15476))
* Stabilized dynamic resharding: shards are now split automatically at epoch boundaries based on runtime state size, without requiring a protocol upgrade to ship a new hard-coded shard layout. ([#15823](https://github.com/near/nearcore/pull/15823))
