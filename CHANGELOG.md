...
* Priced ML-DSA-65 signature verification as gas in nightly. A transaction signed with an ML-DSA-65 key (or carrying a `Delegate` action with an ML-DSA-65 inner signer) burns an extra `ml_dsa_65_verification_cost` (100 Ggas) at transaction conversion, paid by the signer. The charge is the extra verification cost over the classical schemes; ed25519/secp256k1 stay 0 for backwards compatibility. The gas attached to and available for receipts is unchanged, so contracts are unaffected. ([#15842](https://github.com/near/nearcore/pull/15842))
