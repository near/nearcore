...
* State sync now verifies a reconstructed shard state root against the next new chunk before block sync. A mismatch stops the node and preserves the data directory for diagnosis instead of attributing the failure to the chunk producer. ([#15994](https://github.com/near/nearcore/issues/15994))
