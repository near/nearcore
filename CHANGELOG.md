...
* Global contract code RPC queries now select a shard tracked by the local node instead of always using the epoch's first shard, avoiding spurious `DOES_NOT_TRACK_SHARD` errors on nodes that track only a subset of shards. ([#16207](https://github.com/near/nearcore/pull/16207))
