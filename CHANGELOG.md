...
* Make chunk-producer-to-shard assignment sticky across epoch boundaries: preserve assignment by `ShardId` (rather than `ShardIndex`) and, when a shard splits, distribute the parent's chunk producers across its child shards using greedy stake-balanced bin-packing.
