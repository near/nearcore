...
class TestSetup:
    ...
    def _schedule_upgrade_nodes_every_n_minutes(self, minutes):
        ...
        for i in range(1, 5):
            ...
            stop_nodes_args.on = ScheduleMode(mode="active", value=f"{(i * minutes)}m")
            ...
            amend_binaries_args.on = ScheduleMode(mode="active", value=f"{(i * minutes * 60 + 20)}")
            ...
            start_nodes_args.on = ScheduleMode(mode="active", value=f"{(i*minutes + 1)}m")
