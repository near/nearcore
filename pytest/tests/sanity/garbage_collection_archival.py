...
for key in keys:
    ...
    tx = sign_create_account_with_full_access_key_and_balance_tx(nodes[0].signer_key, f"{key}.{nodes[0].signer_key.account_id}", nodes[0].signer_key, NEAR_BASE, nonce, block_id.hash_bytes)
...
for key in keys:
    ...
    tx = sign_delete_account_tx(
        key, account_id, nodes[0].signer_key.account_id, int(block_id.height) * 1_000_000, 
        block_id.hash_bytes)
    res = nodes[1].send_tx_and_wait(tx, 2)
