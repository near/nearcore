    ...
    async def handle(self, msg, fr, to):
        ...
        if msg.enum == "RoutingTableSyncV2" and msg.RoutingTableSyncV2.enum == "Version2":
            if msg.RoutingTableSyncV2.Version2.routing_state.enum == "Done":
                success.value = 1
            logger.info("ROUTING_STATE %s" % msg.RoutingTableSyncV2.Version2.routing_state.enum)
            logger.info("* known_edges %s" % msg.RoutingTableSyncV2.Version2.known_edges)
            logger.info("* edges %s" % len(msg.RoutingTableSyncV2.Version2.edges))
        return True
...
def gen():
    return base58.b58encode(ed25519.create_keypair()[0].to_bytes()).decode('ascii')
...
def new_edge(peer_a, peer_b, nonce):
    ...
    return {
        'nonce': nonce,
        'peer0': 'ed25519:' + min(s1, s2),
        'peer1': 'ed25519:' + max(s1, s2),
        'removal_info': None,
        'signature0': 'ed25519:5ca6A3uzrK3FKGjeAEi7pLNDiBbrzQ1ZPseXF1Rw8rfr3RWYNXPckVDUGGKMZQFE8eKWXeeCws6ZvmeQW8PZEzP6',
        'signature1': 'ed25519:5ca6A3uzrK3FKGjeAEi7pLNDiBbrzQ1ZPseXF1Rw8rfr3RWYNXPckVDUGGKMZQFE8eKWXeeCws6ZvmeQW8PZEzP6'
    }
...
for (left, right, common, TIMEOUT) in tests:
    test_id += 1
    start = time.time()
    logger.info("test with params left: %s right: %s common: %s" % (left, right, common))
    time.sleep(1)
    # set options

    common_nodes = [new_edge(test_id*int(1e9)+int(2e7), test_id*int(1e9) + x + 1 + left + right, 2 * test_id + 1) for x in range(common)]
    left_nodes = [new_edge(test_id*int(1e9)+0, test_id*int(1e9)+  x + 1, 2 * test_id + 1) for x in range(left)]
    right_nodes = [new_edge(test_id*int(1e9)+int(1e7), test_id*int(1e9) + x + 1 + left, 2 * test_id + 1) for x in range(right)]
    to_node0 = left_nodes + common_nodes
    to_node1 = right_nodes + common_nodes

    def simplify(edges):
        return [{'key': (edge['peer0'], edge['peer1']), 'nonce': edge['nonce']} for edge in edges]
    ...
    success.value = 0
    logger.info("starting_sync to %s" % (nodes[0].json_rpc("adv_start_routing_table_syncv2", {
        "peer_id": peer_id1
    })))
