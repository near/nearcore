...
nodes = start_cluster(3, 1, 4, {'local': True, 'near_root': '../target/debug/'}, [["epoch_length", EPOCH_LENGTH], ["block_producer_kickout_threshold", 10], ["chunk_producer_kickout_threshold", 10]], {0: tracked_shards, 1: tracked_shards})
...
for i in range(4):
    ...
    tx = sign_staking_tx(nodes[i].signer_key, nodes[i].validator_key, stake, 1, base58.b58decode(hash_.encode('utf8')))
    nodes[0].send_tx(tx)
