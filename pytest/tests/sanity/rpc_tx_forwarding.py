...
nodes = start_cluster(2, 2, 4, None, [["min_gas_price", 0], ["epoch_length", 10], ["block_producer_kickout_threshold", 70]], {3: {"tracked_shards": [0, 1, 2, 3]}})
...
new_balances = [int(nodes[-1].get_account("test%s" % x)['result']['amount']) for x in [0, 1, 2]]
