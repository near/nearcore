...
class Account:
    ...
    def __init__(self, key, init_nonce, base_block_hash, rpc_info):
        ...
        self.tx_timestamps = []
        logger.info(f'Creating Account {key.account_id} {init_nonce} {rpc_info} {key.pk} {key.sk}')
    ...
    def send_tx(self, signed_tx):
        # return self.json_rpc('broadcast_tx_commit', [base64.b64encode(signed_tx).decode('utf8')])
    ...
    def send_call_contract_raw_tx(self, contract_id, method_name, args,
        ...
        tx = sign_function_call_tx(self.key, contract_id, method_name, args,
                                   300000000000000, int(round(deposit)), self.nonce,
                                   self.base_block_hash)
        return self.send_tx(tx)
