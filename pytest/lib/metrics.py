...
class Metrics:
    def __init__(self, total_blocks, memory_usage, total_transactions, block_processing_time, timestamp):
    ...
    def from_url(cls, metrics_url):
        ...
        prometheus_metrics = dict(map(lambda m: (m.name, m), parser.text_string_to_metric_families(prometheus_string)))
        fold_sample = lambda key: fold(prometheus_metrics, key, lambda m: int(m.samples[0].value), 0)
        ...
        block_processing_time_samples = prometheus_metrics['near_block_processing_time'].samples
        ...
        return cls(total_blocks, memory_usage, total_transactions, block_processing_time, timestamp)
    ...
    def diff(cls, final_metrics, initial_metrics):
        ...
        for sample in final_metrics.block_processing_time.keys():
            block_processing_time[sample] = final_metrics.block_processing_time[sample] - initial_metrics.block_processing_time[sample]
