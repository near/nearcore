...
def create_upgrade_schedule(rpc_nodes, validator_nodes, progressive_upgrade,
    ...
    if progressive_upgrade:
        ...
        for seat, stake, instance_name in seats:
            ...
            print(f'validator node {node.instance_name} will start upgraded')
            seats_upgraded += seat
        ...
        for node in validator_nodes:
            if node.instance_name not in schedule:
                ...
                print(f'validator node {node.instance_name} will upgrade at {schedule[node.instance_name]}')

        for node in rpc_nodes:
            schedule[node.instance_name] = random.randint(0, 4)
            if not schedule[node.instance_name]:
                print(f'rpc node {node.instance_name} will start upgraded')
            else:
                print(f'rpc node {node.instance_name} will upgrade at {schedule[node.instance_name]}')
    else:
