    ...
    > {
        ...
        .map_err(|_| {
            ...
            tracing::warn!(
                ...
                ?tx_info,
                ?fetch_receipt,
                result = ?tx_status_result,
                timeout = ?self.polling_config.polling_timeout,
                "timeout in tx_status_fetch method"
            );
            near_jsonrpc_primitives::types::transactions::RpcTransactionError::TimeoutError
        })?
    }
...
pub async fn start_http(
    ...
    info!(target: "network", %addr, "starting http server");
    ...
    future_spawner.spawn("JSON RPC", async move {
        if let Err(e) = axum::serve(listener, app).await {
            error!(target: "network", ?e, "http server error");
        }
    });

    if let Some(prometheus_addr) = prometheus_addr {
        info!(target: "network", %prometheus_addr, "starting http monitoring server");
        ...
        future_spawner.spawn("Prometheus Metrics", async move {
            if let Err(e) = axum::serve(listener, prometheus_app).await {
                error!(target: "network", ?e, "prometheus server error");
            }
        });
    }
}
...
pub async fn start_http_for_readonly_debug_querying(
    ...
    info!(%addr, "starting readonly debug API server");
    info!(
        "use tools/debug-ui, use localhost as the node, and go to the entity debug tab to start \
         querying"
    );
