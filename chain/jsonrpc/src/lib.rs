...
async fn rpc_handler(
    ...
    let status_code = if let Message::Response(response) = &message {
...
pub fn start_http(
    ...
    let cors_allowed_origins_clone = &cors_allowed_origins;
    ...
    if enable_debug_rpc {
        app = app
            ...
            .route("/debug/api/*api", get(debug_handler))
        ...
        match tokio::net::TcpListener::bind(addr_copy).await {
            Ok(listener) => {
                if let Err(e) = axum::serve(listener, app).await {
                    error!(target: "network", "JSON RPC server error: {:?}", e);
                }
            }
            Err(e) => {
                error!(
                    target: "network",
                    "Could not start http server at {} due to {:?}", addr_copy, e,
                );
            }
        }
    });
    ...
    if let Some(prometheus_addr) = prometheus_addr {
        ...
        let prometheus_addr_copy = prometheus_addr;
        future_spawner.spawn(
            "Prometheus Metrics",
            async move {
                match tokio::net::TcpListener::bind(&prometheus_addr_copy).await {
                    Ok(listener) => {
                    ...
                    Err(e) => {
                        error!(
                            target: "network",
                            "Can't export Prometheus metrics at {} due to {:?}", prometheus_addr_copy, e,
                        );
                    }
                }
            },
