...
- **`process_query_response()` has intentional backward-compatible error formatting.** For `ContractExecutionError` and `UnknownAccessKey`, it returns a flat JSON object with `error`, `logs`, `block_height`, `block_hash` fields instead of a structured RPC error. This is deliberate. `UnknownAccount` gets the same flat treatment, but only for `view_access_key` requests, which is why the function takes an `is_view_access_key` flag. Every other request type must keep returning the structured `UNKNOWN_ACCOUNT` error, so do not drop that guard.
