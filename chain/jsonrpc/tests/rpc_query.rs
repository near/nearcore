...
fn test_block() {
    ...
    System::run(|| {
        ...
        actix::spawn(client.block(BlockId::BlockIndex(0)).then(|res| {
            let res = res.unwrap();
            assert_eq!(res.header.block_index, 0);
...
fn test_block_by_hash() {
    ...
    System::run(|| {
        ...
        actix::spawn(client.block(BlockId::BlockIndex(0)).then(move |res| {
            let res = res.unwrap();
            let mut client = new_client(&format!("http://{}", addr));
...
fn test_chunk_by_hash() {
    ...
    System::run(|| {
        ...
        let mut client = new_client(&format!("http://{}", addr.clone()));
        actix::spawn(
            client.chunk(ChunkId::BlockShardId(BlockId::BlockIndex(0), ShardId::from(0u64))).then(
                move |chunk| {
                    ...
                    assert_eq!(chunk.header.block_index_created, 0);
                    assert_eq!(chunk.header.block_index_included, 0);
    ...
    System::run(|| {
        ...
        actix::spawn(client.query("account/test".to_string(), "".to_string()).then(
            |query_response| {
                ...
                assert_eq!(query_response.block_index, 0);
                let account_info =
...
/// Retrieve gas price
#[test]
fn test_gas_price_by_block_index() {
    init_test_logger();
    ...
    System::run(|| {
        ...
        let mut client = new_client(&format!("http://{}", addr));
        actix::spawn(client.gas_price(Some(BlockId::BlockIndex(0))).then(|res| {
