...
fn serve<T: Send + Sync + 'static>(http_api: Arc<HttpApi<T>>, req: Request<Body>) -> BoxFut {
    match (req.method(), req.uri().path()) {
        ...
        (&Method::POST, "/submit_transaction") => {
            Box::new(req.into_body().concat2().map(move |chunk| {
                match serde_json::from_slice(&chunk) {
                    Ok(data) => match http_api.submit_transaction(&data) {
        ...
        (&Method::POST, "/call_view_function") => {
            Box::new(req.into_body().concat2().map(move |chunk| {
                match serde_json::from_slice(&chunk) {
                    Ok(data) => match http_api.call_view_function(&data) {
                        Ok(response) => build_response()
                            .body(Body::from(serde_json::to_string(&response).unwrap()))
                            .unwrap(),
                        Err(e) => build_response()
        ...
        (&Method::POST, "/view_account") => Box::new(req.into_body().concat2().map(move |chunk| {
            match serde_json::from_slice(&chunk) {
                Ok(data) => match http_api.view_account(&data) {
                    Ok(response) => build_response()
                        .body(Body::from(serde_json::to_string(&response).unwrap()))
                        .unwrap(),
                    Err(e) => build_response()
        ...
        (&Method::POST, "/view_state") => Box::new(req.into_body().concat2().map(move |chunk| {
            match serde_json::from_slice(&chunk) {
                Ok(data) => match http_api.view_state(&data) {
                    Ok(response) => build_response()
                        .body(Body::from(serde_json::to_string(&response).unwrap()))
                        .unwrap(),
                    Err(e) => build_response()
        ...
        (&Method::POST, "/get_beacon_block_by_hash") => {
            Box::new(req.into_body().concat2().map(move |chunk| {
                match serde_json::from_slice(&chunk) {
                    Ok(data) => match http_api.get_beacon_block_by_hash(&data) {
                        Ok(response) => build_response()
                            .body(Body::from(serde_json::to_string(&response).unwrap()))
                            .unwrap(),
                        Err(e) => build_response()
        ...
        (&Method::POST, "/view_latest_shard_block") => {
            Box::new(future::ok(match http_api.view_latest_shard_block() {
                Ok(response) => build_response()
                    .body(Body::from(serde_json::to_string(&response).unwrap()))
                    .unwrap(),
                Err(_) => unreachable!(),
        ...
        (&Method::POST, "/get_shard_block_by_hash") => {
            Box::new(req.into_body().concat2().map(move |chunk| {
                match serde_json::from_slice(&chunk) {
                    Ok(data) => match http_api.get_shard_block_by_hash(&data) {
                        Ok(response) => build_response()
                            .body(Body::from(serde_json::to_string(&response).unwrap()))
                            .unwrap(),
                        Err(e) => build_response()
        ...
        (&Method::POST, "/get_beacon_blocks_by_index") => {
            Box::new(req.into_body().concat2().map(move |chunk| {
                match serde_json::from_slice(&chunk) {
                    Ok(data) => match http_api.get_beacon_blocks_by_index(&data) {
                        Ok(response) => build_response()
                            .body(Body::from(serde_json::to_string(&response).unwrap()))
                            .unwrap(),
                        Err(e) => build_response()
        ...
        (&Method::POST, "/get_shard_blocks_by_index") => {
            Box::new(req.into_body().concat2().map(move |chunk| {
                match serde_json::from_slice(&chunk) {
                    Ok(data) => match http_api.get_shard_blocks_by_index(&data) {
                        Ok(response) => build_response()
                            .body(Body::from(serde_json::to_string(&response).unwrap()))
                            .unwrap(),
                        Err(e) => build_response()
        ...
        (&Method::POST, "/get_transaction_info") => {
            Box::new(req.into_body().concat2().map(move |chunk| {
                match serde_json::from_slice(&chunk) {
                    Ok(data) => match http_api.get_transaction_info(&data) {
                        Ok(response) => build_response()
                            .body(Body::from(serde_json::to_string(&response).unwrap()))
                            .unwrap(),
                        Err(e) => generate_error_response(e),
        ...
        (&Method::POST, "/get_receipt_info") => {
            Box::new(req.into_body().concat2().map(move |chunk| {
                match serde_json::from_slice(&chunk) {
                    Ok(data) => match http_api.get_receipt_info(&data) {
                        Ok(response) => build_response()
                            .body(Body::from(serde_json::to_string(&response).unwrap()))
                            .unwrap(),
                        Err(e) => generate_error_response(e),
        ...
        (&Method::POST, "/get_transaction_final_result") => {
            Box::new(req.into_body().concat2().map(move |chunk| {
                match serde_json::from_slice(&chunk) {
                    Ok(data) => match http_api.get_transaction_final_result(&data) {
                        Ok(response) => build_response()
                            .body(Body::from(serde_json::to_string(&response).unwrap()))
                            .unwrap(),
                        Err(_) => unreachable!(),
        ...
        (&Method::POST, "/get_transaction_result") => {
            Box::new(req.into_body().concat2().map(move |chunk| {
                match serde_json::from_slice(&chunk) {
                    Ok(data) => match http_api.get_transaction_result(&data) {
                        Ok(response) => build_response()
                            .body(Body::from(serde_json::to_string(&response).unwrap()))
                            .unwrap(),
                        Err(_) => unreachable!(),
...
pub fn spawn_server<T: Send + Sync + 'static>(http_api: HttpApi<T>, addr: Option<SocketAddr>) {
    ...
    let service = move || {
        ...
        service_fn(move |req| serve(http_api.clone(), req))
    };
