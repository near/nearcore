...
impl EpochManager {
    ...
    pub fn record_block_info(
        ...
        if !self.has_block_info(&current_hash)? {
            ...
            if block_info.prev_hash == CryptoHash::default() {
                ...
                println!("KRYA get block info for {:?}, {:?}", block_info.prev_hash, current_hash);
                let prev_block_info = self.get_block_info(&block_info.prev_hash)?.clone();
                println!("KRYA got block info for {:?}", block_info.prev_hash);
                ...
                println!("KRYA get epoch info for {:?}", block_info.epoch_id);
                let epoch_info = self.get_epoch_info(&block_info.epoch_id)?.clone();
                println!("KRYA got epoch info for {:?}", block_info.epoch_id);
    ...
    pub fn get_chunk_producer_info(
        ...
        println!("KRYA verify_chunk_signature_with_header_parts D");
        let epoch_info = self.get_epoch_info(epoch_id)?.clone();
        println!("KRYA verify_chunk_signature_with_header_parts E");
        let validator_id = Self::chunk_producer_from_info(&epoch_info, height, shard_id);
        println!("KRYA verify_chunk_signature_with_header_parts F");
        Ok(epoch_info.validators[validator_id as usize].clone())
    }
