...
pub struct EpochManager {
    ...
    epochs_info: LruCache<EpochId, EpochInfo>,
    /// Cache of block information.
    blocks_info: LruCache<CryptoHash, BlockInfo>,
    /// Cache of epoch id to epoch start height
    epoch_id_to_start: LruCache<EpochId, BlockHeight>,
    /// Epoch validators ordered by `block_producer_settlement`.
    epoch_validators_ordered: LruCache<EpochId, Vec<(ValidatorStake, bool)>>,
    /// Unique validators ordered by `block_producer_settlement`.
    epoch_validators_ordered_unique: LruCache<EpochId, Vec<(ValidatorStake, bool)>>,
...
impl EpochManager {
    ...
    pub fn new(
        ...
        let mut epoch_manager = EpochManager {
            ...
            epochs_info: LruCache::new(EPOCH_CACHE_SIZE),
            blocks_info: LruCache::new(BLOCK_CACHE_SIZE),
            epoch_id_to_start: LruCache::new(EPOCH_CACHE_SIZE),
            epoch_validators_ordered: LruCache::new(EPOCH_CACHE_SIZE),
            epoch_validators_ordered_unique: LruCache::new(EPOCH_CACHE_SIZE),
