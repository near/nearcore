...
pub(crate) static CHUNK_STATE_WITNESS_NETWORK_ROUNDTRIP_TIME_MS: Lazy<HistogramVec> = Lazy::new(
    || {
        try_create_histogram_vec(
            "near_chunk_state_witness_network_roundtrip_time_ms",
            "Time in milliseconds between sending state witness through the network to chunk producer and receiving the corresponding ack message",
            &["witness_size_bucket"],
