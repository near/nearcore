...
use crate::{metrics, DistributeStateWitnessRequest};
...
#[derive(Debug)]
pub enum PartialWitnessMsg {
    DistributeStateWitnessRequest(Box<DistributeStateWitnessRequest>),
    ChunkStateWitnessAckMessage(ChunkStateWitnessAckMessage),
    PartialEncodedStateWitnessMessage(PartialEncodedStateWitnessMessage),
    PartialEncodedStateWitnessForwardMessage(PartialEncodedStateWitnessForwardMessage),
    ChunkContractAccessesMessage(ChunkContractAccessesMessage),
    PartialEncodedContractDeploysMessage(PartialEncodedContractDeploysMessage),
    ContractCodeRequestMessage(ContractCodeRequestMessage),
    ContractCodeResponseMessage(ContractCodeResponseMessage),
}

#[derive(Clone)]
pub struct PartialWitnessSender(MpscSender<PartialWitnessMsg>);
...
pub struct PartialWitnessService {
    rx: Receiver<PartialWitnessMsg>,
...
impl PartialWitnessService {
    pub fn new(
        ...
        let sender = PartialWitnessSender(tx);
        sender
    }
    ...
    pub async fn run(mut self) -> Result<(), Error> {
        while let Some(msg) = self.rx.recv().await {
            match msg {
                ...
                PartialWitnessMsg::ChunkStateWitnessAckMessage(msg) => {
                    self.handle_chunk_state_witness_ack(msg.0).await;
                }

                PartialWitnessMsg::PartialEncodedStateWitnessMessage(msg) => {
                    if let Err(err) = self.handle_partial_encoded_state_witness(msg.0).await {
                        tracing::error!(target: "client", ?err, "Failed to handle PartialEncodedStateWitnessMessage");
                    }
                }

                PartialWitnessMsg::PartialEncodedStateWitnessForwardMessage(msg) => {
                    if let Err(err) = self.handle_partial_encoded_state_witness_forward(msg.0).await
                    {
                        tracing::error!(target: "client", ?err, "Failed to handle PartialEncodedStateWitnessForwardMessage");
                    }
                }

                PartialWitnessMsg::ChunkContractAccessesMessage(msg) => {
                    if let Err(err) = self.handle_chunk_contract_accesses(msg.0).await {
                        tracing::error!(target: "client", ?err, "Failed to handle ChunkContractAccessesMessage");
                    }
                }

                PartialWitnessMsg::PartialEncodedContractDeploysMessage(msg) => {
                    if let Err(err) = self.handle_partial_encoded_contract_deploys(msg.0).await {
                        tracing::error!(target: "client", ?err, "Failed to handle PartialEncodedContractDeploysMessage");
                    }
                }

                PartialWitnessMsg::ContractCodeRequestMessage(msg) => {
                    if let Err(err) = self.handle_contract_code_request(msg.0).await {
                        tracing::error!(target: "client", ?err, "Failed to handle ContractCodeRequestMessage");
                    }
                }

                PartialWitnessMsg::ContractCodeResponseMessage(msg) => {
                    if let Err(err) = self.handle_contract_code_response(msg.0).await {
    ...
    async fn handle_distribute_state_witness_request(
        &mut self,
        msg: Box<DistributeStateWitnessRequest>,
    ) -> Result<(), Error> {
    ...
    async fn handle_partial_encoded_state_witness(
        &mut self,
        ...
        let pw_clone = partial_witness.clone();
        ...
        .await
        .expect("Failed to validate partial encoded state witness");
    ...
    async fn handle_partial_encoded_state_witness_forward(
        &mut self,
        partial_witness: PartialEncodedStateWitness,
    ) -> Result<(), Error> {
    ...
    async fn handle_partial_encoded_contract_deploys(
        &mut self,
    ...
    async fn handle_chunk_state_witness_ack(&mut self, witness_ack: ChunkStateWitnessAck) {
    ...
    async fn handle_chunk_contract_accesses(
        &mut self,
    ...
    async fn handle_contract_code_request(
        &mut self,
    ...
    async fn handle_contract_code_response(
        &mut self,
