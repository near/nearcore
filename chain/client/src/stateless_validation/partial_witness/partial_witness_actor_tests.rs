...
fn forward_drops_kicked_witness_before_spawn() {
    ...
    assert_eq!(
        ...
        0,
        "kicked witness forward must be dropped by the gate before spawning validate+store",
    );
}

/// Behind-node drop on the init-emit path: a V2 part whose anchor is not
/// available locally is dropped on the actor thread, before any spawn and
/// without any deferral state. Strict anchored reads are nightly-only.
#[cfg(feature = "nightly")]
#[test]
fn init_emit_drops_v2_with_unknown_anchor_before_spawn() {
    let (chain, epoch_manager, runtime, signer) = setup(Clock::real());
    let genesis_hash = *chain.genesis().hash();
    ...
    assert_eq!(
        spawn_count.load(Ordering::SeqCst),
        0,
        "anchor-absent V2 part must be dropped before spawning validate+forward",
    );
}

/// The same part is accepted once the anchor state is available (genesis rows
/// are seeded at init): no deferral machinery needed, a re-send suffices.
#[cfg(feature = "nightly")]
#[test]
fn init_emit_accepts_v2_with_known_anchor() {
    let (chain, epoch_manager, runtime, signer) = setup(Clock::real());
    let genesis_hash = *chain.genesis().hash();
    let genesis_height = chain.genesis().height();
    let epoch_id = epoch_manager.get_epoch_id_from_prev_block(&genesis_hash).unwrap();
    ...
    assert_eq!(
        spawn_count.load(Ordering::SeqCst),
        1,
        "anchor-present V2 part must proceed to spawned validation",
    );
}
