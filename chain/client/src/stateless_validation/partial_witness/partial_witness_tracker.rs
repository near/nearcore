...
/// producer is resolved from the grandparent anchor carried by the message, two blocks at the
/// anchor height can name two different producers for the same (shard, epoch, height), and both
/// of their messages carry a valid signature.
#[derive(Clone, Debug, PartialEq, Eq, Hash)]
struct WitnessCacheKey {
    chunk: ChunkProductionKey,
    /// Grandparent anchor the message was signed against, or `None` for messages that carry no
    /// anchor, where the epoch sampler already pins exactly one producer per chunk key.
...
enum AccessedContractsState {
    ...
    Unknown,
    /// Received `ChunkContractAccesses` and sent `ContractCodeRequest`. The producer
    /// answers in several responses when the set does not fit one, so codes collect
    /// in `received` until every requested hash is in.
    Requested {
    ...
    fn try_finalize(&mut self) -> Option<(DecodePartialWitnessResult, Vec<CodeBytes>)> {
        ...
        let decode_result = match &mut self.witness_parts {
            ...
            WitnessPartsState::Decoded { .. } => {
                ...
                let WitnessPartsState::Decoded { decode_result, .. } =
                    mem::replace(&mut self.witness_parts, WitnessPartsState::Empty)
                else {
                    unreachable!()
                };
                decode_result
        ...
        let contracts: Vec<CodeBytes> = match &mut self.accessed_contracts {
            ...
            AccessedContractsState::Received(_) => {
                ...
                let AccessedContractsState::Received(contracts) =
                    mem::replace(&mut self.accessed_contracts, AccessedContractsState::Unknown)
                else {
                    unreachable!()
                };
                contracts
...
impl PartialEncodedStateWitnessTracker {
    ...
    pub fn store_partial_encoded_state_witness(
        ...
        let key = WitnessCacheKey::new(chunk_key, partial_witness.prev_prev_block_hash());
    ...
    /// The (anchor, pinned `encoded_length`) of every cache entry under `chunk`, for assertions.
    /// `None` where the entry has no parts yet or has already decoded.
    #[cfg(test)]
    pub(super) fn witness_entries(
        &self,
        chunk: &ChunkProductionKey,
    ) -> Vec<(Option<CryptoHash>, Option<usize>)> {
        let Some(shard_tracker_mutex) = self.existing_shard_tracker(chunk.shard_id) else {
            return Vec::new();
        };
        ...
        shard_tracker
            .parts_cache
            .iter()
            .filter(|(key, _)| &key.chunk == chunk)
            .map(|(key, entry)| {
                let encoded_length = match &entry.witness_parts {
                    WitnessPartsState::WaitingParts(parts) => Some(parts.encoded_length()),
                    WitnessPartsState::Empty | WitnessPartsState::Decoded { .. } => None,
                };
                (key.anchor, encoded_length)
            })
            .collect_vec()
    }

    /// The accessed-contracts state of every cache entry under `chunk`, for assertions. Sorted, so
    /// assertions do not depend on the cache's recency order.
    #[cfg(test)]
    pub(super) fn contract_states(&self, chunk: &ChunkProductionKey) -> Vec<String> {
        let Some(shard_tracker_mutex) = self.existing_shard_tracker(chunk.shard_id) else {
            return Vec::new();
        };
        let shard_tracker = shard_tracker_mutex.lock();
        shard_tracker
            ...
            .iter()
            .filter(|(key, _)| &key.chunk == chunk)
            .map(|(_, entry)| entry.accessed_contracts.metrics_label().to_owned())
            .sorted()
            .collect_vec()
    }
    ...
    #[cfg(test)]
    fn existing_shard_tracker(&self, shard_id: ShardId) -> Option<Arc<Mutex<ShardWitnessTracker>>> {
        self.shard_trackers.lock().get(&shard_id).map(Arc::clone)
    }
    ...
    /// Calls [`Self::is_anchor_on_our_chain`], which is otherwise only reachable through eviction.
    #[cfg(test)]
    pub(super) fn anchor_on_our_chain(&self, anchor: &CryptoHash) -> bool {
        self.is_anchor_on_our_chain(anchor)
    }
