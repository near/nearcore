...
fn query_client() {
    ...
    run_actix(async {
        ...
        .with_span_context();
        actix::spawn(view_client.send(request).then(|res| {
...
fn query_status_not_crash() {
    ...
    run_actix(async {
        ...
        let request = GetBlockWithMerkleTree::latest().with_span_context();
        actix::spawn(view_client.send(request).then(move |res| {
            ...
            let request = NetworkClientMessages::Block(next_block, PeerInfo::random().id, false)
                .with_span_context();
            actix::spawn(client.send(request).then(move |_| {
                let request = Status { is_health_check: true, detailed: false }.with_span_context();
                actix::spawn(client.send(request).then(move |_| {
                    System::current().stop();
                    future::ready(())
                }));
                    ...
                    if let NetworkRequests::Block { block } = msg.as_network_requests_ref() {
                        if block.header().height() > target_height {
                            ...
                            for (_, view_client) in conns.iter() {
                                ...
                                .with_span_context();
                                tests.push(actix::spawn(view_client.send(request).then(
                                    move |res| {
                            ...
                            let request = Query::new(
                                ...
                                QueryRequest::ViewAccount { account_id: "test1".parse().unwrap() },
                            )
                            .with_span_context();
                            tests.push(actix::spawn(view_client_non_archival.send(request).then(
                                move |res| {
                                    let res = res.unwrap();
                                    match res {
                            ...
                            .with_span_context();
                            tests.push(actix::spawn(view_client_archival.send(request).then(
                                move |res| {
                                    let res = res.unwrap().unwrap();
                                    match res.kind {
                                        QueryResponseKind::ViewAccount(_) => (),
