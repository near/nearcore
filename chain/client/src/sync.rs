...
impl EpochSync {
    ...
    pub fn run(
        ...
        println!("KRYA need to request? {:?}", need_to_request);
        println!(
            "KRYA need to request details: {:?} {:?} {:?} {:?} {:?}",
            !is_epoch_sync,
            received_requested_epoch,
            request_timed_out,
            self.last_request_peer_id.is_none(),
    ...
    pub fn on_response_finalize(
        ...
        store_update.save_header_head_forcibly(&tip).unwrap();
        println!("KRYA save_head is finished");
        match store_update.save_block_header_no_update_tree(header_sync_init_header.clone()) {
            Err(e) => error!("KRYA ERROR save_block_header_no_update_tree {:?}", e),
            _ => {}
        }
        ...
        match chain.sync_block_headers(prev_epoch_headers, true, |_| {}) {
            Err(e) => error!("KRYA ERROR sync_block_headers prev_epoch_headers {:?}", e),
            _ => {}
        }
        self.sync_hash = cur_epoch_header.hash().clone();
        match chain.sync_block_headers(vec![cur_epoch_header], false, |_| {}) {
            Err(e) => error!("KRYA ERROR sync_block_headers cur_epoch_header {:?}", e),
            _ => {}
        }
