...
fn new_header_response(
    ...
    StateRequestResult(Box::new(info))
}

fn new_header_response_empty(shard_id: ShardId, sync_hash: CryptoHash) -> StateRequestResult {
...
fn new_part_response_empty(shard_id: ShardId, sync_hash: CryptoHash) -> StateRequestResult {
...
impl Handler<StateRequestHeader, Option<StateRequestResult>> for StateRequestActor {
    #[perf]
    fn handle(&mut self, msg: StateRequestHeader) -> Option<StateRequestResult> {
...
impl Handler<StateRequestPart, Option<StateRequestResult>> for StateRequestActor {
    #[perf]
    fn handle(&mut self, msg: StateRequestPart) -> Option<StateRequestResult> {
