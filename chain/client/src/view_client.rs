...
impl Handler<WithSpanContext<GetExecutionOutcome>> for ViewClientActor {
    ...
    fn handle(
        ...
        match chain.get_execution_outcome(&id) {
            ...
            Err(near_chain::Error::DBNotFoundErr(_)) => {
                // TODO(#6119): Should this be chain and runtime_adapter? Or
                // just always talk to hot?  Or something even more complicated?
                let head = chain.head()?;
                let target_shard_id =
                    self.hot_runtime_adapter.account_id_to_shard_id(&account_id, &head.epoch_id)?;
                if self.hot_runtime_adapter.cares_about_shard(
                    self.validator_account_id.as_ref(),
                    &head.last_block_hash,
                    target_shard_id,
                    true,
