...
impl ClientActor {
    ...
    fn log_summary(&mut self) {
        ...
        let validator_info = if !is_syncing {
            let block_and_chunk_validators = unwrap_or_return!(self
                .client
                .runtime_adapter
                .get_epoch_block_producers_ordered(&head.epoch_id, &head.last_block_hash));
            ...
            let num_block_and_chunk = block_and_chunk_validators.len();
            let num_chunk_only_producers = num_validators.saturating_sub(num_block_and_chunk);
            ...
            Some(ValidatorInfoHelper { is_validator, num_validators, num_chunk_only_producers })
        } else {
            None
        };
