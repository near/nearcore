...
impl messaging::Actor for ClientActor {
    ...
    fn wrap_handler<M, R>(
        ...
        let _span = tracing::debug_span!("NetworkClientMessage").entered();
...
impl ClientActor {
    pub fn new(
        ...
        if let Some(vs) = &client.validator_signer.get() {
            tracing::info!(validator_id = %vs.validator_id(), "starting validator node");
    ...
    fn handle(&mut self, msg: NetworkAdversarialMessage) -> Option<u64> {
        match msg {
            ...
            NetworkAdversarialMessage::AdvSwitchToHeight(height) => {
                tracing::info!(?height, "switching to height");
            ...
            NetworkAdversarialMessage::AdvGetSavedBlocks => {
                tracing::info!("requested number of saved blocks");
            ...
            NetworkAdversarialMessage::AdvCheckStorageConsistency => {
                ...
                let timeout = 1500;
                tracing::info!(?timeout, "check storage consistency, timeout set to milliseconds");
                ...
                if store_validator.is_failed() {
                    tracing::error!(errors = ?store_validator.errors, "storage validation failed");
                    Some(0)
                } else {
            ...
            NetworkAdversarialMessage::AdvProduceChunks(adv_produce_chunks) => {
                tracing::info!(mode=?adv_produce_chunks, "setting adversary produce chunks");
            ...
            NetworkAdversarialMessage::AdvInsertInvalidTransactions(on) => {
                tracing::info!(on, "invalid transactions");
...
impl Handler<SpanWrapped<OptimisticBlockMessage>> for ClientActor {
    fn handle(&mut self, msg: SpanWrapped<OptimisticBlockMessage>) {
        ...
        tracing::debug!(block_height = optimistic_block.inner.block_height, prev_block_hash = ?optimistic_block.inner.prev_block_hash, ?from_peer, "optimistic block message");
...
impl Handler<SpanWrapped<BlockResponse>> for ClientActor {
    fn handle(&mut self, msg: SpanWrapped<BlockResponse>) {
        ...
        tracing::debug!(block_height = block.header().height(), block_hash = ?block.header().hash(), "received block response");
                ...
                Ok(epoch_id) => {
                    if let Some(hashes) = blocks_at_height.unwrap().get(&epoch_id) {
                        if !hashes.contains(block.header().hash()) {
                            tracing::warn!(block_hash = ?block.header().hash(), block_height = block.header().height(), "rejecting un-requested block");
                        }
                    }
                }
                _ => {}
...
impl Handler<SpanWrapped<BlockHeadersResponse>, Result<(), ReasonForBan>> for ClientActor {
    fn handle(&mut self, msg: SpanWrapped<BlockHeadersResponse>) -> Result<(), ReasonForBan> {
        ...
        if self.receive_headers(headers, peer_id) {
            ...
            tracing::warn!("banning node for sending invalid block headers");
...
impl Handler<SpanWrapped<BlockApproval>> for ClientActor {
    fn handle(&mut self, msg: SpanWrapped<BlockApproval>) {
        ...
        tracing::debug!(?approval, ?peer_id, "received block approval");
    ...
    fn handle(&mut self, msg: SpanWrapped<StateResponseReceived>) {
        ...
        match state_response {
            StateResponse::Ack(ref ack) => {
                tracing::trace!(%shard_id, ?hash, part_id = ?state_response.part_id_or_header(), ack_body = ?ack.body, "received state request ack");
            }
            StateResponse::State(ref state) => {
                tracing::trace!(%shard_id, ?hash, part_id = ?state_response.part_id_or_header(), size = ?state.payload_length(), "received state response");
            }
        }
        ...
        tracing::error!(
            ?hash,
            "state sync received hash that we're not expecting, potential malicious peer or a very delayed response"
        );
    }
}
...
impl ClientActor {
    ...
    fn check_send_announce_account(&mut self, prev_block_hash: CryptoHash) {
        ...
        if self.network_info.num_connected_peers == 0 {
            tracing::debug!("no peers: skip account announce");
            return;
        }
        ...
        if self.client.is_validator(&next_epoch_id) {
            tracing::debug!(validator_id = %signer.validator_id(), "sending announce account for validator");
    ...
    fn handle_block_production(&mut self) -> Result<(), Error> {
        let _span = tracing::debug_span!("handle_block_production").entered();
        // If syncing, don't try to produce blocks.
        if self.client.sync_handler.sync_status.is_syncing() {
            tracing::debug!(sync_status = ?self.client.sync_handler.sync_status, "syncing - block production disabled");
            return Ok(());
        }
        ...
        if latest_known.height + 1 <= self.client.doomslug.get_largest_height_crossing_threshold() {
            ...
            tracing::debug!(start_height, end_height, "considering blocks for production");
        } else {
            tracing::debug!(
                height = latest_known.height,
                "cannot produce any block: not enough approvals beyond height"
            );
        }
            ...
            latest_known.height + 1..=self.client.doomslug.get_largest_height_crossing_threshold()
        {
            ...
            if self.client.doomslug.ready_to_produce_block(
                ...
                if let Err(err) = self.produce_block(height) {
                    ...
                    tracing::error!(height, ?err, "block production failed");
                } else {
        ...
        if let Err(err) = self.produce_optimistic_block(optimistic_block_height) {
            ...
            tracing::error!(optimistic_block_height, ?err, "optimistic block production failed");
        }
    ...
    fn check_triggers(&mut self, ctx: &mut dyn DelayedActionRunner<Self>) -> Duration {
        let _span = tracing::debug_span!("check_triggers").entered();
            ...
            if let Some(block_height_to_shutdown) = self.client.config.expected_shutdown.get() {
                if head.height >= block_height_to_shutdown {
                    tracing::info!(
                        ...
                        ?block_height_to_shutdown,
                        "expected shutdown triggered"
                    );
    ...
    fn handle_on_post_state_ready(&mut self, msg: PostStateReadyMessage) {
        tracing::trace!(?msg, "received post state ready message");
    ...
    fn try_process_unfinished_blocks(&mut self) {
        let _span = debug_span!("try_process_unfinished_blocks").entered();
        let (accepted_blocks, errors) = self.client.postprocess_ready_blocks(
            Some(self.client.myself_sender.apply_chunks_done.clone()),
            true,
        );
        if !errors.is_empty() {
            tracing::error!(?errors, "try_process_unfinished_blocks got errors");
        }
        self.process_accepted_blocks(accepted_blocks);
    }

    fn try_handle_block_production(&mut self) {
        let _span = debug_span!("try_handle_block_production").entered();
        if let Err(err) = self.handle_block_production() {
            tracing::error!(?err, "handle block production failed")
        }
    }

    fn try_doomslug_timer(&mut self) {
        let _span = tracing::debug_span!("try_doomslug_timer").entered();
        let _ = self.client.check_and_update_doomslug_tip();
    ...
    fn produce_block(&mut self, next_height: BlockHeight) -> Result<(), Error> {
        let _span = tracing::debug_span!("produce_block", next_height).entered();
        ...
        match error {
            ...
            near_chain::Error::BlockPendingOptimisticExecution => {
                tracing::debug!("block pending optimistic execution");
                // If block is pending optimistic execution, it will be
                // processed in `postprocess_optimistic_block`.
                Ok(())
            }
            _ => {
                tracing::error!(?error, "failed to process freshly produced block");
    ...
    fn process_accepted_blocks(&mut self, accepted_blocks: Vec<CryptoHash>) {
        let _span =
            tracing::debug_span!("process_accepted_blocks", num_blocks = accepted_blocks.len())
                .entered();
        for accepted_block in accepted_blocks {
            let block = self.client.chain.get_block(&accepted_block).unwrap().clone();
            tracing::debug!(height = block.header().height(), "process_accepted_block");
    ...
    fn receive_headers(&mut self, headers: Vec<Arc<BlockHeader>>, peer_id: PeerId) -> bool {
        let _span = tracing::debug_span!("receive_headers", num_headers = headers.len(), ?peer_id)
            .entered();
        if headers.is_empty() {
            tracing::info!("received an empty set of block headers");
            return true;
        }
        match self.client.sync_block_headers(headers) {
            ...
            Err(err) => {
                if err.is_bad_data() {
                    tracing::error!(?err, "error processing sync blocks");
                    false
                } else {
                    tracing::debug!(?err, "block headers refused by chain");
                    true
                }
            }
        }
    ...
    fn catchup(&mut self, ctx: &mut dyn DelayedActionRunner<Self>) {
        {
            ...
            let _span = tracing::debug_span!("catchup").entered();
            if let Err(err) = self.client.run_catchup(
                &self.sync_jobs_sender.block_catch_up,
                Some(self.client.myself_sender.apply_chunks_done.clone()),
            ) {
                tracing::error!(?err, "error occurred during catchup for the next epoch");
            }
        }
        ...
        match sync {
            ...
            | SyncRequirement::AdvHeaderSyncDisabled => {
                if currently_syncing {
                    ...
                    tracing::debug!(prev_sync_status = ?self.client.sync_handler.sync_status, "disabling sync");
            ...
            SyncRequirement::SyncNeeded { highest_height, .. } => {
                if !currently_syncing {
                    tracing::info!(?sync, "enabling sync");
                }
    ...
    fn log_summary(&mut self) {
        let _span = tracing::debug_span!("log_summary").entered();
        let signer = self.client.validator_signer.get();
    ...
    fn maybe_receive_state_sync_blocks(&mut self, block: Arc<Block>) -> bool {
        ...
        if block_hash == sync_hash {
            ...
            if let Err(err) = self.client.chain.validate_block(&block) {
                byzantine_assert!(false);
                tracing::error!(?err, ?block_hash, "received an invalid block during state sync");
            }
            tracing::debug!(block_hash=?block.hash(), "maybe_receive_state_sync_blocks - save sync hash block");
        ...
        if &block_hash == header.prev_hash() {
            ...
            tracing::debug!(block_hash=?block.hash(), "maybe_receive_state_sync_blocks - save prev hash block");
            ...
            if let Err(err) = self.client.chain.save_block(block) {
                tracing::error!(?err, ?block_hash, "failed to save a block during state sync");
            }
            return true;
        }
        ...
        tracing::trace!(
            ?extra_block_hashes,
            "maybe_receive_state_sync_blocks: extra block hashes for state sync"
        );
        ...
        if extra_block_hashes.contains(&block_hash) {
            ...
            tracing::debug!(block_hash=?block.hash(), "maybe_receive_state_sync_blocks - save extra block");
            if let Err(err) = self.client.chain.save_block(block) {
                tracing::error!(?err, ?block_hash, "failed to save a block during state sync");
            } else {
    ...
    pub fn adv_produce_blocks_on(
        ...
        tracing::info!(num_blocks, "starting adversary blocks production");
        if only_valid {
        ...
        for height in start_height.. {
            ...
            tracing::info!(blocks_produced, num_blocks, height, "producing adversary block");
            self.network_adapter.send(PeerManagerMessageRequest::NetworkRequests(
