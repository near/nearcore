...
pub(crate) struct ChunksDelayTracker {
    ...
    chunks_in_progress: LruCache<ChunkHash, ChunkInProgress>,
}

const CHUNK_DELAY_CACHE_SIZE: usize = 1_000_000;

impl Default for ChunksDelayTracker {
    fn default() -> Self {
        Self {
            blocks_received: HashMap::new(),
            chunks_in_progress: LruCache::new(CHUNK_DELAY_CACHE_SIZE),
        }
    }
}
...
impl ChunksDelayTracker {
    pub fn add_block_timestamp(&mut self, block_hash: &CryptoHash, timestamp: Instant) {
        self.blocks_received.entry(*block_hash).or_insert(timestamp);
    }

    pub fn add_chunk_timestamp(&mut self, chunk_hash: &ChunkHash, timestamp: Instant) {
        match self.chunks_in_progress.get_mut(chunk_hash) {
            Some(entry) => {
                entry.chunk_received.get_or_insert(timestamp);
            }
            None => {
                self.chunks_in_progress.put(
                    chunk_hash.clone(),
                    ChunkInProgress { chunk_received: Some(timestamp), chunk_requested: None },
                );
            }
        }
    }
    ...
    pub fn requested_chunk(&mut self, chunk_hash: &ChunkHash, timestamp: Instant) {
        match self.chunks_in_progress.get_mut(chunk_hash) {
            Some(entry) => {
                entry.chunk_requested.get_or_insert(timestamp);
            }
            None => {
                self.chunks_in_progress.put(
                    chunk_hash.clone(),
                    ChunkInProgress { chunk_received: None, chunk_requested: Some(timestamp) },
                );
            }
        }
    }
