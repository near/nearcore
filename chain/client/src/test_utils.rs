        ...
        let clients = (0..num_clients)
            .map(|i| {
                ...
                let make_state_snapshot_callback : Option<MakeSnapshotCallback> = if self.add_state_snapshots {
                    ...
                    let snapshot : MakeSnapshotCallback = Arc::new(move |prev_block_hash, desired_flat_head, shard_uids| {
                        tracing::info!(target: "state_snapshot", ?prev_block_hash, "make_snapshot_callback");
                        runtime.get_tries().make_state_snapshot(&prev_block_hash, &desired_flat_head, &shard_uids).unwrap();
                    });
                    Some(snapshot)
                } else {
                    None
