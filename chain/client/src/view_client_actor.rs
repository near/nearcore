...
impl Handler<GetBlockProof, Result<GetBlockProofResponse, GetBlockProofError>> for ViewClientActor {
    fn handle(&mut self, msg: GetBlockProof) -> Result<GetBlockProofResponse, GetBlockProofError> {
        ...
        if block_header.is_spice() {
            // Spice: anchor on the certifying block's batch root, then that block into
            // `block_merkle_root`. `proof` carries the second hop.
            let batch =
                self.chain.spice_certified_batch_proof(&msg.block_hash, &msg.head_block_hash)?;
            return Ok(GetBlockProofResponse {
                block_header_lite: batch.block_header_lite,
                proof: batch.block_proof,
                batch_proof: Some(batch.batch_proof),
                certifying_block_header_lite: Some(batch.certifying_block_header_lite),
            });
        }
        ...
        Ok(GetBlockProofResponse {
            ...
            proof,
            batch_proof: None,
            certifying_block_header_lite: None,
        })
    }
}
