...
use near_async::messaging::{AsyncSender, IntoSender, WrappedAsyncSender};
...
impl StateSync {
    ...
    pub fn new(
        ...
        let downloader = Arc::new(StateSyncDownloader {
            ...
            header_validation_sender: WrappedAsyncSender::from_impl(chain_requests_sender.clone())
                .into_sender(),
                ...
                Entry::Vacant(entry) => {
                    ...
                    let shard_sync = run_state_sync_for_shard(
                        ...
                        *shard_id,
                        ...
                        status.clone(),
                        WrappedAsyncSender::from_impl(self.chain_requests_sender.clone())
                            .into_sender(),
                        cancel.clone(),
