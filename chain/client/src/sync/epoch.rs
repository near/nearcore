...
impl EpochSync {
    ...
    fn apply_validated_proof(
        ...
        *first_block_info_in_epoch.epoch_id_mut() = *last_header.epoch_id();
        ...
        // still key on it and would error on the synced epoch without this row:
        // `get_validator_info` (RPC), `compare_epoch_id`, and
        // `find_target_epoch_to_produce_proof_for` (serving epoch sync to other nodes).
        store_update
            .epoch_store_update()
