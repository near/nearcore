...
impl DataId {
    ...
    pub(super) fn herd(&self) -> Herd {
        match self {
            DataId::ReceiptProof { source, to_shard } => {
                Herd::ReceiptProof { from_shard: source.shard_id, to_shard: *to_shard }
            }
        }
    }
}
...
/// Items one producer set serves at successive heights. Pulls work a herd in height
/// order, `pull_window` items at a time.
#[derive(Debug, PartialEq, Eq, Hash)]
pub(super) enum Herd {
    ReceiptProof { from_shard: ShardId, to_shard: ShardId },
}
