...
use crate::spice::data_manager::{
    PartsOutcome, Policies, PullConfig, PullRequest, SenderFault, SpiceData, SpiceDataManager,
...
impl Handler<ProcessedBlock> for SpiceDataDistributorActor {
    fn handle(&mut self, ProcessedBlock { block_hash, certified_frontier }: ProcessedBlock) {
        ...
        match self.data_manager.on_block_processed(
            &block_hash,
            &certified_frontier,
            self.clock.now(),
        ) {
...
impl SpiceDataDistributorActor {
    pub fn new(
        ...
        let data_manager = SpiceDataManager::new(
            PullConfig {
                max_ids_per_request: MAX_REQUESTED_DATA_IDS,
                max_parts_per_request: MAX_REQUESTED_PARTS,
                ..PullConfig::default()
            },
            DATA_PARTS_RATIO,
            chain_store.clone(),
