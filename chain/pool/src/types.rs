...
/// Trait acts like an iterator. It iterates over transactions groups by
/// returning mutable references to them. Each transaction group implements a
/// draining iterator to pull transactions. The order of the transaction groups
/// is round robin scheduling. When this iterator is dropped the remaining
/// transactions are returned back to the pool.
...
pub struct TransactionGroup {
    ...
    /// Hashes of the transactions that were pulled from the group using
    /// `.next()`.
    pub(crate) removed_transaction_hashes: Vec<CryptoHash>,
}

impl TransactionGroup {
    /// Returns the next transaction with the smallest nonce and removes it from
    /// the group. It also stores all hashes of returned transactions.
