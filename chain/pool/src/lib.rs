...
impl TransactionPool {
    ...
    /// became invalid.
    pub fn remove_transactions(&mut self, validated_txs: &[ValidatedTransaction]) {
        ...
        for validated_tx in validated_txs {
            ...
            if !self.unique_transactions.remove(&validated_tx.get_hash()) {
                continue;
            }
...
impl<'a> TransactionGroupIterator for PoolIteratorWrapper<'a> {
    fn next(&mut self) -> Option<&mut TransactionGroup> {
        if !self.pool.transactions.is_empty() {
            ...
            transactions.sort_by_key(|vt| std::cmp::Reverse(vt.nonce()));
