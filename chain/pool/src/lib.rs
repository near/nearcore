...
impl TransactionPool {
    ...
    pub fn remove_transactions(&mut self, transactions: &[ValidatedTransaction]) {
        ...
        for tx in transactions {
            ...
            let signer_public_key = tx.to_tx().public_key();
...
impl<'a> TransactionGroupIterator for PoolIteratorWrapper<'a> {
    fn next(&mut self) -> Option<&mut TransactionGroup> {
        if !self.pool.transactions.is_empty() {
            ...
            transactions.sort_by_key(|vt| std::cmp::Reverse(vt.to_tx().nonce()));
...
mod tests {
    ...
    fn test_remove_transactions() {
        ...
        expected_txs.sort_by_key(|tx| tx.to_tx().nonce());
        let expected_txs =
    ...
    fn test_pool_iterator() {
        ...
        let mut nonces: Vec<_> = res.into_iter().map(|tx| tx.to_tx().nonce()).collect();
