    ...
    pub(crate) fn from_error_kind(err: ErrorKind) -> Self {
        match err {
            ErrorKind::InvalidInput(message) => {
                Self { code: 400, message: format!("InvalidInput: {}", message), retriable: true }
            }
            ErrorKind::NotFound(message) => {
                Self { code: 404, message: format!("NotFound: {}", message), retriable: true }
            }
            ErrorKind::WrongNetwork(message) => {
                Self { code: 404, message: format!("WrongNetwork: {}", message), retriable: true }
            }
            ErrorKind::Timeout(message) => {
                Self { code: 504, message: format!("Timeout: {}", message), retriable: true }
            }
            ErrorKind::Other(message) => {
                Self { code: 500, message: format!("Other: {}", message), retriable: true }
            }
        }
    }
}
...
#[derive(Debug, Clone, PartialEq, serde::Serialize, serde::Deserialize, Apiv2Schema)]
pub(crate) struct MempoolRequest {
    pub network_identifier: NetworkIdentifier,
}
...
pub(crate) struct Operation {
    ...
    /// can be returned here is also specified in the NetowrkStatus. This can
