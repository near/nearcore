...
pub enum Error {
    ...
    /// `prev_spice_chunk_endorsement_stats` on a spice block header doesn't
    /// match the per-ChunkExecutionResult bitmaps derived from prev block's body.
    #[error("Invalid prev_spice_chunk_endorsement_stats: {0}")]
    ...
    pub fn prometheus_label_value(&self) -> &'static str {
        match self {
            ...
            Error::InvalidSpiceChunkEndorsementStats(_) => {
                "invalid_prev_spice_chunk_endorsement_stats"
            }
        }
    }
}
