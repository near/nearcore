use crate::Block;
use crate::types::{ApplyChunkBlockContext, ApplyChunkResult, RuntimeAdapter, Tip};
...
/// Writes spice's two execution-head columns into the caller-provided
/// `store_update` (does not commit). Returns `Some(new_final_tip)` when the
/// final-execution-head advanced — signals the caller to run flat-storage
/// advance + memtrie GC; `None` otherwise.
pub fn apply_block_postprocessing(
    store_update: &mut StoreUpdate,
    block: &Block,
) -> Result<Option<Arc<Tip>>, Error> {
    let new_final_head =
        store_update.chain_store_update().update_spice_final_execution_head(block)?;
    store_update
        .chain_store_update()
