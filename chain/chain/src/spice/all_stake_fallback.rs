...
fn blocks_between_fallback_only_chunks(
    ...
    (blocks_between > 0).then_some(blocks_between)
}
...
pub(super) fn is_fallback_only_height_for_shard_index(
    ...
    let Some(blocks_between) = blocks_between_fallback_only_chunks(epoch_length, num_shards) else {
        return false;
    };
    if !height.is_multiple_of(blocks_between) {
        return false;
    }
    shard_index as u64 == (height / blocks_between + epoch_height) % num_shards as u64
}
