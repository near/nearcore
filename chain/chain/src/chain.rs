...
impl Chain {
    ...
    pub fn get_state_response_header(
        ...
        // Check cache
        let key = StateHeaderKey(shard_id, borsh::to_vec(&sync_hash))?;
    ...
    pub fn set_state_part(
        ...
        // Saving the part data.
        let mut store_update = self.store.store().store_update();
        let key = StatePartKey(sync_hash, shard_id, borsh::to_vec(&part_id.idx))?;
...
impl Chain {
    ...
    pub fn build_receipts_hashes(
        ...
        shard_receipts
            .into_iter()
            .map(|(i, rs)| {
                let bytes = (i, borsh::to_vec(&rs)).unwrap();
                hash(&bytes)
            })
            .collect()
    }
