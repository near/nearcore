...
impl<'a> ChainUpdate<'a> {
    ...
    fn update_head(&mut self, header: &BlockHeader) -> Result<Option<Tip>, Error> {
        ...
        if header.height() > head.height {
            ...
            tracing::debug!(target: "chain", last_block_hash = ?tip.last_block_hash, height = %tip.height, prev_head_height = head.height, "head updated");
            Ok(Some(tip))
        } else {
            tracing::debug!(target: "chain", block_hash = ?header.hash(), block_height = header.height(), head_height = head.height, "head NOT updated (not higher)");
            Ok(None)
        }
    }
