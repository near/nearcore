...
mod test {
    ...
    // we add #[allow(dead_code)] to everything here because for some reason this generates
    // dead code warnings even though everything is very clearly used below... maybe it's this?
    ...
    async fn test_raw_connection() {
        for t in TESTCASES.iter() {
            ...
            let conn = raw::Connection::connect(
                addr,
                peer_id,
                None,
                "unittestnet",
