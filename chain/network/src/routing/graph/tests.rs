...
impl routing::Graph {
    async fn check(&self, want_mem: &[Edge], want_db: &[Component]) {
        ...
        let got_db: HashSet<_> = self.inner.lock().store.await.list_components().into_iter().map(|c| c.normal()).collect();
...
async fn one_edge() {
    ...
    clock.advance(2 * SEC);
    g.update_routing_table(&clock.clock(), vec![]).await;
    g.check(&[], &[Component { edges: vec![e1v2.clone()], peers: vec![p1.clone()] }]);
}
...
async fn components_nonces_are_tracked_in_storage() {
    ...
    let store = store();
    let g = routing::GraphWithCache::new(cfg,store.clone());
    
    // Add an inactive edge and prune it.
    let p1 = data::make_peer_id();
    let e1 = edge(&cfg.node_id, &p1, 2);
    g.update_routing_table(&clock.clock(), vec![e1.clone()]).await;
    g.check( &[], &[Component { edges: vec![e1.clone()], peers: vec![p1.clone()] }]);

    // Add an active unreachable edge, which also should get pruned.
    let p2 = data::make_peer_id();
    let p3 = data::make_peer_id();
...
async fn expired_edges() {
    ...
    // Add an active edge.
    g.update_routing_table(&clock.clock(), vec![e1.clone(), old_e2.clone()]).await;
    ...
    g.update_routing_table(&clock.clock(), vec![still_old_e2.clone()]).await;
    g.check(&[e1.clone()], &[]);

    // But adding the fresh edge should work.
    g.update_routing_table(&clock.clock(), vec![fresh_e2.clone()]).await;
    ...
    let e1v2 = edge(&cfg.node_id, &p1, to_active_nonce(clock.utc_now()) + 1);
    g.upate_routing_table(&clock.clock(), vec![e1v2.clone()]);
    g.check(&[e1v2.clone()], &[]);
