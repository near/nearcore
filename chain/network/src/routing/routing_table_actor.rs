...
pub struct RoutingTableActor {
    ...
    /// DBCol::ColLastComponentNonce -> stores component_nonce: u64, which is the lowest nonce that
    ...
    /// DBCol::ColPeerComponent      -> Mapping from `peer_id` to last component nonce if there
...
impl RoutingTableActor {
    pub fn new(my_peer_id: PeerId, store: Store) -> Self {
        let component_nonce = store
            .get_ser::<u64>(DBCol::ColLastComponentNonce, &[])
            .unwrap_or(None)
    ...
    fn prune_unreachable_edges_and_save_to_db(
        ...
        let _ =
            update.set_ser(DBCol::ColLastComponentNonce, &[], &self.next_available_component_nonce);
    ...
    /// Get edges stored in DB under `DBCol::ColPeerComponent` column at `peer_id` key.
    fn component_nonce_from_peer(&self, peer_id: &PeerId) -> Result<u64, ()> {
        match self
            .store
            .get_ser::<u64>(DBCol::ColPeerComponent, peer_id.try_to_vec().unwrap().as_ref())
        {
    ...
    fn get_and_remove_component_edges(
        ...
        let res =
            match self.store.get_ser::<Vec<Edge>>(DBCol::ColComponentEdges, enc_nonce.as_ref()) {
                Ok(Some(edges)) => Ok(edges),
                _ => Err(()),
            };

        update.delete(DBCol::ColComponentEdges, enc_nonce.as_ref());
