...
// We'd treat all nonces that are below this values as 'old style' (without any
// expiration time). And all nonces above this value as new style (that would
// expire after some time). This value is set to August 8, 2022.
...
impl Edge {
    ...
    /// Helper function when adding a new edge and we receive information from
    /// new potential peer to verify the signature.
    ...
    pub fn create_fresh_nonce(clock: &time::Clock) -> u64 {
        ...
        // Even nonce means that the edge should be removed, so if the timestamp is
        // even, add one to get the odd value.
        if nonce % 2 == 0 {
            nonce += 1;
        }
        nonce
    ...
    /// It will be considered as a new edge if the nonce is odd, otherwise it is
    /// canceling the previous edge.
...
/// An `Edge` represents a direct connection between two peers in Near Protocol
/// P2P network.
///
/// Note that edge might either in `Active` or `Removed` state.
/// We need to keep explicitly `Removed` edges, in order to be able to proof,
/// that given `Edge` isn't `Active` anymore. In case, someone delivers a proof
/// that the edge existed.
#[derive(BorshSerialize, BorshDeserialize, Clone, Debug, PartialEq, Eq, Hash)]
#[cfg_attr(feature = "test_features", derive(serde::Serialize, serde::Deserialize))]
pub struct EdgeInner {
    ...
    /// Odd number indicates that `edge` has been added, `even` number that it
    /// was removed. New edge starts with value of `1`.
    /// We update the edge only if it's `nonce` is higher. All versions of
    /// `Edge` with lower `nonce` will be ignored.
    nonce: u64,
    /// Each `edge` consists of two signatures, one for each `peer`.
    /// It's generated by signing triple (key.0, key.1, nonce) by each `peer`
    /// private key. `Signature` is generated at the time when edge is
    /// added, that is when `nonce` is `odd`. `Signature` can be verified by
    /// checking `peers` `PublicKey` against the signature. `Signature` from
    /// peer `key.0`.
    signature0: Signature,
    ...
    /// - `nonce` is even, then the structure will be a pair with a signature of
    ///   the party removing the edge:
    ///           - `bool` - `false` if `peer0` signed the `edge` `true` if
    ///             `peer1`.
    ///           - `Signature` - `Signature` of either `peer0` or `peer1`,
    ///             depending on which peer
    ///           removed the edge.
...
/// Every edge starts in `Active` state. It can be removed and go to `Removed`
/// state, and then added back to go to `Active` state, etc.
#[derive(BorshSerialize, BorshDeserialize, Clone, PartialEq, Eq, Debug, Hash)]
pub enum EdgeState {
    /// `Edge` is `Active` if there is an active connection between two peers on
    /// the network.
    Active,
    /// `Edge` is in `Removed` state if it was previously in `Active` state, but
    /// has been removed. A signature of one of the peers is requires,
    /// otherwise the edge will stay active. Though, it may be removed  from
    /// memory if both peers become unreachable.
    Removed,
}
