...
impl<Arg: 'static + Send, Res: 'static + Send> BatchProcessor<Arg, Res> {
    ...
    /// This matches the demux API: call(arg, handler) where handler processes Vec<Arg> -> Vec<Res>
    pub async fn call<F, Fut>(
        &self,
        arg: Arg,
        handler: F,
    ) -> Result<Res, ProcessorStoppedError>
    where
    ...
    async fn process_batch(batch: Vec<BatchRequest<Arg, Res>>) {
        ...
        // Use the first handler (matching demux behavior)
...
mod tests {
    ...
    async fn test_batch_processor_basic() {
        let processor = BatchProcessor::new(RateLimit { qps: 10.0, burst: 5 });
    ...
    async fn test_rate_limiting() {
        let processor = BatchProcessor::new(RateLimit { qps: 2.0, burst: 1 });
    ...
    async fn test_batching() {
        ...
        let processor = BatchProcessor::new(RateLimit { qps: 1.0, burst: 1000 });
