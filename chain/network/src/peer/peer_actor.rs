...
pub(crate) struct PeerActor {
    ...
    /// XXX: Changed to use CryptoHash instead of Signature.
    routed_message_cache: LruCache<(PeerId, PeerIdOrHash, CryptoHash), time::Instant>,
    ...
    fn handle_msg_ready(
        ...
        match peer_msg {
            ...
            PeerMessage::Routed(mut msg) => {
                ...
                let key = (msg.author().clone(), msg.target().clone(), msg.hash());
                let now = self.clock.now();
