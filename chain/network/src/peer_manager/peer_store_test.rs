...
fn ban_store() {
    ...
    {
        let store = schema::Store::new(StoreOpener::with_default_config(tmp_dir.path()).open());
        let mut peer_store =
    ...
    {
        let store_new = schema::Store::new(StoreOpener::with_default_config(tmp_dir.path()).open());
        let peer_store_new =
...
fn test_unconnected_peer() {
    ...
    {
        let store =
            schema::Store::new(StoreOpener::with_default_config(tmp_dir.path()).open());
        let peer_store =
            PeerStore::new(&clock.clock(), store, &boot_nodes, Default::default()).unwrap();
...
fn assert_peers_in_store(store_path: &std::path::Path, want: &[PeerId]) {
    let store = schema::Store::new(StoreOpener::with_default_config(store_path).open());
    let got: HashSet<PeerId> = store.iter::<schema::Peers>().map(|x| x.unwrap().0).collect();
