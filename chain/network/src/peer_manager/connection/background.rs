    ...
    pub(crate) async fn spawn(
        ...
        if tier==tcp::Tier::T2 {
            ...
            s.spawn(async {
                ...
                loop {
                    interval.tick().await?;
                    conn.send_message(&PeerMessage::PeersRequest);
                }
            });
            ...
            s.spawn(async {
                ...
                loop {
                    ...
                    if let Some(chain_info) = state.chain_info.load().as_ref() {
                        conn.send(&PeerMessage::Block(chain_info.block.clone())).await?;
                    }
                }
            });
