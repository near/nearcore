...
/// Peers which last message is was within this period of time are considered
/// active recent peers.
...
/// If non-zero - we'll skip sending tombstones during initial sync and for that
...
pub struct Config {
    ...
    /// Examples:
    ///   ed25519:86EtEy7epneKyrcJwSWP7zsisTkfDRH5CFVszt4qiQYw@31.192.22.209:
    /// 24567   ed25519:86EtEy7epneKyrcJwSWP7zsisTkfDRH5CFVszt4qiQYw@
    /// nearnode.com:24567
    pub boot_nodes: String,
    /// Comma separated list of whitelisted nodes. Inbound connections from the
    /// nodes on the whitelist are accepted even if the limit of the inbound
    /// connection has been reached. For each whitelisted node specifying
    /// both PeerId and one of IP:port or Host:port is required: Examples:
    ///   ed25519:86EtEy7epneKyrcJwSWP7zsisTkfDRH5CFVszt4qiQYw@31.192.22.209:
    /// 24567   ed25519:86EtEy7epneKyrcJwSWP7zsisTkfDRH5CFVszt4qiQYw@
    /// nearnode.com:24567
    #[serde(default)]
    ...
    /// considered active recent peers (in seconds).
    ...
    /// It can be IP:Port or IP (to blacklist all connections coming from this
    /// address).
    #[serde(default)]
    pub blacklist: Vec<String>,
    /// Time to persist Accounts Id in the router without removing them in
    /// seconds.
    ...
    /// List of the public addresses (in the format "<node public
    /// key>@<IP>:<port>") of trusted nodes, which are willing to route
    /// messages to this node. Useful only if this node is a validator. This
    /// list will be signed and broadcasted to the whole network, so that
    /// everyone knows how to reach the validator.
    ///
    /// Example:
    ///   ["ed25519:86EtEy7epneKyrcJwSWP7zsisTkfDRH5CFVszt4qiQYw@31.192.22.209:
    /// 24567"]
    ...
    /// In the simplest case this list should contains just 1 public address
    /// (with the node public key) of this validator.
    /// In case the validator doesn't have a public IP (i.e. it is hidden in a
    /// private network), this list should contain public addresses of the
    /// trusted nodes which will be routing messages to the validator -
    /// validator will connect to these nodes immediately after startup.
    /// TODO(gprusak): in case a connection cannot be established (the peer is
    /// unreachable/down/etc.) validator should probably remove (temporarily)
    /// the problematic peer from the list and broadcast the new version of
    /// the list.
    ///
    /// Less recommended setup (requires exactly one public dynamic/ephemeral or
    /// static IP): If the list is empty, the validator node will query
    /// trusted_stun_servers to determine its own IP. Only if the answer
    /// from the STUN servers is unambiguous (at least 1 server responds and
    /// all received responses provide the same IP), the IP (together with the
    /// port deduced from the addr field in this config) will be signed and
    /// broadcasted.
    ///
    /// Discouraged setup (might be removed in the future)
    /// If the list is empty and STUN servers' response is ambiguous, the peers
    /// which connect to this validator node will natually observe the
    /// address of the validator and broadcast it. This setup is not
    /// reliable in presence of byzantine peers.
    #[serde(default)]
    pub public_addrs: Vec<PeerAddr>,
    /// For local tests only (localnet). Allows specifying IPs from private
    /// range (which are not visible from the public internet) in
    /// public_addrs field.
    #[serde(default)]
    ...
    /// example `stun.l.google.com:19302`. The STUN servers are queried
    /// periodically in parallel. We do not expect all the servers listed to
    /// be up all the time, but all the responses are expected to be
    /// consistent - if different servers return differn IPs, then
    /// the response set would be considered ambiguous and the node won't
    /// advertise any proxy in such a case.
    #[serde(default = "default_trusted_stun_servers")]
    pub trusted_stun_servers: Vec<stun::ServerAddr>,
    // Experimental part of the JSON config. Regular users/validators should not have to set any
    // values there. Field names in here can change/disappear at any moment without warning.
    #[serde(default)]
