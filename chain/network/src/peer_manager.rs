...
impl PeerManagerActor {
    ...
    fn most_weight_peers(&self) -> Vec<FullPeerInfo> {
        // This finds max of weight and block index and returns such peer.
        let max_weight_and_score = match self
            ...
            .map(|active_peers| {
                (
                    ...
                    active_peers.full_peer_info.chain_info.block_index,
                )
            })
            .max()
        {
        ...
        // Find all peers whose block_index is within `most_weighted_peer_block_index_horizon` from max weight peer(s).
        self.active_peers
            .values()
