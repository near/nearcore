...
impl From<RpcViewCodeError> for crate::errors::RpcError {
    fn from(error: RpcViewCodeError) -> Self {
        ...
        let error_data_value = match serde_json::to_value(&error) {
            ...
            Err(err) => {
                return Self::new_internal_error(
                    None,
                    format!("Failed to serialize RpcViewCodeError: {:?}", err),
                );
            }
        };
...
impl From<crate::types::query::RpcQueryError> for RpcViewCodeError {
