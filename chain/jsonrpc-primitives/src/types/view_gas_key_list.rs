...
impl From<RpcViewGasKeyListError> for crate::errors::RpcError {
    fn from(error: RpcViewGasKeyListError) -> Self {
        ...
        let error_data_value = match serde_json::to_value(&error) {
            ...
            Err(err) => {
                return Self::new_internal_error(
                    None,
                    format!("Failed to serialize RpcViewGasKeyListError: {:?}", err),
                );
            }
        };
...
impl From<crate::types::query::RpcQueryError> for RpcViewGasKeyListError {
