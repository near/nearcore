...
impl From<RpcViewAccessKeyListError> for crate::errors::RpcError {
    fn from(error: RpcViewAccessKeyListError) -> Self {
        ...
        let error_data_value = match serde_json::to_value(&error) {
            ...
            Err(err) => {
                return Self::new_internal_error(
                    None,
                    format!("Failed to serialize RpcViewAccessKeyListError: {:?}", err),
                );
            }
        };
...
impl From<crate::types::query::RpcQueryError> for RpcViewAccessKeyListError {
