...
pub trait ShardsManagerAdapterForClient: Send + Sync + 'static {
    /// Processes the header seen from a block we received, if we have not
    /// already received the header earlier from the chunk producer (via
    /// PartialEncodedChunk). This can happen if we are not a validator, or
    /// if we are a validator but somehow missed the chunk producer's
    /// message.
    ...
    /// As a chunk producer, distributes the given chunk to the other validators
    /// (by sending PartialEncodedChunk messages to them).
    /// The partial_chunk and encoded_chunk represent the same data, just in
    /// different formats.
    ///
    ...
    /// Only the parts and receipt proofs that this node cares about will be
    /// fetched; when the fetching is complete, a response of
    /// ClientAdapterForShardsManager::did_complete_chunk will be sent back
    /// to the client.
    fn request_chunks(&self, chunks_to_request: Vec<ShardChunkHeader>, prev_hash: CryptoHash);
    /// Similar to request_chunks, but for orphan chunks. Since the chunk
    /// belongs to an orphan block, the previous block is not known and thus
    /// we cannot derive epoch information from that block. Therefore, an
    /// ancestor_hash must be provided which must correspond to a block that
    /// is an ancestor of these chunks' blocks, and which must be in the same
    /// epoch.
    ...
    /// In response to processing a block, checks if there are any chunks that
    /// should have been complete but are just waiting on the previous block
    /// to become available (e.g. a chunk requested by
    /// request_chunks_for_orphan, which then received all needed parts and
    /// receipt proofs, but cannot be marked as complete because the
    /// previous block isn't available), and completes them if so.
