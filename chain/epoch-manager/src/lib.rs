...
impl EpochManager {
    ...
    ) -> (HashMap<AccountId, BlockChunkValidatorStats>, HashMap<AccountId, ValidatorKickoutReason>)
    {
        ...
        if all_kicked_out {
            tracing::info!(
                ...
                ?max_validator,
                "about to kick out all validators in the next two epochs, saving one"
            );
    ...
    fn collect_blocks_info(
        ...
        debug!(
            ...
            ?block_validator_tracker,
            ?chunk_validator_tracker,
            "all proposals and validator stats"
        );
        ...
        let next_next_epoch_info = match proposals_to_epoch_info(
            ...
            Err(EpochError::ThresholdError { stake_sum, num_seats }) => {
                warn!(
                    ...
                    %stake_sum,
                    num_seats,
                    "not enough stake for required number of seats, all validators may have \
                     tried to unstake"
                );
            ...
            Err(EpochError::NotEnoughValidators { num_validators, num_shards }) => {
                warn!(
                    ...
                    num_validators,
                    num_shards,
                    "not enough validators for required number of shards, all validators may \
                     have tried to unstake"
                );
                let mut epoch_info = EpochInfo::clone(&next_epoch_info);
                *epoch_info.epoch_height_mut() += 1;
                epoch_info
    ...
    pub fn compute_stake_return_info(
        ...
        debug!(
            ...
            ?next_epoch_id,
            ?epoch_id,
            "computing stake return info for epochs"
        );
        ...
        debug!(target: "epoch_manager", ?stake_info, ?validator_reward, "computed stake return info");
