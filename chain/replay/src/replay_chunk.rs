...
use near_chain::{
    Chain, ChainStore, ChainStoreAccess, ReceiptFilter, get_incoming_receipts_for_shard,
};
...
pub fn replay_chunk(
...
) -> Result<ChunkReplayResult> {
    let span = tracing::debug_span!(target: "replay", "replay_chunk").entered();

    let block = chain_store.get_block(block_hash).context("failed to get block")?;
    let block_header = block.header();
    let height = block_header.height();

    let prev_block =
        chain_store.get_block(block_header.prev_hash()).context("failed to get prev block")?;

    let shard_id = shard_uid.shard_id();
    let epoch_id = block_header.epoch_id();
    let shard_index = shard_id_to_index(epoch_manager, shard_id, epoch_id)
        .context("failed to get shard index")?;

    let chunk_header = block.chunks()[shard_index].clone();
    let prev_chunk_header = epoch_manager
        .get_prev_chunk_header(&prev_block, shard_id)
        .context("failed to get prev chunk header")?;

    let is_new_chunk = chunk_header.is_new_chunk(height);

    let prev_chunk_extra = chain_store
        .get_chunk_extra(block_header.prev_hash(), &shard_uid)
    ...
    let update_reason = if is_new_chunk {
        let chunk =
            chain_store.get_chunk(&chunk_header.chunk_hash()).context("failed to get chunk")?;
    ...
    let expected_chunk_extra = chain_store
        .get_chunk_extra(block_header.hash(), &shard_uid)
        .context("failed to get expected chunk extra")?;
