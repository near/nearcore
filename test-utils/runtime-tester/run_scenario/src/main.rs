...
fn main() {
    let matches = App::new("Run scenario")
        .arg(Arg::with_name("scenario").long("scenario").required(true).takes_value(true).help("Scenario to run"))
        .get_matches();
    ...
    for block_stats in runtime_stats.blocks_stats {
        if block_stats.block_production_time > Duration::from_secs(1) {
            println!("Time to produce block {} is {:?}", block_stats.height, block_stats.block_production_time);
        }
    }
}