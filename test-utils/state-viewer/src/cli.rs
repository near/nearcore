...
pub enum StateViewerSubCommand {
    ...
    /// Dump all **distinct** contracts from the current DB state to the given folder. It will contain WASM codes named
    /// as `{contract_id}.wasm` where `contract_id` is lexicographically smallest for this code.
    ///
    /// As of 13/12/2021, takes 1m to run, size of resulting folder is ~100 MB.
...
pub struct DumpContractsCmd {
    /// Folder to which contracts will be saved.
    #[clap(long, parse(from_os_str))]
    output: PathBuf,
}
