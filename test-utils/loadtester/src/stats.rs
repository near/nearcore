...
impl Stats {
    ...
    pub async fn measure_from(&mut self, node: &RemoteNode) {
        self.from_height = Some(node.get_current_height().await.unwrap());
        self.from_timestamp = Some(Instant::now());
    }

    /// Measure stats to this moment.
    pub async fn measure_to(&mut self, node: &RemoteNode) {
        self.to_height = Some(node.get_current_height().await.unwrap());
