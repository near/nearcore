...
STUDIO_IMAGE=${2:-nearprotocol/studio:0.2.7}
TOTAL_NODES=${2:-2}
NEARLIB_COMMIT="348509b526cf4ca0495d86cb211d1013d84629a2"
NEARLIB_VERSION="0.5.2"
STUDIO_IP=localhost

sudo docker run -d --name near-testnet-0 -p 3030:3030 -p 26656:26656 --rm \
	-e "NODE_ID=0" \
...
do
sudo docker run -d --name near-testnet-${NODE_ID} -p $((3030+${NODE_ID})):3030 -p $((26656+${NODE_ID})):26656 \
    --add-host=near-testnet-0:172.17.0.2 --rm \
