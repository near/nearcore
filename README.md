...
To spin multiple nodes, you must first spin up one node and then use it as boot node for all the rest:
    ...
    cargo run -- --p2p_port 30333 --rpc_port 3030 --base-path=test1

    cargo run -- --p2p_port 30334 --rpc_port 3031 --base-path=test2 --boot-node /ip4/127.0.0.1/tcp/30333/public_key 

Where `public_key` is the public key of the first node that it prints in the beginning. 
