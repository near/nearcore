...
TEST=test_shard_layout_upgrade_incoming_receipts_impl_v2
...
do
    ...
    # RUST_LOG=debug,chunks=info,chain=debug,test=trace,catchup=trace,resharding=trace \
    # RUST_LOG=info,test=trace,client=trace \
    # RUST_LOG=debug,test=debug,state_snapshot=debug,client=debug \
    RUST_BACKTRACE=all \
    RUST_LOG=trace \
