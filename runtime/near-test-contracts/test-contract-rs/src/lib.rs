...
fn str_as_utf16_bytes(input: &str) -> Vec<u8> {
    let mut result: Vec<u8> = Vec::new();
    for u16_ in input.encode_utf16() {
        result.push(u16_ as u8);
        result.push((u16_ >> 8) as u8);
    }
    result
}
...
pub unsafe fn sanity_check() {
    ...
    let value_utf16 = str_as_utf16_bytes("hello");
