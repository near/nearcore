...
impl Runtime {
    ...
    fn process_transactions(
        ...
        let len = tx_vec.len();
        // FIXME: the magic numbers 6 & 4.
        let chunk_size =
            (tx_vec.len() / 6 / rayon::current_num_threads()).clamp(4, ValidBitmask::BITS as _);
        ...
        for (idx, tx) in tx_vec.iter().enumerate() {
            metrics::TRANSACTION_PROCESSED_TOTAL.inc();

            let validity_bitmask = valid_masks[idx / chunk_size];
            let is_valid = (validity_bitmask & (1 << (idx % chunk_size))) != 0;
            if !is_valid {
