...
pub(crate) fn execute_function_call(
    ...
    let mut outcome = match result {
        Err(VMRunnerError::ContractCodeNotPresent) => {
            // During witness validation a missing body for an account that
            // commits to a code hash means the state is incomplete, so fail like
            // any other missing trie value.
