...
impl wasm_smith::Config for ModuleConfig {
    ...
    fn available_imports(&self) -> Option<std::borrow::Cow<'_, [u8]>> {
        static MODULE: OnceCell<Vec<u8>> = OnceCell::new();
        Some(MODULE.get_or_init(|| filter_module_imports(rs_contract())).into())
    }
}

/// Returns a compiled WebAsssembly module which describes the imports of the
/// input module, discarding everything else.
///
/// Panics if it fails to construct the module.
fn filter_module_imports(bytes: &[u8]) -> Vec<u8> {
    let text = wasmprinter::print_bytes(bytes).unwrap();
    let to_keep = text
        .lines()
        .filter(|line| {
            let line = line.trim_start();
            // WAT generated by `wasmprinter` defines types and uses them in imports, for example:
            // (type $t (func (param i64)))
            // (import "env" "input" (func $env.input (type $t)))
            line.starts_with("(type") || line.starts_with("(import")
        })
        .collect::<Vec<_>>();

    // Check at least *some* imports were grabbed.
    to_keep
        .iter()
        .find(|line| line.trim_start().starts_with("(import"))
        .expect("failed to extract imports");

    let mut new_module = vec!["(module"];
    new_module.extend(to_keep);
    new_module.push(")");
    wat::parse_str(new_module.join("")).unwrap()
}
