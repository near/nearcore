...
// Note, that VMKind is part of serialization protocol, so we cannot remove
// entries from this list if particular VM reached publicly visible networks.
//
// Additionally, this is public only for the purposes of internal tools like
// thea estimator. This API should otherwise be considered a private
// implementation detail of the `near-vm-runner` crate.
pub enum VMKind {
    /// Wasmer 0.17.x VM.
    Wasmer0,
    /// Wasmtime VM.
