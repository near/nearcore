...
pub(crate) fn default_wasmer2_store() -> Store {
    if WASMER2_CONFIG.use_native_engine {
        let engine = if WASMER2_CONFIG.use_cranelift {
            wasmer_engine_dylib::Dylib::new(wasmer_compiler_cranelift::Cranelift::default())
                .engine()
        } else {
            wasmer_engine_dylib::Dylib::new(wasmer_compiler_singlepass::Singlepass::default())
                .engine()
        };
        Store::new(&engine)
    } else {
