...
///
/// With `max_globals_per_contract` set to 100k this contract sits exactly at
/// the cap (`100k <= 100k`), so it still passes `prepare` and is caught by the
/// Wasmtime backstop at load time. This test guards the backstop, not the
/// prepare-phase gate (which is covered by `prepare::tests::too_many_globals`).
///
