...
/// When `Config::p256_verify` is false (i.e. the contract is executed under a
/// protocol version prior to 85), importing `env.p256_verify` must fail at
/// link time. This test guards the feature gate: if someone accidentally
/// stops feature-gating the import, this test will flip to passing at a
/// pre-gate protocol version.
#[test]
fn test_p256_verify_integration_gated_off_fails_to_link() {
    let message = [7u8; 32];
    let (signature, public_key) = sign_and_keys(&message);
    let wat = verify_wat(&signature, &message, &public_key, 64, 33);
    ...
    let ran = RefCell::new(false);
    with_vm_variants(|vm_kind: VMKind| {
        // Derive a config with p256_verify disabled while leaving every other
        // field at current defaults.
        let mut cfg = test_vm_config(Some(vm_kind));
        cfg.p256_verify = false;
        ...
        let aborted = outcome
            .aborted
            .as_ref()
            .unwrap_or_else(|| panic!("expected abort when gated off under {:?}", vm_kind));
        // VMs report the missing import via LinkError (the exact message is
        // VM-specific, so we don't pin the wording — just the failure mode).
        assert!(
            matches!(aborted, FunctionCallError::LinkError { .. }),
            "expected LinkError when p256_verify is gated off under {:?}, got {:?}",
            vm_kind,
            aborted,
        );
