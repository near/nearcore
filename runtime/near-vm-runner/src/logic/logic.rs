...
impl<'a> VMLogic<'a> {
    ...
    /// and gas.
    ///
    /// The created promise will have a special data dependency within a certain protocol-defined
    /// number of blocks (as defined by the `yield_timeout_length_in_blocks` parameter.)
    ///
    /// A resumption token is written by this function into the register denoted by `register_id`.
    /// Call `promise_yield_resume` with the resumption token as an argument in order to satisfy
    /// the special data dependency.
    ///
    /// Only the current account is allowed to resolve this data dependency.
    ///
    /// # Errors
    ///
    ...
    ///
    /// The `data_id` pair of parameters must refer to a value generated by a
    /// call to the [`promise_yield_create`] made by the same account.
    ///
    /// # Errors
    ///
