...
/// Decodes the fixed known-answer vector as `(signature, message, public_key)`.
fn ml_dsa_test_vectors() -> (Vec<u8>, Vec<u8>, Vec<u8>) {
...
fn check_ml_dsa_verify(
    ...
    let result = logic.ml_dsa_verify(
        signature_len,
        signature_ptr,
        message_len,
        message_ptr,
...
#[test]
fn test_ml_dsa_verify_valid_signature() {
    let (signature, message, public_key) = ml_dsa_test_vectors();
    ...
    check_ml_dsa_verify(
        ...
        map! {
            ...
            ExtCosts::ml_dsa_verify_base: 1,
            ExtCosts::ml_dsa_verify_byte: message.len() as u64,
        },
    );
}

#[test]
fn test_ml_dsa_verify_tampered_message() {
    let (signature, message, public_key) = ml_dsa_test_vectors();
    ...
    check_ml_dsa_verify(
        ...
        map! {
            ...
            ExtCosts::read_memory_base: 3,
            ExtCosts::ml_dsa_verify_base: 1,
            ExtCosts::ml_dsa_verify_byte: bad_message.len() as u64,
        },
    );
}
...
fn test_ml_dsa_verify_tampered_signature() {
    let (mut signature, message, public_key) = ml_dsa_test_vectors();
    signature[100] ^= 0xff;
...
#[test]
fn test_ml_dsa_verify_wrong_public_key() {
    let (signature, message, public_key) = ml_dsa_test_vectors();
    let mut other_public_bytes = public_key;
    other_public_bytes[0] ^= 0x01;
...
#[test]
fn test_ml_dsa_verify_invalid_signature_length() {
    let (signature, message, public_key) = ml_dsa_test_vectors();
    let invalid_signature_len = (signature.len() - 1) as u64;
...
#[test]
fn test_ml_dsa_verify_empty_signature_length() {
    let (_signature, message, public_key) = ml_dsa_test_vectors();
...
#[test]
fn test_ml_dsa_verify_invalid_public_key_length() {
    // The public key is read (and the message charged) before the length check,
    // so the per-byte message cost is still incurred before aborting.
    let (signature, message, public_key) = ml_dsa_test_vectors();
...
#[test]
fn test_ml_dsa_verify_malformed_public_key() {
    // An all-zero, correctly-sized (1952-byte) public key is well-formed by
    // length but fails verification, returning 0 rather than aborting.
    let (signature, message, public_key) = ml_dsa_test_vectors();
    let bad_public_key = vec![0u8; public_key.len()];
...
#[test]
fn test_ml_dsa_verify_all_inputs_from_registers() {
    let (signature, message, public_key) = ml_dsa_test_vectors();
