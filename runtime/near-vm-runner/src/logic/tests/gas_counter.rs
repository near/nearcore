...
fn test_dont_burn_gas_when_exceeding_attached_gas_limit() {
    ...
    logic_builder.config.limit_config.max_gas_burnt = NearGas::from_gas(gas_limit * 2);
...
fn test_cant_burn_more_than_max_gas_burnt_gas() {
    ...
    let mut logic_builder = VMLogicBuilder::default();
    logic_builder.config.limit_config.max_gas_burnt = NearGas::from_gas(gas_limit);
...
fn test_hit_prepaid_gas_limit() {
    ...
    let mut logic_builder = VMLogicBuilder::default();
    logic_builder.config.limit_config.max_gas_burnt = NearGas::from_gas(gas_limit * 3);
    logic_builder.context.prepaid_gas = gas_limit;
    let mut logic = logic_builder.build();
...
fn check_action_gas_exceeds_limit(
    ...
    let fee = Fee {
        send_sir: NearGas::from_gas(gas_limit / num_action_paid + 1),
        send_not_sir: NearGas::from_gas(gas_limit / num_action_paid + 10),
        execution: NearGas::from_gas(1), // exec part is `used`, make it small
    };
    let mut logic_builder = VMLogicBuilder::default();
    logic_builder.config.limit_config.max_gas_burnt = NearGas::from_gas(gas_limit);
...
fn check_action_gas_exceeds_attached(
    ...
    let fee = Fee {
        send_sir: NearGas::from_gas(1),      // make burnt gas small
        send_not_sir: NearGas::from_gas(10), // make it easy to distinguish `sir` / `not_sir`
        execution: NearGas::from_gas(gas_attached / num_action_paid + 1),
    };
    let mut logic_builder = VMLogicBuilder::default();
    logic_builder.config.limit_config.max_gas_burnt = NearGas::from_gas(gas_limit);
    logic_builder.fees_config.action_fees[cost] = fee;
