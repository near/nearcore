...
fn action_receipt_creation(ctx: &mut EstimatorContext) -> GasCost {
    ...
    let cost = total_cost.safe_subtract(&(block_overhead_cost * 2), &PER_MILLE_TOLERANCE);
...
fn action_sir_receipt_creation(ctx: &mut EstimatorContext) -> GasCost {
    ...
    let block_overhead_cost = apply_block_cost(ctx);
    let cost = total_cost.safe_subtract(&block_overhead_cost, &PER_MILLE_TOLERANCE);
...
fn action_transfer(ctx: &mut EstimatorContext) -> GasCost {
    ...
    total_cost.safe_subtract(&base_cost, &PER_MILLE_TOLERANCE)
}
...
fn action_add_function_access_key_base(ctx: &mut EstimatorContext) -> GasCost {
    ...
    let cost = total_cost.safe_subtract(&base_cost, &PER_MILLE_TOLERANCE);
...
fn action_add_function_access_key_per_byte(ctx: &mut EstimatorContext) -> GasCost {
    ...
    total_cost.safe_subtract(&base_cost, &PER_MILLE_TOLERANCE) / bytes_per_transaction
}
...
fn contract_compile_base_per_byte_v2(ctx: &mut EstimatorContext) -> (GasCost, GasCost) {
    ...
    for (contract, _) in REAL_CONTRACTS_SAMPLE {
        ...
        let bytes_cost = cost.safe_subtract(&smallest_cost, &PER_MILLE_TOLERANCE)
            / (binary.len() as u64 - smallest_size);
        max_bytes_cost = std::cmp::max(bytes_cost, max_bytes_cost);
    }

    let base_cost = smallest_cost
        .safe_subtract(&(max_bytes_cost.clone() * smallest_size), &PER_MILLE_TOLERANCE);
...
fn pure_deploy_bytes(ctx: &mut EstimatorContext) -> GasCost {
    ...
    if cost_4mb < cost_empty {
        ...
        cost_4mb.safe_subtract(&cost_empty, &PER_MILLE_TOLERANCE)
            / (large_code_len - small_code_len) as u64
    }
}
...
fn data_receipt_creation_base(ctx: &mut EstimatorContext) -> GasCost {
    ...
    total_cost.safe_subtract(&base_cost, &PER_MILLE_TOLERANCE) / 1000
}
...
fn touching_trie_node_read(ctx: &mut EstimatorContext) -> GasCost {
    ...
    assert!(nodes_touched_delta as usize >= 2 * final_key_len - 10);
    let cost_delta = cost_long_key.safe_subtract(&cost_short_key, &PER_MILLE_TOLERANCE);
