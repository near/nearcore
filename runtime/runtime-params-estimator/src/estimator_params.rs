//! Some parameters are use within the estimator to transform measurements to
//! gas costs. These parameters have been estimated manually and are now
//! hard-coded for a more deterministic estimation of runtime parameters.
//! This module contains the hard-coded constants as well as the code to
//! manually re-estimate them.
...
// We use factor of 8 to approximately match the price of SHA256 operation
// between time-based and icount-based metric as measured on 3.2Ghz Core i5.
...
// RocksDB as storage solution. Measuring it for debugging purposes is still
// useful but a conversion from total read/written bytes to gas is always going
// to be inaccurate. Consequently, set both values to 0 such that they do not
// influence gas costs.
...
/// Measure the cost for running a sha256 Rust implementation (on an arbitrary
/// input).
///
/// This runs outside the WASM runtime and is intended to measure the overall
/// hardware capabilities of the test system. (The motivation is to stay as
/// close as possible to original estimations done with this code: <https://github.com/near/calibrator/blob/c6fbb170a905fbc630ebd84ebc97f7226ec87ead/src/main.rs#L8-L21>)
