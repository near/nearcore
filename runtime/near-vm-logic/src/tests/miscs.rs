...
fn test_valid_utf8() {
    ...
    l.logic.log_utf8(l.mem, len, string_bytes.as_ptr() as _).expect("Valid utf-8 string_bytes");
...
fn test_invalid_utf8() {
    ...
    let len = string_bytes.len() as u64;
    assert_eq!(
        l.logic.log_utf8(l.mem, len, string_bytes.as_ptr() as _),
...
fn test_valid_null_terminated_utf8() {
    ...
    let mut l = logic_builder.build(get_context(vec![], false));
    l.logic
        .log_utf8(l.mem, u64::MAX, string_bytes.as_ptr() as _)
...
fn test_log_max_limit() {
    ...
    assert_eq!(
        l.logic.log_utf8(l.mem, string_bytes.len() as _, string_bytes.as_ptr() as _),
...
fn test_log_total_length_limit() {
    ...
    for _ in 0..num_logs - 1 {
        l.logic
            .log_utf8(l.mem, string_bytes.len() as _, string_bytes.as_ptr() as _)
...
fn test_log_number_limit() {
    ...
    for _ in 0..max_number_logs {
        l.logic
            .log_utf8(l.mem, len, string_bytes.as_ptr() as _)
...
fn test_log_utf16_number_limit() {
    ...
    for _ in 0..max_number_logs {
        l.logic
            .log_utf16(l.mem, len, string_bytes.as_ptr() as _)
            .expect("Valid utf-16 string_bytes under the log number limit");
    }
    assert_eq!(
        l.logic.log_utf16(l.mem, len, string_bytes.as_ptr() as _),
        Err(HostError::NumberOfLogsExceeded { limit: max_number_logs }.into())
    );
...
fn test_log_total_length_limit_mixed() {
    ...
    for _ in 0..num_logs_each {
        l.logic
            .log_utf16(l.mem, utf16_bytes.len() as _, utf16_bytes.as_ptr() as _)
            .expect("total is still under the limit");

        l.logic
            .log_utf8(l.mem, utf8_bytes.len() as _, utf8_bytes.as_ptr() as _)
            .expect("total is still under the limit");
    }
    assert_eq!(
        l.logic.log_utf8(l.mem, final_bytes.len() as _, final_bytes.as_ptr() as _),
        Err(HostError::TotalLogLengthExceeded { length: limit + 1, limit }.into())
    );
...
fn test_log_utf8_max_limit_null_terminated() {
    ...
    string_bytes.push(0u8);
    assert_eq!(
        l.logic.log_utf8(l.mem, u64::MAX, string_bytes.as_ptr() as _),
        Err(HostError::TotalLogLengthExceeded { length: limit + 1, limit }.into())
    );
...
fn test_valid_log_utf16_max_log_len_not_even() {
    ...
    l.logic
        .log_utf16(l.mem, u64::MAX, utf16_bytes.as_ptr() as _)
        .expect("Valid utf-16 string_bytes");
    ...
    utf16_bytes.extend_from_slice(&[0, 0]);
    assert_eq!(
        l.logic.log_utf16(l.mem, u64::MAX, utf16_bytes.as_ptr() as _),
...
fn test_log_utf8_max_limit_null_terminated_fail() {
    ...
    let mut l = logic_builder.build(get_context(vec![], false));
    let res = l.logic.log_utf8(l.mem, u64::MAX, string_bytes.as_ptr() as _);
...
fn test_invalid_log_utf16() {
    ...
    let res = l.logic.log_utf16(l.mem, utf16_bytes.len() as _, utf16_bytes.as_ptr() as _);
    let len = utf16_bytes.len() as u64;
...
fn test_valid_log_utf16_null_terminated_fail() {
    ...
    utf16_bytes.push(0);
    utf16_bytes.push(0);
    let res = l.logic.log_utf16(l.mem, u64::MAX, utf16_bytes.as_ptr() as _);
    let len = utf16_bytes.len() as u64;
    assert_eq!(res, Err(HostError::BadUTF16.into()));
    assert_costs(map! {
        ExtCosts::base: 1,
...
fn test_sha256() {
    ...
    l.logic.sha256(l.mem, data.len() as _, data.as_ptr() as _, 0).unwrap();
    let res = &vec![0u8; 32];
    l.logic.read_register(l.mem, 0, res.as_ptr() as _).expect("OK");
    assert_eq!(
        res,
        &[
...
fn test_keccak256() {
    ...
    l.logic.keccak256(l.mem, data.len() as _, data.as_ptr() as _, 0).unwrap();
    let res = &vec![0u8; 32];
    l.logic.read_register(l.mem, 0, res.as_ptr() as _).expect("OK");
    assert_eq!(
        res.as_slice(),
...
fn test_keccak512() {
    ...
    l.logic.keccak512(l.mem, data.len() as _, data.as_ptr() as _, 0).unwrap();
    let res = &vec![0u8; 64];
    l.logic.read_register(l.mem, 0, res.as_ptr() as _).expect("OK");
    assert_eq!(
        res,
...
fn test_ripemd160() {
    ...
    let data = b"tesdsst";
    l.logic.ripemd160(l.mem, data.len() as _, data.as_ptr() as _, 0).unwrap();
    let res = &vec![0u8; 20];
    l.logic.read_register(l.mem, 0, res.as_ptr() as _).expect("OK");
    assert_eq!(
        res,
...
fn test_ecrecover() {
            ...
            .unwrap()
    {
        ...
        let b = l
            .logic
            .ecrecover(l.mem, 32, m.as_ptr() as _, 64, sig.as_ptr() as _, v as _, mc as _, 1)
            .unwrap();
        ...
        if let Some(res) = res {
            ...
            let result = [0u8; 64];
            l.logic.read_register(l.mem, 1, result.as_ptr() as _).unwrap();
...
fn test_hash256_register() {
    ...
    let mut l = logic_builder.build(get_context(vec![], false));
    let data = b"tesdsst";
    l.logic.wrapped_internal_write_register(1, data).unwrap();

    l.logic.sha256(l.mem, u64::MAX, 1, 0).unwrap();
    let res = &vec![0u8; 32];
...
fn test_key_length_limit() {
    ...
    l.logic
        .storage_has_key(l.mem, key.len() as _, key.as_ptr() as _)
    ...
    l.logic
        .storage_read(l.mem, key.len() as _, key.as_ptr() as _, 0)
        .expect("storage_read: key length is under the limit");
    l.logic
        .storage_remove(l.mem, key.len() as _, key.as_ptr() as _, 0)
        .expect("storage_remove: key length is under the limit");
    // Over the limit. Invalid calls.
    key.push(b'a');
    assert_eq!(
        l.logic.storage_has_key(l.mem, key.len() as _, key.as_ptr() as _),
        Err(HostError::KeyLengthExceeded { length: key.len() as _, limit }.into())
    );
    assert_eq!(
        l.logic.storage_write(
            l.mem,
            key.len() as _,
            key.as_ptr() as _,
            val.len() as _,
    ...
    assert_eq!(
        l.logic.storage_read(l.mem, key.len() as _, key.as_ptr() as _, 0),
        Err(HostError::KeyLengthExceeded { length: key.len() as _, limit }.into())
    );
    assert_eq!(
        l.logic.storage_remove(l.mem, key.len() as _, key.as_ptr() as _, 0),
        Err(HostError::KeyLengthExceeded { length: key.len() as _, limit }.into())
    );
}
...
fn test_num_promises() {
    ...
    for _ in 0..num_promises {
        l.logic
            .promise_batch_create(l.mem, account_id.len() as _, account_id.as_ptr() as _)
            .expect("Number of promises is under the limit");
    }
    assert_eq!(
        l.logic.promise_batch_create(l.mem, account_id.len() as _, account_id.as_ptr() as _),
...
fn test_num_joined_promises() {
    ...
    for num in 0..num_deps {
        ...
        l.logic
            .promise_and(l.mem, promises.as_ptr() as _, promises.len() as _)
            .expect("Number of joined promises is under the limit");
    }
    let promises = vec![promise_id; (num_deps + 1) as usize];
    assert_eq!(
        l.logic.promise_and(l.mem, promises.as_ptr() as _, promises.len() as _),
...
fn test_return_value_limit() {
    ...
    let mut l = logic_builder.build(get_context(vec![], false));
    l.logic
        .value_return(l.mem, val.len() as _, val.as_ptr() as _)
        .expect("Returned value length is under the limit");
    val.push(b'a');
    assert_eq!(
        l.logic.value_return(l.mem, val.len() as _, val.as_ptr() as _),
...
fn test_contract_size_limit() {
    ...
    code.push(b'a');
    assert_eq!(
        l.logic.promise_batch_action_deploy_contract(
            l.mem,
            promise_id,
            code.len() as u64,
            code.as_ptr() as _
