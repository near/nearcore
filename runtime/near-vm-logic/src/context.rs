...
pub struct VMContext {
    ...
    /// The public key that was used to sign the original transaction that led
    /// to this execution.
    pub signer_account_pk: PublicKey,
    /// If this execution is the result of cross-contract call or a callback
    /// then predecessor is the account that called it.
    /// If this execution is the result of direct execution of transaction then
    /// it is equal to `signer_account_id`.
    pub predecessor_account_id: AccountId,
    /// The input to the contract call.
    /// Encoded as base64 string to be able to pass input in borsh binary
    /// format.
    pub input: Vec<u8>,
    ...
    /// The balance attached to the given account. Excludes the
    ...
    /// If Some, it means that execution is made in a view mode and defines its
    /// configuration. View mode means that only read-only operations are
    /// allowed. See <https://nomicon.io/Proposals/0018-view-change-method.html> for more details.
    pub view_config: Option<ViewConfig>,
    /// How many `DataReceipt`'s should receive this execution result. This
    /// should be empty if this function call is a part of a batch and it is
    /// not the last action.
