...
fn test_valid_utf8() {
    ...
    assert_eq!(outcome.logs[0], format!("{}", String::from_utf8(string_bytes.clone()).unwrap()));
    assert_costs(map! {
        ExtCosts::base: 1,
        ExtCosts::log_base:  1,
...
fn test_valid_log_utf16() {
    ...
    let outcome = logic.outcome();
    assert_eq!(outcome.logs[0], format!("{}", string));
}
...
fn test_valid_log_utf16_null_terminated_fail() {
    ...
    assert_ne!(logic.outcome().logs[0], format!("{}", string));
}
