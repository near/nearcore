mod tests {
    ...
    use crate::utils::{
        accounts, create_context, encode_meta_call_function_args, setup, sign_eth_transaction,
        CHAIN_ID,
    };
    ...
    fn test_invalid_input() {
        ...
        let mut context =
            create_context(&mut fake_external, &vm_config, &fees_config, accounts(1), 10);
    ...
    fn test_invalid_view_args() {
        let args =
            vec![vec![1u8; 20], vec![2u8; 20], vec![0u8; 32], vec![1u8, 0u8, 0u8, 0u8], vec![1]]
                .concat();
        let (mut fake_external, vm_config, fees_config) = setup();
        let mut context =
    ...
    fn test_invalid_raw_call_args() {
        ...
        assert_eq!(
            context
                .raw_call_function(signed_transaction.rlp_bytes().to_vec())
                .unwrap_err()
                .to_string(),
