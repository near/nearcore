...
Near the end of each epoch, during chunk application, the runtime checks whether a shard should be split. The check is skipped for blocks that are not near the epoch boundary (using `is_next_block_last_in_epoch`) and when the resharding cooldown has not elapsed (`can_reshard`). The decision logic has the following priority:
...
- **`NightshadeRuntime::compute_proposed_split`** -- Called during `apply_transactions`. Gates on `DynamicResharding` feature flag, checks if the next block could be the last of the epoch (`is_next_block_last_in_epoch`), checks the resharding cooldown (`can_reshard`), then delegates to `check_dynamic_resharding`.
