...
use primitives::aggregate_signature::{BlsPublicKey, BlsSecretKey, get_bls_key_pair};
use primitives::hash::hash_struct;
use primitives::network::PeerInfo;
use primitives::signature::{get_key_pair, PublicKey, SecretKey};
...
fn run_node(authority: usize,
    ...
    let node_task = futures::lazy(move || {
        ...
        let payload: ChainPayload = ChainPayload {
            transactions: vec![],
            receipts: vec![],
        };
        ...
        authorities.drain(..).enumerate().for_each(|(authority, account_id)| { auth_map.insert(authority, account_id); });
        ...
        let stop_task = Delay::new(Instant::now() + Duration::from_secs(600))
            .then(|_| {
                control_tx.send(Control::Stop)
                    .map(|_| ())
