...
pub fn generate_test_chain_spec() -> ChainSpec {
    ...
    let public_keys: Vec<PublicKey> = vec![
            "DWsBaCSq9BK9CKnEynDmfXwoziFJ3UwUZCN2ye5FHZFa",
            "DWsBaCSq9BK9CKnEynDmfXwoziFJ3UwUZCN2ye5FHZFa",
            "DWsBaCSq9BK9CKnEynDmfXwoziFJ3UwUZCN2ye5FHZFa"]
        .iter().map(|x| PublicKey::from(x)).collect();
    ChainSpec {
        accounts: vec![
            ("alice".to_string(), public_keys[0].to_string(), 100),
            ("bob".to_string(), public_keys[1].to_string(), 100),
            ("system".to_string(), public_keys[2].to_string(), 0),
        ],
        initial_authorities: vec![("alice".to_string(), public_keys[0].to_string(), 100)],
        genesis_wasm,
...
impl Runtime {
    pub fn apply_all(
        ...
        loop {
            let tt = cur_transactions.clone();
            let mut apply_result = self.apply(&cur_apply_state, &[], cur_transactions);
            println!("Merkle root: {}, receipts: {:?}", apply_result.root, apply_result.new_receipts);
            if apply_result.new_receipts.is_empty() {
                return apply_result;
            }
            println!("Commit apply_all {:?}", tt);
