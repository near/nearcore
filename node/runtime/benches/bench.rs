...
use transaction::{TransactionBody, SendMoneyTransaction, SignedTransaction};
...
fn runtime_send_money(bench: &mut Bencher) {
    ...
    let (mut runtime, _, mut root) = get_runtime_and_state_db_viewer_from_chain_spec(&chain_spec);
    bench.iter(|| {
        for _ in 0..100 {
