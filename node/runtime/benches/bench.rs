...
fn runtime_send_money(bench: &mut Bencher) {
    ...
    bench.iter(|| {
        let (root, _) = user.send_money(root, "bob.near", 1);
    });
}
...
fn runtime_wasm_set_value(bench: &mut Bencher) {
    ...
    bench.iter(|| {
        let (root, _) = user.call_function(
            root, "test_contract", "setValue", "{\"value\": \"123\"}".as_bytes().to_vec()
        );
    });
}
...
fn runtime_wasm_benchmark(bench: &mut Bencher) {
    let (mut user, mut root) = setup_test_contract(include_bytes!("../../../tests/hello.wasm"));
    bench.iter(|| {
        let (root, _) = user.call_function(root, "test_contract", "benchmark", vec![]);
    });
}
