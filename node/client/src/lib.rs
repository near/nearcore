...
impl Client {
    ...
    pub fn try_produce_block(&self, block_index: BlockIndex, payload: ChainPayload) -> BlockProductionResult {
        ...
        let receipt_block = self.shard_client.get_receipt_block(last_shard_block_header.index(), last_shard_block_header.shard_id());
        ...
        let (mut shard_block, (transaction, authority_proposals, tx_results, new_receipts)) = self
            .shard_client
            .prepare_new_block(last_beacon_block_header.body.shard_block_hash, receipt_blocks, payload.transactions);
    ...
    pub fn try_import_blocks(
        ...
        let new_best_block = self.beacon_client.chain.best_block_header();
...
mod tests {
    ...
    fn make_coupled_blocks(prev_beacon_block: &SignedBeaconBlockHeader, prev_shard_block: &SignedShardBlockHeader, count: u32, signers: &Vec<Arc<InMemorySigner>>) -> Vec<(SignedBeaconBlock, SignedShardBlock)> {
    ...
    fn test_block_catchup() {
        ...
        let blocks = make_coupled_blocks(
            &client.beacon_client.chain.best_block_header(), &client.shard_client.chain.best_block_header(), 10, &signers);
        for i in (0..10).rev() {
