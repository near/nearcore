...
use beacon::types::{AuthorityProposal, BeaconBlock, BeaconBlockBody};
...
impl Client {
    pub fn new(chain_spec: &ChainSpec, storage: Arc<Storage>, signer: Arc<Signer>) -> Self {
        ...
        let genesis = BeaconBlock::new(0, CryptoHash::default(), genesis_root, vec![], vec![]);
    ...
    fn import_block(&self, block: BeaconBlock) -> bool {
        ...
        if self.beacon_chain.is_known(&parent_hash) && self.validate_signature(&block) {
            let (header, mut body) = block.deconstruct();
...
mod tests {
    ...
    fn test_duplicate_import() {
        ...
        let block0 = BeaconBlock::new(
            0, parent_hash, MerkleHash::default(), vec![], vec![]
        );
