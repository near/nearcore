...
fn slow_test_state_sync_no_parts_provided() {
    ...
    env.test_loop.run_until(
        |data| {
            ...
            //tracing::debug!(?client.sync_handler.sync_status, "checking sync status");
            //assert!(!matches!(client.sync_handler.sync_status, SyncStatus::BlockSync { .. }), "Node should not be in block sync");
            if is_state_sync {
