...
/// Throttle header sync on a node by limiting the number of headers in each
/// `BlockHeaders` response delivered to the node.
///
/// Registers a transport-level message filter that intercepts `BlockHeaders`
/// messages destined for the target node and truncates them to `max_headers`.
///
...
pub fn throttle_header_sync(
    ...
    let target_peer_id = node_data.peer_id.clone();
    shared_state.network_shared_state.register_message_filter(move |_from, to, msg| {
        if to == &target_peer_id {
            if let PeerMessage::BlockHeaders(headers) = msg {
                if headers.len() > max_headers {
                    let truncated = headers.iter().take(max_headers).cloned().collect();
                    return Some(PeerMessage::BlockHeaders(truncated));
                }
            }
        }
        Some(msg.clone())
