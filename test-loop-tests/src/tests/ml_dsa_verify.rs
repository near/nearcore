use crate::setup::builder::TestLoopBuilder;
use crate::utils::account::create_account_id;
use assert_matches::assert_matches;
use near_async::time::Duration;
use near_o11y::testonly::init_test_logger;
use near_parameters::RuntimeConfigStore;
use near_primitives::errors::{ActionError, ActionErrorKind, TxExecutionError};
use near_primitives::gas::Gas;
use near_primitives::types::Balance;
use near_primitives::version::{MIN_SUPPORTED_PROTOCOL_VERSION, PROTOCOL_VERSION, ProtocolVersion};
use near_primitives::views::FinalExecutionStatus;

const PUBKEY_HEX: &str = "";
const SIG_HEX: &str = "";
const MSG_HEX: &str = "0707070707070707070707070707070707070707070707070707070707070707";

fn ml_dsa_verify_enabled() -> bool {
    RuntimeConfigStore::new(None).get_config(PROTOCOL_VERSION).wasm_config.ml_dsa_verify_host_fn
}

/// Lowest supported protocol version at which the `ml_dsa_verify` host function
/// is enabled, or `None` if it is not enabled at any of them.
fn ml_dsa_verify_activation_version() -> Option<ProtocolVersion> {
    let store = RuntimeConfigStore::new(None);
    (MIN_SUPPORTED_PROTOCOL_VERSION..=PROTOCOL_VERSION)
        .find(|pv| store.get_config(*pv).wasm_config.ml_dsa_verify_host_fn)
}

/// Build a WASM contract that imports `env.ml_dsa_verify`, bakes the supplied
/// `signature`, `public_key` and `message` into linear memory at fixed offsets,
/// calls the host function, and returns the resulting `u64` via `value_return`.
///
/// Layout (within the single 64 KiB page):
///   signature:  offset 0,    3309 bytes
///   public_key: offset 4096, 1952 bytes
///   message:    offset 8192, `message.len()` bytes
///   result (u64): offset 65528 (last 8 bytes of the page)
fn ml_dsa_verify_wasm(signature: &[u8], public_key: &[u8], message: &[u8]) -> Vec<u8> {
    fn bytes_to_data(bytes: &[u8]) -> String {
        let mut s = String::new();
        for byte in bytes {
            s.push_str(&format!("\\{:02x}", byte));
        }
        s
    }

    let sig_data = bytes_to_data(signature);
    let pk_data = bytes_to_data(public_key);
    let msg_data = bytes_to_data(message);
    let sig_len = signature.len();
    let pk_len = public_key.len();
    let msg_len = message.len();

    let wat = format!(
        r#"(module
  (import "env" "ml_dsa_verify"
    (func $ml_dsa_verify (param i64 i64 i64 i64 i64 i64) (result i64)))
  (import "env" "value_return" (func $value_return (param i64 i64)))
  (memory (export "memory") 1)

  (data (i32.const 0) "{sig_data}")
  (data (i32.const 4096) "{pk_data}")
  (data (i32.const 8192) "{msg_data}")

  (func (export "main")
    (local $result i64)
    (local.set $result
      (call $ml_dsa_verify
        (i64.const {sig_len})
        (i64.const 0)
        (i64.const {msg_len})
        (i64.const 8192)
        (i64.const {pk_len})
        (i64.const 4096))
    )
    (i64.store (i32.const 65528) (local.get $result))
    (call $value_return (i64.const 8) (i64.const 65528))
  )
)"#,
    );
    near_test_contracts::wat_contract(&wat)
}

fn valid_triple() -> (Vec<u8>, Vec<u8>, Vec<u8>) {
    let signature = hex::decode(SIG_HEX).expect("bad sig hex");
    let public_key = hex::decode(PUBKEY_HEX).expect("bad pk hex");
    let message = hex::decode(MSG_HEX).expect("bad msg hex");
    (signature, public_key, message)
}

/// End-to-end: deploy a contract that calls `ml_dsa_verify` with a correctly
/// signed message and assert the host function returns 1.
#[test]
fn test_ml_dsa_verify_valid_returns_1() {
    init_test_logger();
    if !ml_dsa_verify_enabled() {
        tracing::info!(
            "skipping: ml_dsa_verify host fn not enabled at protocol version {PROTOCOL_VERSION}"
        );
        return;
    }

    let user = create_account_id("user");
    let mut env = TestLoopBuilder::new()
        .enable_rpc()
        .add_user_account(&user, Balance::from_near(100))
        .build();

    let (signature, public_key, message) = valid_triple();
    let wasm = ml_dsa_verify_wasm(&signature, &public_key, &message);

    let deploy_tx = env.rpc_node().tx_deploy_contract(&user, wasm);
    env.rpc_runner().run_tx(deploy_tx, Duration::seconds(5));

    let call_tx =
        env.rpc_node().tx_call(&user, &user, "main", vec![], Balance::ZERO, Gas::from_teragas(300));
    let outcome = env.rpc_runner().execute_tx(call_tx, Duration::seconds(5)).unwrap();
    match &outcome.status {
        FinalExecutionStatus::SuccessValue(bytes) => {
            assert_eq!(bytes.len(), 8, "expected 8-byte u64 return");
            let value = u64::from_le_bytes(bytes[..8].try_into().unwrap());
            assert_eq!(value, 1, "valid signature should return 1");
        }
        other => panic!("expected SuccessValue(1), got {:?}", other),
    }
}

/// End-to-end: same contract shape, but the message passed to `ml_dsa_verify`
/// is tampered after signing. The host function returns 0 (verification
/// failure, not an abort).
#[test]
fn test_ml_dsa_verify_invalid_returns_0() {
    init_test_logger();
    if !ml_dsa_verify_enabled() {
        tracing::info!(
            "skipping: ml_dsa_verify host fn not enabled at protocol version {PROTOCOL_VERSION}"
        );
        return;
    }

    let user = create_account_id("user");
    let mut env = TestLoopBuilder::new()
        .enable_rpc()
        .add_user_account(&user, Balance::from_near(100))
        .build();

    let (signature, public_key, message) = valid_triple();
    let mut tampered = message;
    tampered[0] ^= 0x01;
    let wasm = ml_dsa_verify_wasm(&signature, &public_key, &tampered);

    let deploy_tx = env.rpc_node().tx_deploy_contract(&user, wasm);
    env.rpc_runner().run_tx(deploy_tx, Duration::seconds(5));

    let call_tx =
        env.rpc_node().tx_call(&user, &user, "main", vec![], Balance::ZERO, Gas::from_teragas(300));
    let outcome = env.rpc_runner().execute_tx(call_tx, Duration::seconds(5)).unwrap();
    match &outcome.status {
        FinalExecutionStatus::SuccessValue(bytes) => {
            assert_eq!(bytes.len(), 8, "expected 8-byte u64 return");
            let value = u64::from_le_bytes(bytes[..8].try_into().unwrap());
            assert_eq!(value, 0, "tampered message should return 0");
        }
        other => panic!("expected SuccessValue(0), got {:?}", other),
    }
}

/// Before the protocol version that activates `ml_dsa_verify`, a contract that
/// imports `env.ml_dsa_verify` must fail at call time (the import can't be
/// linked against a runtime that doesn't expose it).
#[test]
fn test_ml_dsa_verify_pre_activation_call_fails() {
    init_test_logger();
    if !ml_dsa_verify_enabled() {
        tracing::info!(
            "skipping: ml_dsa_verify host fn not enabled at protocol version {PROTOCOL_VERSION}"
        );
        return;
    }

    let activation_pv = ml_dsa_verify_activation_version()
        .expect("host fn is enabled, so it must activate at some supported protocol version");
    let pre_activation_pv =
        activation_pv.checked_sub(1).filter(|pv| *pv >= MIN_SUPPORTED_PROTOCOL_VERSION).expect(
            "the last protocol version without ml_dsa_verify is no longer supported, so there is \
             nothing left to test here - remove this test",
        );
    let user = create_account_id("user");
    let mut env = TestLoopBuilder::new()
        .enable_rpc()
        .protocol_version(pre_activation_pv)
        .add_user_account(&user, Balance::from_near(100))
        .build();

    let (signature, public_key, message) = valid_triple();
    let wasm = ml_dsa_verify_wasm(&signature, &public_key, &message);

    let deploy_tx = env.rpc_node().tx_deploy_contract(&user, wasm);
    env.rpc_runner().run_tx(deploy_tx, Duration::seconds(5));

    let call_tx =
        env.rpc_node().tx_call(&user, &user, "main", vec![], Balance::ZERO, Gas::from_teragas(300));
    let outcome = env.rpc_runner().execute_tx(call_tx, Duration::seconds(5)).unwrap();
    // The exact error variant (LinkError vs CompilationError) is VM-specific and
    // protocol-dependent, so just assert that the function call failed.
    assert_matches!(
        &outcome.status,
        FinalExecutionStatus::Failure(TxExecutionError::ActionError(ActionError {
            kind: ActionErrorKind::FunctionCallError(_),
            ..
        }))
    );
}
