...
fn test_invalid_optimistic_block() {
    ...
    let chain =
        &env.test_loop.data.get(&env.node_datas[0].client_sender.actor_handle()).client.chain;
    assert!(&chain.check_optimistic_block(&make_invalid_ob(&env, OptimisticBlockAdvType::InvalidVrfValue)).is_err());
    assert!(&chain.check_optimistic_block(&make_invalid_ob(&env, OptimisticBlockAdvType::InvalidVrfProof)).is_err());
    assert!(&chain.check_optimistic_block(&make_invalid_ob(&env, OptimisticBlockAdvType::InvalidRandomValue)).is_err());
    assert!(&chain.check_optimistic_block(&make_invalid_ob(&env, OptimisticBlockAdvType::InvalidTimestamp(0))).is_err());
    assert!(&chain.check_optimistic_block(&make_invalid_ob(&env, OptimisticBlockAdvType::InvalidPrevBlockHash)).is_err());
    assert!(&chain.check_optimistic_block(&make_invalid_ob(&env, OptimisticBlockAdvType::InvalidSignature)).is_err());
    assert!(&chain.check_optimistic_block(&make_invalid_ob(&env, OptimisticBlockAdvType::Normal)).is_ok());
