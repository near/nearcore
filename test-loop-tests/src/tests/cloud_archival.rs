...
/// Check that cloud archival writer does not crash and `cloud_head` progresses, if cold store is enabled.
#[test]
...
/// Check that GC works fine if cold store is disabled.
#[test]
fn test_cloud_archival_gc_without_cold_store() {
