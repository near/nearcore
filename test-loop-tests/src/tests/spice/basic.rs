...
fn test_spice_chain() {
    ...
    // Skewed stakes keep the producer set stable across reward epochs: using
    // desired_roles and the default setup, each chunk producer misses the first
    // chunk after genesis, which is enough to cause more rewards for
    // validators-only accounts and cause them to become chunk producers in the next epoch.
