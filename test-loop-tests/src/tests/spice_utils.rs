...
impl TestLoopEnv {
    ...
    pub fn delay_endorsements_propagation(&mut self, delay_height: u64) {
        self.set_execution_delay(delay_height);
...
/// Thin free-function wrapper around [`TestLoopEnv::delay_endorsements_propagation`]
/// kept for the older `delay_endorsements_propagation(&mut env, N)` callsites.
/// New code should prefer the method form `env.delay_endorsements_propagation(N)`.
pub(super) fn delay_endorsements_propagation(env: &mut TestLoopEnv, delay_height: u64) {
    env.delay_endorsements_propagation(delay_height);
}
