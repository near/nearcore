...
fn test_cross_shard_token_transfer() {
    ...
    for tx in &txs {
        ...
        let process_tx_request =
            ProcessTxRequest { transaction: tx.clone(), is_forwarded: false, check_only: false };
        rpc_node.data().rpc_handler_sender.send(process_tx_request);
    }
    ...
    let delayed_receipt: Receipt = near_store::get(
        &trie,
    ...
    assert_eq!(
        ...
        ExecutionStatus::SuccessReceiptId(*delayed_receipt.receipt_id())
    );
