...
use near_primitives::views::{
    BlockHeaderView, BlockView, FinalExecutionStatus, QueryRequest, QueryResponse,
    QueryResponseKind,
};
...
pub struct ViewClientSender {
    ...
    pub tx_status_request:
        AsyncSender<TxStatusRequest, anyhow::Result<Option<near_primitives::views::TxStatusView>>>,
}
...
async fn wait_for_transaction_finalization(
    ...
    loop {
        ...
        if let Ok(Some(outcome)) = view_client_sender
            .tx_status_request
            .send_async(TxStatusRequest { tx_hash, signer_account_id: creator_id.clone() })
            .await?
        {
            if let Some(final_outcome) = outcome.execution_outcome {
                match final_outcome.into_outcome().status {
...
async fn ft_contract_account_create(
    ...
    tracing::info!(target: "transaction-generator",
        "creating ft contract account from wasm at {:?}", wasm_path
    );

    let wasm_code = std::fs::read(&wasm_path).expect("failed to read fungible token wasm code");

    let creator_id = creator.id.clone();
    let creator_signer = creator.as_signer();

    // create fixed contract account
    let ft_contract_account_id: AccountId =
        format!("ft_contract.{}", creator_id).parse().expect("invalid ft contract account");
    ...
    let initial_balance_for_contract: Balance = Balance::from_yoctonear(100);
    ...
    {
        Ok(true) => {
            tracing::info!(target: "transaction-generator", "create+deploy tx accepted for {}", ft_contract_account_id);
...
async fn ft_contract_account_init(
    ...
    tracing::info!(target: "transaction-generator",
        "initializing ft contract at {}", ft_contract_account_id
    );
    let creator_signer = creator.as_signer();

    let init_args = serde_json::json!({
        "owner_id": creator.id.to_string(),
        "total_supply": "1_000_000_000_000_000_000_000_000", // 1 million tokens with 18 decimals
        "metadata": {
            "spec": "ft-1.0.0",
    ...
    {
        Ok(true) => {
            tracing::info!(target: "transaction-generator", "ft init tx accepted for {}", ft_contract_account_id);
            Ok(())
        }
...
async fn ft_contract_register_accounts(
    ...
    for receiver_id in receiver_ids {
        ...
        let creator_signer = creator.as_signer().clone(); // .as_signer(); // Account::as_signer returns an owned Signer
        ...
        futs.spawn(async move {
            ...
            match wait_for_transaction_finalization(
                ...
                Ok(true) => {
                    tracing::info!(target: "transaction-generator", "storage_deposit tx accepted for {}", receiver_id);
                    Ok(())
                }
                Ok(false) => {
                    Err(anyhow::anyhow!("timeout waiting for storage_deposit tx to be finalized for {}", receiver_id))
                }
                Err(err) => {
    ...
    for receiver_id in receiver_ids {
        ...
        futs.spawn(async move {
            ...
            match wait_for_transaction_finalization(
                ...
                Ok(true) => {
                    tracing::info!(target: "transaction-generator", "ft_transfer tx accepted for {}", receiver_id);
                    Ok(())
                }
                Ok(false) => {
                    Err(anyhow::anyhow!("timeout waiting for ft_transfer tx to be finalized for {}", receiver_id))
                }
                Err(err) => {
...
impl FungibleTokenTxSender {
    pub async fn new(
        ...
        tracing::info!(target: "transaction-generator",
            "fungible token contract at {} is ready",
            ft_contract_account_id
        );
    ...
    pub async fn generate_ft_transfer_transaction(
            ...
            .await
        {
            ...
            Ok(_) => {
                tracing::debug!(target: "transaction-generator", request_rsp="invalid response");
                false
            }
            Err(err) => {
                tracing::debug!(target: "transaction-generator", request_err=format!("{err}"), "error");
                false
            }
        }
    }
