...
impl TxGenerator {
    ...
    pub fn start(self: &mut Self) -> anyhow::Result<()> {
        ...
        let stats = Arc::new(Stats {
                pool_accepted: 0.into(),
                pool_rejected: 0.into(),
            });
    ...
    fn start_transactions_loop(
        ...
        let rx_accounts = Self::prepare_accounts(&config.accounts_path, view_client_sender.clone())?;
        ...
        tokio::spawn(async move {
            ...
            for load in schedule {
                Self::run_load(
                    ...
                    load.clone(),
                    rx_block.clone(),
                    Arc::clone(&stats)).await;                                
            }
            panic::set_hook(Box::new(|_| {
                println!("transaction-generator completed running the schedule. stopping the neard...");
            }));
            panic!("the end");
        });
