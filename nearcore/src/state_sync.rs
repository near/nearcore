...
impl StateSyncDumper {
    ...
    pub fn start(self) -> anyhow::Result<Arc<StateSyncDumpHandle>> {
        let dump_config = if let Some(dump_config) = self.client_config.state_sync.dump.clone() {
            ...
            tracing::debug!("not spawning the state sync dump loop");
            return Ok(Arc::new(StateSyncDumpHandle::new()));
        };
        tracing::info!("spawning the state sync dump loop");
        ...
        if let Some(shards) = dump_config.restart_dump_for_shards.as_ref() {
            for shard_id in shards {
                ...
                tracing::debug!(%shard_id, "dropped existing progress");
            }
        }
...
impl StateSyncDumpHandle {
    ...
    pub fn stop(&self) {
        tracing::warn!("stopping state dumper");
...
async fn get_missing_part_ids_for_epoch(
    ...
    if !file_names.is_empty() {
        ...
        tracing::debug!(?num_missing, ?directory_path, "some parts have already been dumped");
        Ok(missing_nums)
    } else {
        tracing::debug!(?total_parts, ?directory_path, "no part has been dumped");
    ...
    async fn set_missing_parts(&self, external: &StateSyncConnection, chain_id: &str) {
        for (shard_id, s) in &self.dump_state {
            ...
            .await
            {
                ...
                Err(error) => {
                    tracing::error!(?error, %shard_id, "failed to list stored state parts");
                }
            }
        }
    }
    ...
    async fn cancel(&mut self) {
        ...
        while !self.dump_state.is_empty() {
            ...
            if let Err(error) = result {
                tracing::error!(epoch_id = ?&self.epoch_id, %shard_id, ?error, "shard dump failed after cancellation");
            }
        }
    }
}
        ...
        loop {
            ...
            match self.external.put_file(file_type.clone(), &bytes, self.shard_id, &location).await
            {
                Ok(()) => {
                    ...
                    tracing::debug!(shard_id = %self.shard_id, epoch_height=%self.epoch_height, epoch_id=?&self.epoch_id, ?part_id, "uploaded state part");
                    return Ok(());
                }
                Err(error) => {
                    tracing::warn!(
...
impl HeaderUploader {
    ...
    async fn check_stored_headers(self: Arc<Self>, dump: &mut DumpState) {
        ...
        tokio_stream::iter(shards)
            ...
            .for_each(|shard_id| {
                tracing::info!(
                    %shard_id, epoch_height = %dump.epoch_height,
    ...
    async fn upload_header(self: Arc<Self>, shard_id: ShardId, header: Option<Vec<u8>>) {
        ...
        loop {
            match self.external.put_file(file_type.clone(), &header, shard_id, &location).await {
                Ok(_) => {
                    tracing::info!(
                        %shard_id, epoch_height = %self.epoch_height,
                ...
                Err(err) => {
                    tracing::warn!(
                        %shard_id, epoch_height = %self.epoch_height, ?err,
    ...
    async fn header_stored(self: Arc<Self>, shard_id: ShardId) -> bool {
            ...
            .await
        {
            Ok(stored) => stored,
            Err(err) => {
                tracing::error!(?err, %shard_id, "failed to determine header presence in external storage");
                false
            }
        }
    }
...
impl StateDumper {
    ...
    fn get_dump_state(&self, sync_header: &BlockHeader) -> anyhow::Result<NewDump> {
        ...
        for shard_id in shard_ids {
            if !self.shard_tracker.cares_about_shard(sync_header.prev_hash(), shard_id) {
                tracing::debug!(
                    epoch_height = %epoch_info.epoch_height(), epoch_id = ?sync_header.epoch_id(), %shard_id,
        ...
        if dump_state.is_empty() {
            tracing::warn!(
                epoch_height = %epoch_info.epoch_height(), epoch_id = ?sync_header.epoch_id(),
    ...
    async fn check_parts_upload(&mut self) -> anyhow::Result<()> {
        ...
        match result {
            Ok(()) => {
                tracing::info!(epoch_id = ?&dump.epoch_id, %shard_id, "shard dump finished");
            }
            Err(error) => {
                tracing::error!(epoch_id = ?&dump.epoch_id, %shard_id, ?error, "shard dump failed");
            }
        }
...
async fn state_sync_dump(
    ...
    tracing::info!("running state sync dump loop");
    ...
    if let CurrentDump::InProgress(mut dump) = dumper.current_dump {
        tracing::debug!("awaiting upload task cancellation");
        dump.cancel().await;
    }
    tracing::debug!("stopped state dump thread");
    Ok(())
}
...
async fn do_state_sync_dump(
    ...
    .await
    {
        tracing::error!(?error, "state dumper failed");
    }
}
