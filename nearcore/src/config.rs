...
pub fn init_configs(
    ...
    match chain_id.as_ref() {
        near_primitives::chains::MAINNET => {
            ...
            info!(target: "near", path = %dir.display(), "generated mainnet genesis file");
        }
        near_primitives::chains::TESTNET => {
            ...
            info!(
                ...
                %chain_id,
                path = %dir.display(),
                "generated network node key and genesis file"
            );
        }
        _ => {
            ...
            info!(
                target: "near",
                path = %dir.display(),
                "generated node key, validator key, genesis file"
            );
        }
    }
...
pub fn download_genesis(url: &str, path: &Path) -> Result<(), FileDownloadError> {
    info!(target: "near", %url, "downloading genesis file");
    let result = run_download_file(url, path);
    if result.is_ok() {
        info!(target: "near", path = %path.display(), "saved genesis file");
    }
    result
}

pub fn download_records(url: &str, path: &Path) -> Result<(), FileDownloadError> {
    info!(target: "near", %url, "downloading records file");
    let result = run_download_file(url, path);
    if result.is_ok() {
        info!(target: "near", path = %path.display(), "saved records file");
    }
    result
}

pub fn download_config(url: &str, path: &Path) -> Result<(), FileDownloadError> {
    info!(target: "near", %url, "downloading config file");
    let result = run_download_file(url, path);
    if result.is_ok() {
        info!(target: "near", path = %path.display(), "saved config file");
    }
    result
}
