    ...
    fn query_impl(&self, store: Store, query: EntityQuery) -> anyhow::Result<EntityDataValue> {
        match query {
            ...
            EntityQuery::ChunkByHash { chunk_hash } => {
                ...
                let author = self
                    .epoch_manager
                    .get_chunk_producer_info_from_prev_block(chunk.prev_block(), chunk.shard_id())?
                    .take_account_id();
...
impl BlockMiscData {
    pub fn from_store(store: &Store) -> anyhow::Result<BlockMiscData> {
        Ok(BlockMiscData {
            ...
            // TODO(cloud_archival) Add cloud head keys (CLOUD_BLOCK_HEAD_KEY, CLOUD_SHARD_HEAD_PREFIX, CLOUD_PREV_EPOCH_END_KEY)
        })
    }
}
