...
for shard_id in {0..3}; do
  objects_shard=$(aws s3 ls s3://state-parts-xiangyi/multi_node_tmp_1/chain_id=testnet/epoch_height=1925/epoch_id=6LUGTybvygryAbiixHCwbpkKWT2bGpQNbVnPNbDoqwDG/shard_id=$shard_id/ | sort -k1,2)
