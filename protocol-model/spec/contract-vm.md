...
| `runtime/near-vm-runner/src/prepare.rs:21` | `prepare_contract` | Validate and instrument with prepare_v3. |
...
- The exact per-check ordering inside `prepare_v3` (which limit is reported first when several are violated) is only spot-checked via the tests in `prepare.rs`; the module-internal `instrument_v3`/`prepare_v3` files were not read line-by-line here.
- `NearVm` remains in the serialized `VMKind` enum for compatibility, but its runtime backend is no longer available.
