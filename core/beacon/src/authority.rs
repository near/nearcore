...
impl Authority {
    ...
    fn epoch_to_slots(&self, epoch: Epoch) -> impl Iterator<Item = Slot> {
        ...
        epoch * self.authority_config.epoch_length + 1
            ..(epoch + 1) * self.authority_config.epoch_length + 1
    }
    ...
    pub fn new(
        ...
        let (accepted_authorities, threshold) = result.compute_threshold_accepted(
            &CryptoHash::default(),
            result.authority_config.initial_proposals.iter().cloned(),
            HashMap::new(),
        );
        let mut slot = 0;
        for epoch in 0..=1 {
        ...
        // Catch up with the blockchain. Note, the last block might change while we are
        // iterating.
        // TODO: Take care of the current beacon block being changed while we are iterating.
        let mut index = 1;
    ...
    /// Computes accepted authorities and the threshold from the given adjusted proposals.
    fn compute_threshold_accepted(
        &self,
        seed: &CryptoHash,
        proposals: impl Iterator<Item = AuthorityProposal>,
        adjustments: HashMap<AccountId, u64>,
    ) -> (Vec<Vec<AuthorityProposal>>, u64) {
        // Adjust the stakes by applying the adjustments.
        let (filtered_proposals, filtered_stakes): (Vec<_>, Vec<_>) = proposals
            .filter_map(move |prop| {
                let amount = prop.amount;
                match adjustments.get(&prop.account_id) {
                    None => Some((prop, amount)),
                    Some(&adj) => {
                        let adjusted = amount + adj;
                        if adjusted > 0 {
                            Some((prop, adjusted))
                        } else {
                            None
        ...
        let threshold = find_threshold(&filtered_stakes, num_seats)
            .expect("Threshold is not found for given proposals");
        // Duplicate proposals per each seat that they get.
        let mut dup_proposals: Vec<_> = filtered_proposals
            .iter()
            .zip(filtered_stakes.iter())
            .flat_map(|(prop, stake)| {
                iter::repeat(prop).cloned().take((*stake / threshold) as usize)
            })
            .collect();
        assert!(
    ...
    fn compute_accepted_authorities(&mut self, epoch: Epoch) {
        ...
        // blocks in epoch-4.
        if let Some(threshold) = { self.thresholds.get(&(epoch - 2)).map(|t| *t) } {
            // First, compute the adjustments.
            let mut adjustments = HashMap::new();
            for s in self.epoch_to_slots(epoch - 2) {
                // We can unwrap because the first check has passed.
                let accepted = self.accepted_authorities.get(&s).unwrap().iter();
                let participation = self.participation.get(&s).unwrap().iter();
                for (acc, participated) in accepted.zip(participation) {
                    let entry = adjustments.entry(acc.account_id).or_insert(0);
                    if *participated {
                        *entry += threshold;
                    } else {
                        *entry -= threshold;
            ...
            // Second, use the proposals and the adjustments.
            // TODO(#308): Use proper seed.
            let (mut accepted_authorities, new_threshold) = {
                let proposals = self
                    .epoch_to_slots(epoch - 2)
                    .flat_map(|s| self.proposals.get(&s).unwrap().iter().cloned());
                self.compute_threshold_accepted(&CryptoHash::default(), proposals, adjustments)
            };
    ...
    pub fn get_authorities(&self, slot: Slot) -> Result<Vec<AuthorityProposal>, String> {
        if slot == 0 {
