...
use crate::{
    get_account, has_received_data, set, set_access_key, set_account, set_postponed_receipt,
    set_promise_yield_receipt, set_received_data, ShardTries, TrieUpdate,
};
...
// The StateRecord type used not to include the index of delayed receipts, so
// here we keep track of the indices we're writing for delayed receipts based on whether
// the records were generated by a binary that writes indices. If not, we just start from
// index 0 and increment as we go, although this may lead to a different order than they
// would have appeared in in the source chain.
enum DelayedReceipts {
    WithIndices(std::ops::Range<u64>),
    NoIndices(u64),
    None,
}

impl DelayedReceipts {
    fn next(&mut self, index: Option<u64>) -> TrieKey {
        let index = match self {
            Self::WithIndices(bounds) => {
                let Some(index) = index else {
                    panic!("serialized state records in genesis contain some delayed receipts with indices and some without");
                };
                bounds.start = std::cmp::min(bounds.start, index);
                bounds.end = std::cmp::max(bounds.end, index);
                index
            }
            Self::NoIndices(num_receipts) => {
                if index.is_some() {
                    panic!("serialized state records in genesis contain some delayed receipts with indices and some without");
                };
                let index = *num_receipts;
                *num_receipts += 1;
                index
            }
            Self::None => match index {
                Some(index) => {
                    *self = Self::WithIndices(std::ops::Range { start: index, end: index });
                    index
                }
                None => {
                    *self = Self::NoIndices(1);
                    0
                }
            },
        };
    ...
    fn write_indices(self, storage: &mut AutoFlushingTrieUpdate) {
        let indices = match self {
            Self::WithIndices(bounds) => DelayedReceiptIndices {
                first_index: bounds.start,
                next_available_index: bounds.end + 1,
            },
            Self::NoIndices(num_receipts) => {
                DelayedReceiptIndices { first_index: 0, next_available_index: num_receipts }
            }
            Self::None => return,
        };
...
impl GenesisStateApplier {
    fn apply_batch(
        ...
        delayed_receipts_indices: &mut DelayedReceipts,
        shard_uid: ShardUId,
        ...
        genesis.for_each_record(|record: &StateRecord| {
            ...
            match record {
                ...
                StateRecord::DelayedReceipt(receipt) => storage.modify(|state_update| {
                    let key = delayed_receipts_indices.next(receipt.index);
                    set(state_update, key, &receipt.receipt);
                }),
            }
        });
    ...
    pub fn apply(
        ...
        let mut delayed_receipts_indices = DelayedReceipts::None;
        let mut storage =
        ...
        delayed_receipts_indices.write_indices(&mut storage);
