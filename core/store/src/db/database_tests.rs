...
fn test_stress_trie_and_storage() {
    ...
    for i in start_idx..batches {
        ...
        if (i + 1) % log_each == 0 {
            eprintln!(
                "Processed batch {:04}: {:08} accounts, new root: {}, (memory: {:?}, disk: {:?})",
                i + 1,
    ...
    for i in 0..batches {
        ...
        if (i + 1) % log_each == 0 {
            eprintln!(
                "Processed batch (updates) {:04}:  new root: {}, (memory: {:?}, disk: {:?})",
                i + 1,
                new_root,
                measured_times.memory.as_millis() / (log_each as u128),
                measured_times.disk.as_millis() / (log_each as u128)
...
impl MeasuredTimes {
    fn new() -> Self {
        Self { memory: std::time::Duration::ZERO, disk: std::time::Duration::ZERO }
    }
