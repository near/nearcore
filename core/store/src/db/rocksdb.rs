...
impl Database for RocksDB {
    ...
    fn write(&self, transaction: DBTransaction) -> io::Result<()> {
        ...
        // Collect statistics about columns to use if the batch creation takes too long
        let mut col_stats: std::collections::HashMap<DBCol, (usize, usize)> =
            std::collections::HashMap::new();
        let total_ops = transaction.ops.len();
        for op in &transaction.ops {
            let (col, key_size, value_size) = match op {
                DBOp::Set { col, key, value } => (*col, key.len(), value.len()),
                DBOp::Insert { col, key, value } => (*col, key.len(), value.len()),
                DBOp::UpdateRefcount { col, key, value } => (*col, key.len(), value.len()),
                DBOp::Delete { col, key } => (*col, key.len(), 0),
                DBOp::DeleteAll { col } => (*col, 0, 0),
                DBOp::DeleteRange { col, from, to } => (*col, from.len() + to.len(), 0),
            };

            let entry = col_stats.entry(col).or_insert((0, 0));
            entry.0 += 1; // increment count
            entry.1 += key_size + value_size; // add key+value size
        }

        let batch = self.build_write_batch(transaction)?;
        let elapsed = write_batch_start.elapsed();
        if elapsed.as_secs_f32() > 0.15 {
            // Find top 10 columns by number of entries
            let mut col_stats_vec: Vec<_> = col_stats.into_iter().collect();
            col_stats_vec.sort_by(|a, b| b.1.0.cmp(&a.1.0)); // Sort by count (descending)
            let top_cols = col_stats_vec
                .into_iter()
                .take(10)
                .map(|(col, (count, size))| format!("{:?}: {} entries, {} bytes", col, count, size))
                .collect::<Vec<_>>()
                .join("\n");
