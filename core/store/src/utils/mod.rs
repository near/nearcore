...
pub fn set_gas_key_nonce(
    ...
    set(state_update, TrieKey::GasKey { account_id, public_key, index }, &nonce);
}
...
pub fn remove_gas_key_nonce(
    ...
    state_update.remove(TrieKey::GasKey { account_id, public_key, index });
}
...
pub fn get_gas_key_nonce(
    ...
    get(
        trie,
        &TrieKey::GasKey { account_id: account_id.clone(), public_key: public_key.clone(), index },
    )
}
...
pub fn remove_account(
    ...
    for (public_key, nonce_index) in public_keys {
        if let Some(nonce_index) = nonce_index {
            state_update.remove(TrieKey::GasKey {
