...
impl<NodePtr: Debug + Copy> TrieDescentStage<NodePtr> {
    ...
    fn node_memory_usage<Value, Getter>(&self, get_node: Getter) -> u64
    where
        Getter: Fn(NodePtr) -> GenericTrieNodeWithSize<NodePtr, Value>,
    {
        match self {
            ...
            // Extension nodes also use some memory, but we don't account for this, because
            // it's not significant enough in real cases.
            _ => 0,
        }
    }
    ...
    {
        if nibbles.is_empty() {
            return self.subtree_memory_usage();
        }
        match self {
    ...
    Storage: GenericTrieInternalStorage<NodePtr, Value>,
{
    ...
    fn current_nodes_mem_usage(&self) -> u64 {
        let get_node =
            |ptr| self.trie_storage.get_node_with_size(ptr, AccessOptions::DEFAULT).unwrap();
        self.subtree_stages.iter().map(|stage| stage.node_memory_usage(get_node)).sum()
    }
    ...
    pub fn find_mem_usage_split(mut self) -> TrieSplit {
        ...
        let mut best_split = TrieSplit::new(SmallVec::from_elem(0, 4), 0, u64::MAX);
        ...
        // at least 2 bytes long, include an even number of nibbles, and not end with a special char.
        while self.current_split_account().is_none() {
            println!("Forcing next step. {:?} {:?}", self.nibbles, self.subtree_stages[0]);
        ...
        debug_assert!(best_split.mem_diff() < u64::MAX);
        best_split
    }
    ...
    fn next_step(&self) -> Option<(u8, u64, u64)> {
        ...
        let attached_data = self.attached_data_mem_usage();
        let threshold = (self.total_memory() / 2)
            .checked_sub(self.left_memory)?
            .checked_sub(current_nodes_memory)?
            .checked_sub(attached_data)?;
        ...
        self.subtree_stages[0].can_descend_nibble(middle_child).then_some((
            middle_child,
            child_mem_usage,
            left_mem_usage,
        ))
    ...
    fn current_split(&self) -> TrieSplit {
        debug_assert!(self.current_split_account().is_some());
        ...
        if left1.abs_diff(right1) <= left2.abs_diff(right2) {
            TrieSplit::new(self.nibbles.clone(), left1, right1)
        } else {
            let mut nibbles = self.nibbles.clone();
            nibbles.extend(bytes_to_nibbles("-0".as_bytes()));
            TrieSplit::new(nibbles, left2, right2)
        }
    }
    ...
    fn descend_step(&mut self, nibble: u8, child_mem_usage: u64, left_mem_usage: u64) {
        let parent_mem_usage = self.current_nodes_mem_usage() + self.attached_data_mem_usage();
        ...
        let get_node =
            |ptr| self.trie_storage.get_node_with_size(ptr, AccessOptions::DEFAULT).unwrap();
        // println!("Descending {nibble} accounts stage: {:?}", self.subtree_stages[0]);
...
mod tests {
    ...
    mod trie_descent {
        ...
        fn big_middle_account_with_children() {
            ...
            subtrees[0] = {
                let mut children = [None; NUM_CHILDREN];
                children[0] = Some(new_leaf(&mut arena, &[], &[1u8; 1000]));
                children[2] = Some(new_leaf(&mut arena, &[], &[1u8; 3000]));
                ...
                children[1] = Some(new_branch(&mut arena, Some(&[1u8; 10000]), children2));
                let branch = new_branch(&mut arena, None, children);
                new_extension(&mut arena, &[0, 0, 0], branch)
            };
            let split = init(&mut arena, subtrees).find_mem_usage_split();
            ...
            // simple optimization implemented such cases, which appends '-0' to the boundary
            // account ID. That's where the two extra bytes in the split path come from.
            assert_eq!(split.split_path_bytes(), [0x00, 0x01, 0x2d, 0x30])
        }
    ...
    mod find_trie_split {
        ...
        fn big_random_trie() {
            ...
            for seed in 0..1000 {
                big_random_trie_inner(seed).unwrap();
            }
        ...
        fn big_random_trie_inner(seed: u64) -> anyhow::Result<()> {
            ...
            if let Some(right_account) = right_account {
                ...
                assert!(
                    right_split.mem_diff() >= trie_split.mem_diff() // || right_split.left_memory > right_split.right_memory
                );
        ...
        fn random_trie_big_account() {
            for seed in 0..1000 {
                random_trie_big_account_inner(seed).unwrap();
            }
        }
        ...
        fn random_trie_big_account_inner(seed: u64) -> anyhow::Result<()> {
            ...
            println!(
                "big account: {account_id} boundary: {boundary_account} split: {trie_split:?}"
            );
            if trie_split.left_memory > trie_split.right_memory {
                assert!(account_id < boundary_account);
                assert!(
                    trie_split.left_memory - (big_account_bytes as u64) < trie_split.right_memory
                );
            } else {
                assert!(account_id >= boundary_account);
                assert!(
                    trie_split.right_memory - (big_account_bytes as u64) < trie_split.left_memory
                );
            }
