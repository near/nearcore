...
use near_primitives::utils::get_block_shard_uid;
use std::rc::Rc;
use std::sync::Arc;
...
impl ShardTries {
    ...
    pub fn new(store: Arc<Store>, shards: &Vec<ShardUId>) -> Self {
        ...
        for op in &transaction.ops {
            match op {
                DBOp::UpdateRefcount { col, ref key, ref value } if *col == DBCol::ColState => {
                    ...
                    shards.get_mut(&shard_uid).unwrap().push((hash, Some(value.clone())));
                    // [shard_uid as usize].push((hash, Some(value.clone())));
                }
...
impl WrappedTrieChanges {
    ...
    pub fn wrapped_into(
        ...
        store_update.set_ser(
            ...
            &get_block_shard_uid(&self.block_hash, self.shard_uid),
            &self.trie_changes,
        )?;
        Ok(())
    }
