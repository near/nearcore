...
mod tests {
    ...
    impl Trie {
        ...
        pub fn combine_state_parts_naive(
            ...
            let mut insertions = insertions
                .into_iter()
                .map(|(k, (v, rc))| TrieRefcountChange { trie_node_or_value_hash: k, trie_node_or_value: v, rc })
                .collect::<Vec<_>>();
            insertions.sort();
            Ok(TrieChanges {
    ...
    fn merge_trie_changes(changes: Vec<TrieChanges>) -> TrieChanges {
        ...
        for changes_set in changes {
            ...
            for TrieRefcountChange { trie_node_or_value_hash, trie_node_or_value, rc } in changes_set.insertions {
                map.entry(trie_node_or_value_hash).or_insert_with(|| (trie_node_or_value, 0)).1 += rc as i32;
            }
            for TrieRefcountChange { trie_node_or_value_hash, trie_node_or_value, rc } in changes_set.deletions {
                map.entry(trie_node_or_value_hash).or_insert_with(|| (trie_node_or_value, 0)).1 -= rc as i32;
            }
        }
        let (insertions, deletions) = Trie::convert_to_insertions_and_deletions(map);
