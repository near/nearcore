...
use near_store::shard_utilization::{
    find_split_account, total_utilization, update_shard_utilization,
};
...
impl EpochStats {
    fn new(shard_layout: ShardLayout) -> Self {
        ...
        Self { shard_layout, shard_tries, state_roots, total_exec_duration: Default::default() }
    }
    ...
    fn process_block(&mut self, msg: &StreamerMessage) -> anyhow::Result<()> {
        for shard in &msg.shards {
            ...
            let exec_duration = shard_update.finalize()?;
            self.total_exec_duration += exec_duration;
        }
        Ok(())
    }

    fn total_trie_size(&self) -> anyhow::Result<u64> {
        self.shard_layout
            .shard_uids()
            .try_fold(0u64, |acc, uid| Ok(acc + self.shard_trie_size(uid)?))
    }
    ...
    fn print_stats(&self) -> anyhow::Result<()> {
        for shard_uid in self.shard_layout.shard_uids() {
            ...
            let split_account =
                find_split_account(&trie)?.map(|id| id.to_string()).unwrap_or_default();
            info!(
                "Shard {shard_uid}: total_utilization = {total_utilization:?}, split_account = '{split_account}'"
            );
        }
        Ok(())
    }
...
async fn main() -> anyhow::Result<()> {
    ...
    while let Some(msg) = receiver.recv().await {
        ...
        if num_blocks % 100 == 0 {
            let total_exec_time = epoch_stats.total_exec_duration;
            let millis_per_block = total_exec_time.as_millis() as u64 / num_blocks;
            let trie_size = epoch_stats.total_trie_size()?;
            info!(
                "{num_blocks} blocks processed. \
                trie size: {trie_size} bytes \
                total execution time: {total_exec_time:?} ({millis_per_block} ms / block)"
            );
