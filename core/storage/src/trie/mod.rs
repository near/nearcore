...
mod tests {
    ...
    #[test]
    fn test_trie_remove_non_exist_key() {
        let trie = create_trie();
        let empty_root = Trie::empty_root();
        let mut changes = vec![(b"123".to_vec(), None)];
        trie.update(&empty_root, changes.drain(..));
    }
    ...
    fn test_failure() {
        let trie = create_trie();
        let empty_root = Trie::empty_root();
        let mut initial: Vec<(Vec<u8>, Option<Vec<u8>>)> = (vec![
            vec![0, 115, 116, 117, 100, 105, 111, 45, 104, 54, 97, 111, 97, 116, 55, 106, 99],
            vec![
                0, 115, 116, 117, 100, 105, 111, 45, 104, 54, 97, 111, 97, 116, 55, 106, 99, 44,
                100, 58, 58, 49,
            ],
            vec![
                0, 115, 116, 117, 100, 105, 111, 45, 104, 54, 97, 111, 97, 116, 55, 106, 99, 44,
        ...
        let mut changes = vec![
            (
                vec![
                    0, 115, 116, 117, 100, 105, 111, 45, 104, 54, 97, 111, 97, 116, 55, 106, 99,
                    44, 109, 58, 108, 101, 110,
                ],
                Some(vec![123]),
            ),
            (
