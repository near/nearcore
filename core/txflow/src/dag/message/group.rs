...
impl<'a, P: 'a + Payload> Group<'a, P> {
    pub fn new() -> Self {
        Group { messages_by_owner: HashMap::new(), v: HashSet::new() }
    }
    ...
    pub fn union_update(&mut self, other: &Self) {
        for (owner_uid, per_owner) in &other.messages_by_owner {
            self.messages_by_owner.entry(*owner_uid).or_insert_with(HashSet::new).extend(per_owner);
        }
    }
...
impl<'a, P: 'a + Payload> Clone for Group<'a, P> {
    fn clone(&self) -> Self {
        Group { messages_by_owner: self.messages_by_owner.clone(), v: HashSet::new() }
    }
}
...
impl<'a, P: 'a + Payload> GroupsPerEpoch<'a, P> {
    pub fn new() -> Self {
        GroupsPerEpoch { messages_by_epoch: HashMap::new() }
    }
    ...
    pub fn union_update(&mut self, other: &Self) {
        for (epoch, per_epoch) in &other.messages_by_epoch {
            self.messages_by_epoch.entry(*epoch).or_insert_with(Group::new).union_update(per_epoch);
        }
    }
    ...
    pub fn filter_by_owner(
        ...
        (&self.messages_by_epoch).iter().filter_map(move |(epoch, per_epoch)| {
