...
mod tests {
    ...
    // We created one root and received another one from some other witness.
//    #[test]
//    fn simple_two_roots() {
//        let owner_uid = 0;
//        let starting_epoch = 0;
...
//        let (out_gossip_tx, _out_gossip_rx) = mpsc::channel(1_024);
//        let selector = FakeWitnessSelector::new();
//        let mut task = TxFlowTask::<FakePayload, _>::new(
//            owner_uid, starting_epoch, sample_size, inc_gossip_rx,
//            inc_payload_rx, out_gossip_tx, selector);
//        task.init_dag();
//
//
//        // First create an initial message.
//        let a = task.dag_as_mut().create_root_message(FakePayload{}, vec![]);
//        // Feed another message from another participant
//        let arena = Arena::new();
//        let mut all = vec![];
//        let b;
//        simple_bare_messages!(arena, all [1, 0 => b;]);
//        task.process_gossip(Gossip {
//            sender_uid: 1,
//            receiver_uid: 0,
//            sender_sig: 0,
//            body: GossipBody::Unsolicited(b.clone()),
//        });
//
//        // Take new root.
//        let c = task.dag_as_mut().create_root_message(FakePayload{}, vec![]);
//        // Assert that the new root has the right subroots.
//        assert!(c.data.body.parents.contains(&a.data.hash));
//        assert!(c.data.body.parents.contains(&b.hash));
//    }
//
//    #[test]
//    fn simple_fetch() {
...
//        let (b, c, d);
//
//        simple_bare_messages!(arena, all [[1, 0 => b; 2, 0 => c;] => 1, 0 => d;]);
//        let (uids, missing_hashes) = task.process_gossip(Gossip {
//            sender_uid: 1,
//            receiver_uid: 0,
//            sender_sig: 0,
//            body: GossipBody::Unsolicited(d.clone()),
//        });
//        assert_eq!(uids.len(), 0);  // No replies, since we are missing hashes.
//        assert_eq!(missing_hashes, set!{b.hash, c.hash});
//        assert_eq!(task.future_replies, map!{d.hash => set!{1}});
//        assert_eq!(task.missing_messages, map!{c.hash => set!{d.hash}, b.hash => set!{d.hash}});
//
//         let (uids, missing_hashes) = task.process_gossip(Gossip {
//            sender_uid: 1,
//            receiver_uid: 0,
//            sender_sig: 0,
//            body: GossipBody::FetchReply(vec![
//                b.clone(),
//                c.clone(),
//            ]),
//        });
//        assert_eq!(uids, set!{1});  // Now that we have hashes, reply.
//        assert_eq!(missing_hashes.len(), 0);
//    }
}
