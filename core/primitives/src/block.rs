...
impl Block {
    ...
    pub fn produce(
        ...
        let mut time = prev.raw_timestamp() + 1;
        if let Some(now) = to_timestamp(timestamp_override.unwrap_or_else(StaticClock::utc)) {
            time = if now <= prev.raw_timestamp() { prev.raw_timestamp() + 1 } else { now };
        };
