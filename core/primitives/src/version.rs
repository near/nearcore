...
pub fn get_protocol_upgrade_schedule(chain_id: &str) -> ProtocolUpgradeVotingSchedule {
    ...
    let schedule = match chain_id {
        MAINNET => {
            // Voting starts Monday 01:00 UTC.
            let v1_protocol_version = 79;
            let v1_datetime =
                ProtocolUpgradeVotingSchedule::parse_datetime("2025-08-18 01:00:00").unwrap();
        ...
        _ => {
            ...
            let v1_datetime =
                ProtocolUpgradeVotingSchedule::parse_datetime("2025-08-28 15:00:00").unwrap();
            let schedule = vec![(v1_datetime, v1_protocol_version)];
            schedule
        }
    };
