...
/// Per-anchor chunk producer kickout state stored in `DBCol::ChunkProducers`.
///
/// The row keyed by `(anchor_block_hash, shard_id)` holds the kickout state as of
/// that block. A chunk anchored on that block (the block two hashes behind the
/// chunk, i.e. the chunk's prev_prev block) samples its producer from the chunk
/// epoch's `EpochInfo` at the chunk's signed `height_created`, excluding the
/// blacklisted accounts.
///
/// In the identity landing the blacklist is always empty, so producer selection is
/// byte-identical to `sample_chunk_producer`. A later PR fills the blacklist from
/// per-block chunk production stats.
///
/// Blacklist entries are `AccountId`s (not epoch-relative `ValidatorId`s) so the
/// state stays meaningful when the anchor and the chunk are in different epochs.
#[derive(
    BorshSerialize, BorshDeserialize, Clone, Debug, PartialEq, Eq, serde::Serialize, ProtocolSchema,
)]
pub enum ChunkProducerKickoutState {
    V1(ChunkProducerKickoutStateV1),
}
...
pub struct ChunkProducerKickoutStateV1 {
    /// Accounts excluded from chunk producer sampling at this anchor.
    pub blacklist: BTreeSet<AccountId>,
}

impl Default for ChunkProducerKickoutState {
    fn default() -> Self {
        Self::identity()
    }
}

impl ChunkProducerKickoutState {
    /// The empty-blacklist state: producer selection is identical to the static
    /// epoch sampler.
    pub fn identity() -> Self {
        Self::V1(ChunkProducerKickoutStateV1::default())
    }

    pub fn blacklist(&self) -> &BTreeSet<AccountId> {
        match self {
            Self::V1(v1) => &v1.blacklist,
        }
    }
    ...
    /// Resolve the blacklisted accounts to validator ids in `epoch_info`.
    /// Accounts that are not validators in that epoch are dropped: a producer
    /// blacklisted at the anchor cannot be sampled in an epoch where it is not
    /// in the validator set anyway.
    pub fn exclude_ids(&self, epoch_info: &EpochInfo) -> HashSet<ValidatorId> {
        self.blacklist()
            .iter()
            .filter_map(|account_id| epoch_info.get_validator_id(account_id).copied())
            .collect()
    }
}
...
impl EpochInfo {
    ...
    /// Sample a chunk producer for (shard_id, height), excluding the accounts
    /// blacklisted in `kickout_state`. With the identity state this is exactly
    /// `sample_chunk_producer`. When every eligible producer is blacklisted, fall
    /// back to the unfiltered sampler: a chunk slot must always have a producer.
    pub fn sample_chunk_producer_with_kickout(
        &self,
        shard_layout: &ShardLayout,
        shard_id: ShardId,
        height: BlockHeight,
        kickout_state: &ChunkProducerKickoutState,
    ) -> Option<ValidatorId> {
        let exclude = kickout_state.exclude_ids(self);
        self.sample_chunk_producer_excluding(shard_layout, shard_id, height, &exclude)
            .or_else(|| self.sample_chunk_producer(shard_layout, shard_id, height))
    }
