...
impl From<&ExecutionOutcome> for PartialExecutionOutcome {
    fn from(outcome: &ExecutionOutcome) -> Self {
        Self {
            receipt_ids: outcome.receipt_ids().to_vec(),
...
/// V1 has the same fields as V0, except `compute_usage` is now part of the
/// borsh serialization (V0 skipped it, so it was never persisted to disk).
#[derive(
    BorshSerialize,
    BorshDeserialize,
    PartialEq,
...
pub struct ExecutionOutcomeV1 {
    ...
    pub gas_burnt: Gas,
    /// The amount of compute time spent by the given transaction or receipt.
    /// Unlike V0, this field is now persisted as part of the borsh format.
    pub compute_usage: Option<Compute>,
    /// Sum of tokens burnt for:
    /// - Gas: This value doesn't always equal to the `gas_burnt` multiplied by the gas price,
    ///   because the prepaid gas price might be lower than the actual gas price and it creates
...
/// Upper bound for the first borsh byte of a serialized `ExecutionOutcomeV0`.
/// V0's first field is `logs: Vec<LogEntry>`, encoded with a 4-byte
/// little-endian length prefix; the `max_number_logs = 100` runtime limit
/// (see `core/parameters/res/runtime_configs/parameters.yaml`) ensures the
/// low byte of that length is always in `[0, 100]`. V1+ uses tag values
/// strictly greater than `V0_MAX_FIRST_BYTE` as the leading byte, leaving
/// it free to act as a self-describing version discriminant on the wire
/// without prefixing V0 (preserving backward compatibility).
const V0_MAX_FIRST_BYTE: u8 = 100;

/// Borsh tag byte for `ExecutionOutcome::V1`. Must be > `V0_MAX_FIRST_BYTE`
/// so the deserializer can distinguish it from any valid V0 leading byte.
const V1_TAG: u8 = V0_MAX_FIRST_BYTE + 1;

#[derive(Eq, PartialEq, Clone, ProtocolSchema)]
pub enum ExecutionOutcome {
    V0(ExecutionOutcomeV0),
    V1(ExecutionOutcomeV1),
}

impl Default for ExecutionOutcome {
    fn default() -> Self {
        ExecutionOutcome::V0(ExecutionOutcomeV0::default())
    }
}
...
impl BorshSerialize for ExecutionOutcome {
    fn serialize<W: Write>(&self, writer: &mut W) -> Result<(), Error> {
        match self {
            // V0 is written without a tag prefix to keep the on-disk format
            // identical to the pre-versioned struct layout. The discriminant
            // scheme assumes the first borsh byte falls in
            // `[0, V0_MAX_FIRST_BYTE]`; reject anything that would violate it
            // rather than silently producing bytes a future reader would
            // misclassify as V1+.
            ExecutionOutcome::V0(v0) => {
                if v0.logs.len() > V0_MAX_FIRST_BYTE as usize {
                    return Err(Error::new(
                        ErrorKind::InvalidData,
                        format!(
                            "ExecutionOutcomeV0 logs.len() ({}) exceeds V0_MAX_FIRST_BYTE ({}); \
                             serializing would collide with the V1+ discriminant range",
                            v0.logs.len(),
                            V0_MAX_FIRST_BYTE,
                        ),
                    ));
...
impl BorshDeserialize for ExecutionOutcome {
    /// Disambiguate V0 from V1+ by inspecting the first byte. For V0 it is
    /// the low byte of `logs.len()` (a u32 LE length) and is guaranteed to be
    /// in `[0, V0_MAX_FIRST_BYTE]` by the `max_number_logs` runtime limit.
    /// V1+ uses tag values strictly greater than `V0_MAX_FIRST_BYTE`.
    fn deserialize_reader<R: Read>(reader: &mut R) -> std::io::Result<Self> {
        let u1 = u8::deserialize_reader(reader)?;
        if u1 <= V0_MAX_FIRST_BYTE {
            let prefix = [u1];
            let mut reader = prefix.as_slice().chain(reader);
            let v0 = ExecutionOutcomeV0::deserialize_reader(&mut reader)?;
            return Ok(ExecutionOutcome::V0(v0));
        }
        if u1 == V1_TAG {
            let v1 = ExecutionOutcomeV1::deserialize_reader(reader)?;
            return Ok(ExecutionOutcome::V1(v1));
        }
        Err(Error::new(
            ErrorKind::InvalidData,
            format!("invalid execution outcome version tag: {}", u1),
        ))
    }
}
...
impl ExecutionOutcome {
    pub fn logs(&self) -> &[LogEntry] {
        match self {
            ExecutionOutcome::V0(v0) => &v0.logs,
            ExecutionOutcome::V1(v1) => &v1.logs,
        }
    }

    pub fn logs_mut(&mut self) -> &mut Vec<LogEntry> {
        match self {
            ExecutionOutcome::V0(v0) => &mut v0.logs,
            ExecutionOutcome::V1(v1) => &mut v1.logs,
        }
    }

    pub fn receipt_ids(&self) -> &[CryptoHash] {
        match self {
            ExecutionOutcome::V0(v0) => &v0.receipt_ids,
            ExecutionOutcome::V1(v1) => &v1.receipt_ids,
        }
    }

    pub fn gas_burnt(&self) -> Gas {
        match self {
            ExecutionOutcome::V0(v0) => v0.gas_burnt,
            ExecutionOutcome::V1(v1) => v1.gas_burnt,
        }
    }

    pub fn compute_usage(&self) -> Option<Compute> {
        match self {
            ExecutionOutcome::V0(v0) => v0.compute_usage,
            ExecutionOutcome::V1(v1) => v1.compute_usage,
        }
    }

    pub fn set_compute_usage(&mut self, compute_usage: Option<Compute>) {
        match self {
            ExecutionOutcome::V0(v0) => v0.compute_usage = compute_usage,
            ExecutionOutcome::V1(v1) => v1.compute_usage = compute_usage,
        }
    }

    pub fn tokens_burnt(&self) -> Balance {
        match self {
            ExecutionOutcome::V0(v0) => v0.tokens_burnt,
            ExecutionOutcome::V1(v1) => v1.tokens_burnt,
        }
    }

    pub fn executor_id(&self) -> &AccountId {
        match self {
            ExecutionOutcome::V0(v0) => &v0.executor_id,
            ExecutionOutcome::V1(v1) => &v1.executor_id,
        }
    }

    pub fn status(&self) -> &ExecutionStatus {
        match self {
            ExecutionOutcome::V0(v0) => &v0.status,
            ExecutionOutcome::V1(v1) => &v1.status,
        }
    }

    pub fn metadata(&self) -> &ExecutionMetadata {
        match self {
            ExecutionOutcome::V0(v0) => &v0.metadata,
            ExecutionOutcome::V1(v1) => &v1.metadata,
        }
    }
}
...
impl fmt::Debug for ExecutionOutcome {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let version: u32 = match self {
            ExecutionOutcome::V0(_) => 0,
            ExecutionOutcome::V1(_) => 1,
        };
...
impl ExecutionOutcomeWithId {
    ...
    pub fn to_hashes(&self) -> Vec<CryptoHash> {
        ...
        let mut result = Vec::with_capacity(2 + logs.len());
        result.push(self.id);
        result.push(CryptoHash::hash_borsh(PartialExecutionOutcome::from(&self.outcome)));
        result.extend(logs.iter().map(|log| hash(log.as_bytes())));
        result
    }
}
...
mod tests {
    ...
    fn test_outcome_to_hashes() {
        let outcome = ExecutionOutcome::V0(ExecutionOutcomeV0 {
    ...
    #[test]
    fn test_execution_outcome_v0_round_trip() {
        let outcome = ExecutionOutcome::V0(ExecutionOutcomeV0 {
            status: ExecutionStatus::SuccessValue(vec![1, 2, 3]),
            logs: vec!["log".to_string()],
            receipt_ids: vec![CryptoHash([7u8; 32])],
            gas_burnt: Gas::from_gas(42),
            compute_usage: None,
            tokens_burnt: Balance::from_yoctonear(99),
            executor_id: "alice".parse().unwrap(),
            metadata: ExecutionMetadata::V1,
        });
        let bytes = borsh::to_vec(&outcome).unwrap();
        let decoded = ExecutionOutcome::try_from_slice(&bytes).unwrap();
        assert_eq!(outcome, decoded);
    }

    /// Boundary check: an outcome with `logs.len() == V0_MAX_FIRST_BYTE` (the
    /// largest count whose low byte still falls in the V0 range) must still
    /// round-trip and its first byte must equal the discriminant boundary.
    #[test]
    fn test_execution_outcome_v0_max_logs_round_trip() {
        let outcome = ExecutionOutcome::V0(ExecutionOutcomeV0 {
            status: ExecutionStatus::SuccessValue(vec![]),
            logs: vec!["x".to_string(); V0_MAX_FIRST_BYTE as usize],
            receipt_ids: vec![],
            gas_burnt: Gas::ZERO,
            compute_usage: None,
        ...
        let bytes = borsh::to_vec(&outcome).unwrap();
        assert_eq!(bytes[0], V0_MAX_FIRST_BYTE);
        let decoded = ExecutionOutcome::try_from_slice(&bytes).unwrap();
        assert_eq!(outcome, decoded);
    }

    /// `max_number_logs` is the runtime cap that backs the
    /// `V0_MAX_FIRST_BYTE` discriminant: every V0 outcome ever written
    /// satisfies `logs.len() ≤ V0_MAX_FIRST_BYTE`. If the runtime config ever
    /// raises this above the discriminant, this test fails loudly so the
    /// versioning scheme can be revisited before the change ships.
    #[test]
    fn test_max_number_logs_within_v0_discriminant_range() {
        let store = near_parameters::RuntimeConfigStore::new(None);
        let config = store.get_config(near_primitives_core::version::PROTOCOL_VERSION);
        assert!(
            config.wasm_config.limit_config.max_number_logs <= V0_MAX_FIRST_BYTE as u64,
            "max_number_logs ({}) must stay <= V0_MAX_FIRST_BYTE ({}) to preserve the ExecutionOutcome borsh discriminant",
            config.wasm_config.limit_config.max_number_logs,
            V0_MAX_FIRST_BYTE,
        );
    }

    #[test]
    fn test_execution_outcome_invalid_version_tag() {
        let bytes = vec![200u8; 16];
        let err = ExecutionOutcome::try_from_slice(&bytes).unwrap_err();
        assert_eq!(err.kind(), ErrorKind::InvalidData);
        assert!(err.to_string().contains("invalid execution outcome version tag"));
    }

    #[test]
    fn test_execution_outcome_v1_round_trip() {
        let outcome = ExecutionOutcome::V1(ExecutionOutcomeV1 {
            status: ExecutionStatus::SuccessValue(vec![1, 2, 3]),
            logs: vec!["log".to_string()],
            receipt_ids: vec![CryptoHash([7u8; 32])],
            gas_burnt: Gas::from_gas(42),
            compute_usage: Some(123),
            tokens_burnt: Balance::from_yoctonear(99),
            executor_id: "alice".parse().unwrap(),
            metadata: ExecutionMetadata::V1,
        });
        let bytes = borsh::to_vec(&outcome).unwrap();
        assert_eq!(bytes[0], V1_TAG);
        let decoded = ExecutionOutcome::try_from_slice(&bytes).unwrap();
        assert_eq!(outcome, decoded);
        // The whole point of V1: compute_usage survives a borsh round-trip.
        assert_eq!(decoded.compute_usage(), Some(123));
    }

    /// V0 serialization must reject `logs.len() > V0_MAX_FIRST_BYTE`: such an
    /// outcome's first borsh byte would fall in the V1+ tag range and confuse
    /// the deserializer. This guards against a future config bump that lifts
    /// `max_number_logs` past the discriminant boundary.
    #[test]
    fn test_execution_outcome_v0_serialize_rejects_logs_over_limit() {
        let outcome = ExecutionOutcome::V0(ExecutionOutcomeV0 {
            status: ExecutionStatus::SuccessValue(vec![]),
            logs: vec!["x".to_string(); V0_MAX_FIRST_BYTE as usize + 1],
            receipt_ids: vec![],
            gas_burnt: Gas::ZERO,
            compute_usage: None,
            tokens_burnt: Balance::ZERO,
        ...
        let err = borsh::to_vec(&outcome).unwrap_err();
        assert_eq!(err.kind(), ErrorKind::InvalidData);
        assert!(err.to_string().contains("exceeds V0_MAX_FIRST_BYTE"));
    }

    /// V0 still drops `compute_usage` through borsh (the field is `#[borsh(skip)]`).
    /// This guards the invariant that justifies introducing V1.
    #[test]
    fn test_execution_outcome_v0_drops_compute_usage() {
        let outcome = ExecutionOutcome::V0(ExecutionOutcomeV0 {
            status: ExecutionStatus::SuccessValue(vec![]),
            logs: vec![],
            receipt_ids: vec![],
            gas_burnt: Gas::ZERO,
            compute_usage: Some(42),
            tokens_burnt: Balance::ZERO,
            executor_id: "alice".parse().unwrap(),
            metadata: ExecutionMetadata::V1,
        });
        let bytes = borsh::to_vec(&outcome).unwrap();
        let decoded = ExecutionOutcome::try_from_slice(&bytes).unwrap();
        assert_eq!(decoded.compute_usage(), None);
    }
