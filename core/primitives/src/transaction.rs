...
pub enum ExecutionOutcome {
    ...
    // To introduce V1: add a `V1(ExecutionOutcomeV1)` variant. The borsh impls
    // below will then need a discriminant scheme to disambiguate versions on
    // the wire while preserving V0 backward compatibility.
}
...
impl BorshSerialize for ExecutionOutcome {
    fn serialize<W: Write>(&self, writer: &mut W) -> Result<(), Error> {
        match self {
            ExecutionOutcome::V0(v0) => v0.serialize(writer),
        }
    }
}
...
impl BorshDeserialize for ExecutionOutcome {
    fn deserialize_reader<R: Read>(reader: &mut R) -> std::io::Result<Self> {
        let v0 = ExecutionOutcomeV0::deserialize_reader(reader)?;
        Ok(ExecutionOutcome::V0(v0))
    }
}
