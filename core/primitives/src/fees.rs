//! Fee and gas accounting for actions, transactions and receipts.
//!
//! Everything here is a pure function of the [`RuntimeConfig`] and the
//! primitives, so it lives below the runtime crate. That lets genesis state
//! initialization compute the metadata of receipts it stores in state queues
//! the same way the runtime does.

use crate::account::AccessKeyPermission;
use crate::action::DeployGlobalContractAction;
use crate::action::delegate::VersionedDelegateActionRef;
use crate::errors::IntegerOverflowError;
use crate::receipt::{Receipt, VersionedActionReceipt, VersionedReceiptEnum};
use crate::transaction::{Action, DeployContractAction, Transaction};
use crate::types::{AccountId, Balance, Compute, Gas};
use crate::universal_state_init::{RawStateInit, state_init_counts};
...
pub fn storage_removes_compute(
    ext: &ExtCostsConfig,
    count: usize,
...
pub fn delegate_signature_verification_compute(
    fees: &RuntimeFeesConfig,
    public_key: &PublicKey,
) -> Compute {
...
/// protocol upgrade.
pub fn compute_receipt_congestion_gas(
    receipt: &Receipt,
    config: &RuntimeConfig,
...
/// The calculation is part of protocol and should only be modified with a
/// protocol upgrade.
pub fn compute_receipt_size(receipt: &Receipt) -> Result<u64, IntegerOverflowError> {
