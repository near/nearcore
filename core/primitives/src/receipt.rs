...
/// `ReceiptEnum::Action` of `ReceiptEnum::Data`.
#[derive(BorshSerialize, BorshDeserialize, Serialize, Deserialize, Debug, PartialEq, Eq, Clone)]
pub struct Receipt {
    /// An issuer account_id of a particular receipt.
    /// `predecessor_id` could be either `Transaction` `signer_id` or
    /// intermediate contract's `account_id`.
...
impl Receipt {
    ...
    /// Generates a receipt with a transfer from system for a given balance
    /// without a receipt_id. This should be used for token refunds instead
    /// of gas refunds. It doesn't refund the allowance of the access key.
    /// For gas refunds use `new_gas_refund`.
    ...
    /// Generates a receipt with a transfer action from system for a given
    /// balance without a receipt_id. It contains `signer_id` and
    /// `signer_public_key` to indicate this is a gas refund. The execution
    /// of this receipt will try to refund the allowance of the access key
    /// with the given public key. NOTE: The access key may be replaced by
    /// the owner, so the execution can't rely that the access key is the
    /// same and it should use best effort for the refund.
...
/// An incoming (ingress) `DataReceipt` which is going to a Receipt's `receiver`
/// input_data_ids Which will be converted to `PromiseResult::Successful(value)`
/// or `PromiseResult::Failed`
...
/// Stores indices for a persistent queue for delayed receipts that didn't fit
/// into a block.
