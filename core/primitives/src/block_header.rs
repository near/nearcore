...
pub struct BlockHeaderInnerRestV7 {
    ...
    /// Per-ChunkExecutionResult endorsement bitmaps derived from the previous
    /// block's body of spice core statements, ordered 1:1 with
    /// `iter_execution_results`. Each entry carries the (shard_id, height,
    /// chunk_epoch) needed to look up the validator assignment, plus a bit per
    /// validator slot in that assignment.
...
impl BlockHeader {
    ...
    /// Returns `None` for header versions prior to V7. See `SpiceChunkEndorsementStats`.
    #[inline]
