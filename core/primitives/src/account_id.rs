...
lazy_static::lazy_static! {
    ...
    pub static ref SYSTEM_ACCOUNT: AccountId = AccountId::try_from("system").unwrap();
}
...
impl AccountId {
    ...
    pub fn is_valid_top_level_account_id(&self) -> bool {
    ...
    pub fn is_valid_sub_account_id(&self, parent_account_id: &AccountId) -> bool {
...
// NOTE: We cannot implement it over generic AsRef<str> due to an overlapping
// implementation with std:
// https://www.reddit.com/r/rust/comments/c1yubv/why_this_code_does_not_compile/
...
impl std::convert::TryFrom<&[u8]> for AccountId {
    type Error = ParseAccountError;

    fn try_from(account_id: &[u8]) -> Result<Self, Self::Error> {
        Self::try_from(std::str::from_utf8(account_id).map_err(|_| Self::Error::Invalid)?)
    }
}
...
mod tests {
    ...
    fn test_is_valid_account_id() {
        ...
        for account_id in bad_account_ids {
            assert!(
                AccountId::validate(&account_id.to_string()).is_err(),
    ...
    fn test_is_valid_top_level_account_id() {
        ...
        for account_id in ok_top_level_account_ids {
            ...
            assert!(
                AccountId::is_valid_top_level_account_id(&account_id),
    ...
    fn test_is_valid_sub_account_id() {
        ...
        for (signer_id, sub_account_id) in ok_pairs {
            let signer_id = AccountId::try_from(signer_id).unwrap();
            let sub_account_id = AccountId::try_from(sub_account_id).unwrap();
            assert!(
                sub_account_id.is_valid_sub_account_id(&signer_id),
