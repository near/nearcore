...
pub mod trie_key_parsers {
    ...
    pub fn parse_account_id_from_contract_data_key(
        ...
        Ok(AccountId::try_from(account_id_prefix).map_err(|_| {
            std::io::Error::new(
                std::io::ErrorKind::InvalidData,
    ...
    pub fn parse_account_id_from_account_key(raw_key: &[u8]) -> Result<AccountId, std::io::Error> {
        ...
        Ok(AccountId::try_from(account_id).map_err(|_| {
            std::io::Error::new(
                std::io::ErrorKind::InvalidData,
...
mod tests {
    ...
    fn test_key_for_account_consistency() {
        for account_id in OK_ACCOUNT_IDS.iter().map(|x| AccountId::try_from(*x).unwrap()) {
