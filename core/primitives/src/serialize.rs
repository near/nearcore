...
pub fn from_base64(s: &str) -> Result<Vec<u8>, String> {
    base64::decode(s).map_err(|err| err.to_string())
}

pub fn from_base64_buf(s: &str, buffer: &mut Vec<u8>) -> Result<(), String> {
    base64::decode_config_buf(s, base64::STANDARD, buffer).map_err(|err| err.to_string())
}
