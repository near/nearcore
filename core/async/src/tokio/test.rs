...
async fn test_tokio_actor_futures_delayed_actions_and_lifetime() {
    ...
    impl Actor for MyActor {
        fn start_actor(&mut self, ctx: &mut dyn crate::futures::DelayedActionRunner<Self>) {
            ...
            ctx.run_later("test", ::time::Duration::milliseconds(100), move |actor, ctx| {
                actor.delayed_action_executed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
                ctx.run_later("test2", ::time::Duration::milliseconds(100), move |actor, _ctx| {
                    actor
                        .delayed_action_executed
        ...
        fn wrap_handler<M, R>(
            &mut self,
            msg: M,
            ctx: &mut dyn crate::futures::DelayedActionRunner<Self>,
            f: impl FnOnce(&mut Self, M, &mut dyn crate::futures::DelayedActionRunner<Self>) -> R,
        ) -> R {
            assert_eq!(self.started.load(std::sync::atomic::Ordering::Relaxed), 1);
            assert_eq!(self.stopped.load(std::sync::atomic::Ordering::Relaxed), 0);
            self.wrapped.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            f(self, msg, ctx)
        }

        fn stop_actor(&mut self) {
            self.stopped.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        }
    }
    ...
    handle.spawn("test_future", {
        ...
        async move {
            ...
            future_executed.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        }
    });
    ...
    assert_eq!(wrapped.load(std::sync::atomic::Ordering::Relaxed), 2);
    assert_eq!(stopped.load(std::sync::atomic::Ordering::Relaxed), 0);
    assert_eq!(delayed_action_executed.load(std::sync::atomic::Ordering::Relaxed), 0);
    assert_eq!(future_executed.load(std::sync::atomic::Ordering::Relaxed), 0);
    // Wait a bit for the delayed actions and future to execute.
    for _ in 0..10 {
        if delayed_action_executed.load(std::sync::atomic::Ordering::Relaxed) != 4
            || future_executed.load(std::sync::atomic::Ordering::Relaxed) != 1
        {
            tokio::time::sleep(std::time::Duration::from_millis(100)).await;
        }
    }
    assert_eq!(delayed_action_executed.load(std::sync::atomic::Ordering::Relaxed), 4);
    assert_eq!(future_executed.load(std::sync::atomic::Ordering::Relaxed), 1);
    ...
    for _ in 0..10 {
        if delayed_action_executed.load(std::sync::atomic::Ordering::Relaxed) < 7
            || future_executed.load(std::sync::atomic::Ordering::Relaxed) < 5
        {
            tokio::time::sleep(std::time::Duration::from_millis(100)).await;
        }
    }
    assert!(delayed_action_executed.load(std::sync::atomic::Ordering::Relaxed) >= 7);
    assert!(future_executed.load(std::sync::atomic::Ordering::Relaxed) >= 5);
    ...
    actor_system.stop();
    for _ in 0..10 {
        if stopped.load(std::sync::atomic::Ordering::Relaxed) == 0 {
            tokio::time::sleep(std::time::Duration::from_millis(100)).await;
        }
    }
    assert_eq!(stopped.load(std::sync::atomic::Ordering::Relaxed), 1);
    let delayed_action_count = delayed_action_executed.load(std::sync::atomic::Ordering::Relaxed);
    let future_count = future_executed.load(std::sync::atomic::Ordering::Relaxed);
    // Wait a bit to make sure no more recurrent delayed actions are executed.
    tokio::time::sleep(std::time::Duration::from_millis(200)).await;
    assert_eq!(
        delayed_action_executed.load(std::sync::atomic::Ordering::Relaxed),
        delayed_action_count
    );
    assert_eq!(future_executed.load(std::sync::atomic::Ordering::Relaxed), future_count);
}
...
async fn test_tokio_runtime_shutdown() {
    ...
    tokio::time::sleep(std::time::Duration::from_millis(100)).await;
    assert_eq!(dropped.dropped.load(std::sync::atomic::Ordering::Relaxed), 0);
    ...
    handle1.stop();
    for _ in 0..10 {
        if dropped.dropped.load(std::sync::atomic::Ordering::Relaxed) != 2 {
            tokio::time::sleep(std::time::Duration::from_millis(100)).await;
        }
    }
    assert_eq!(dropped.dropped.load(std::sync::atomic::Ordering::Relaxed), 2);

    // Stop the actor system; this should drop the remaining actors and the delayed action.
    actor_system.stop();
    for _ in 0..10 {
        if dropped.dropped.load(std::sync::atomic::Ordering::Relaxed) != 5 {
            tokio::time::sleep(std::time::Duration::from_millis(100)).await;
        }
    }
    assert_eq!(dropped.dropped.load(std::sync::atomic::Ordering::Relaxed), 5);
