...
static THREAD_POOL_NUM_THREADS: LazyLock<IntGaugeVec> = LazyLock::new(|| {
    try_create_int_gauge_vec(
        ...
        "current number of threads in a thread pool",
        &["pool_name"],
    )
    .unwrap()
});
...
static THREAD_POOL_MAX_NUM_THREADS: LazyLock<IntGaugeVec> = LazyLock::new(|| {
    try_create_int_gauge_vec(
        ...
        "maximum observed number of threads in a thread pool",
        &["pool_name"],
    )
    .unwrap()
});
...
/// Scheduling policy for threads spawned by [`ThreadPool`].
#[derive(Clone, Copy)]
pub enum Priority {
    /// Spawn threads with the normal OS scheduling policy (`SCHED_OTHER`).
    /// Appropriate for work that is not latency-critical (e.g., contract
    /// compilation).
    Default,
    /// Spawn threads under round-robin realtime policy (`SCHED_RR`) with the
    /// given priority in [0; 100]. Realtime threads always take precedence
    /// over threads using normal policy, so the value applies only among
    /// other realtime threads.
    Realtime(u8),
}

/// OS thread pool for spawning tasks onto dedicated threads.
///
...
/// reused for new tasks. See [`Priority`] for the available scheduling options.
pub struct ThreadPool {
    /// Name of the pool. Used for logging/debugging purposes.
    name: &'static str,
    /// Scheduling policy used for spawned worker threads.
...
#[derive(Clone, Copy)]
enum PriorityResolved {
    Default,
    Realtime(ThreadPriority),
}
...
impl ThreadPool {
    /// Create a new thread pool. Panics if `priority` is
    /// [`Priority::Realtime`] with a value outside [0; 100].
    pub fn new(
        name: &'static str,
        idle_timeout: Duration,
        limit: usize,
        ...
        let priority = match priority {
            Priority::Default => PriorityResolved::Default,
            Priority::Realtime(value) => {
                PriorityResolved::Realtime(value.try_into().expect("priority out of range"))
            }
        };
        Self {
            name,
    ...
    fn spawn_thread(&self) {
        ...
        match self.priority {
            PriorityResolved::Realtime(priority) => {
...
mod tests {
    ...
    const DEFAULT_PRIORITY: Priority = Priority::Realtime(50);
    ...
    fn invalid_priority() {
        ThreadPool::new(POOL_NAME, DEFAULT_IDLE_TIMEOUT, DEFAULT_LIMIT, Priority::Realtime(101));
    }
    ...
    #[test]
    fn single_job_default_priority() {
        let pool =
            ThreadPool::new(POOL_NAME, DEFAULT_IDLE_TIMEOUT, DEFAULT_LIMIT, Priority::Default);
        execute_job(&pool);
    }
