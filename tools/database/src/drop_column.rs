...
impl DropColumnCommand {
    pub(crate) fn run(&self, home: &PathBuf) -> anyhow::Result<()> {
        ...
        let column = match resolve_column(&self.column) {
            Ok(col) => {
                println!(
                    "Column {} is recognized by the binary. Note that it will be automatically recreated each time neard is run.",
                    self.column
                );
                Some(col)
            }
            Err(_) => {
                println!(
                    "Column {} is not recognized by the binary. Will attempt to remove it.",
                    self.column
                );
                None
            }
