...
fn apply_block_from_range(
    ...
    let existing_outcomes: Vec<_> = existing_all_outcomes.get(&shard_id).cloned().unwrap_or_default().iter().map(|outcome| outcome.outcome_with_id).collect();
    ...
    let existing_ids = HashSet::from_iter(existing_receipts.keys().cloned());
    let ids = HashSet::from_iter(receipts.keys().cloned());
    let diff_ids: Vec<&CryptoHash> = existing_ids.difference(&ids).collect();
    if diff_ids.len() > 0 {
