...
impl ApplyCmd {
    pub fn run(
        ...
        apply_block_at_height(
            ...
            near_config,
            store,
            self.save_trie.map(|temperature| initialize_write_store(temperature, node_storage)),
        )
        .unwrap();
    }
}
...
impl ApplyRangeCmd {
    pub fn run(
        ...
        if matches!(self.mode, ApplyRangeMode::Benchmarking) && self.save_trie.is_some() {
...
fn initialize_write_store(temperature: Temperature, node_storage: NodeStorage) -> Store {
    match temperature {
        Temperature::Hot => node_storage.get_hot_store(),
        Temperature::Cold => node_storage
            .get_cold_store()
