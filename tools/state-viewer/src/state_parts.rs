...
fn apply_state_parts(
    ...
    let (state_root, epoch_height, epoch_id, sync_hash) = if let (
        Some(state_root),
        EpochSelection::EpochHeight { epoch_height },
    ) =
        (maybe_state_root, &epoch_selection)
    {
        ...
        let sync_block = chain.get_block(&sync_hash).unwrap();
        tracing::info!(
            target: "state-parts",
            ?sync_hash,
            height = sync_block.header().height(),
            state_roots = ?sync_block.chunks().iter().map(|chunk| chunk.prev_state_root()).collect::<Vec<StateRoot>>());

        assert!(chain.runtime_adapter.is_next_block_epoch_start(&sync_hash).unwrap());
        assert!(
            shard_id < sync_block.chunks().len() as u64,
            "shard_id: {}, #shards: {}",
            shard_id,
            sync_block.chunks().len()
        );
...
fn dump_state_parts(
    ...
    let state_root = sync_block.chunks()[shard_id as usize].prev_state_root();
    let state_root_node =
        chain.runtime_adapter.get_state_root_node(shard_id, &sync_hash, &state_root).unwrap();
