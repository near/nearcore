...
impl Drop for Ctx_ {
    fn drop(&mut self) {
        self.done.set(CtxErr::Cancelled);
    }
}
...
impl Ctx {
    ...
    fn cancel(&self) {
        self.0.done.set(CtxErr::Cancelled);
    }

    // err() returns the CtxErr if the Context was already cancelled,
    // None otherwise.
    pub fn err(&self) -> Option<CtxErr> {
        self.0.done.get()
    }

    // done() waits until the context gets cancelled and returns the CtxErr.
    pub async fn done(&self) -> CtxErr {
    ...
    // instead).
    pub async fn wrap<F, T>(&self, f: F) -> Result<T, CtxErr>
    where
    ...
    // has passed, or CtxErr if the context got cancelled.
    ...
    fn new_child(&self, label: Option<String>, deadline: Option<time::Instant>) -> Ctx {
        ...
        let done = Arc::new(Once::<CtxErr>::new());
        tokio::spawn({
            ...
            async move {
                ...
                tokio::select! {
                    _ = deadline_wait => { child_done.set(CtxErr::Timeout); }
                    e = parent_done.wait() => { child_done.set(e); }
                    _ = child_done.wait() => {}
                }
            }
        });
