...
impl TxTracker {
            ...
            if let Some(c) = &s.chunk {
                if c.header.height_included == msg.block.header.height {
                    ...
                    for r in c.receipts.iter() {
                        match &r.receipt {
                            ReceiptEnumView::Action { signer_id, actions, .. } => {
                                let mut deleg = false;
                                ...
                                if deleg {
                                    tracing::info!(
                                        "zzzzzzzzzz receipt {} signer {} pred {} receiver {}",
                                        r.receipt_id,
                                        signer_id,
                                        r.predecessor_id,
                                        r.receiver_id
    ...
    pub(crate) async fn on_txs_sent(
        ...
        target_height: BlockHeight,
        start_time: Option<(tokio::time::Instant, chrono::DateTime<chrono::Utc>)>,
    ) -> anyhow::Result<()> {
        ...
        let (txs_sent, provenance) = match sent_batch {
            SentBatch::MappedBlock(b) => {
                ...
                let reset_time_debug = start_time.map(|s| s.1 + block_delay);
                let reset_time =
                    start_time.map_or_else(tokio::time::Instant::now, |s| s.0) + block_delay;
