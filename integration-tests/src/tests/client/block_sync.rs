...
fn test_block_sync() {
    ...
    let mut block_sync = BlockSync::new(Clock::real(), network_adapter.as_multi_sender(), false);
...
fn test_block_sync_archival() {
    let network_adapter = Arc::new(MockPeerManagerAdapter::default());
    let max_block_requests = 10;
    let mut block_sync = BlockSync::new(Clock::real(), network_adapter.as_multi_sender(), true);
