...
fn test_storage_after_commit_of_cold_update() {
    ...
    for height in 1..max_height {
        ...
        update_cold_db(&archival_store, &client_store, &shard_layout, &height, 4).unwrap();
...
fn test_cold_db_head_update() {
    ...
    for height in 1..max_height {
        ...
        let client_store = env.clients[0].runtime_adapter.store();
        update_cold_head(&archival_store, &client_store, &height).unwrap();
        ...
        let cold_head_in_cold = archival_store.get_head().unwrap();
...
fn test_cold_db_copy_with_height_skips() {
    ...
    for height in 1..max_height {
        ...
        let epoch_id = client.epoch_manager.get_epoch_id_from_prev_block(&last_hash).unwrap();
        let shard_layout = client.epoch_manager.get_shard_layout(&epoch_id).unwrap();
        update_cold_db(&archival_store, &client.runtime_adapter.store(), &shard_layout, &height, 1)
            .unwrap();
...
fn test_cold_loop_on_gc_boundary() {
    ...
    let start_cold_head = archival_store.get_head().unwrap().unwrap().height;
    ...
    let end_cold_head = archival_store.get_head().unwrap().unwrap().height;
