...
fn sync_state_dump() {
    heavy_test(|| {
        ...
        run_actix(async move {
            ...
            near1.client_config.tracked_shards = vec![0.into()]; // Track all shards.
                    ...
                    match view_client1.send(GetBlock::latest().with_span_context()).await {
                        Ok(Ok(b)) if b.header.height >= genesis.config.epoch_length + 2 => {
                            ...
                            if view_client2_holder2.is_none() {
                                ...
                                near2.client_config.tracked_shards = vec![0.into()]; // Track all shards.
...
fn test_state_sync_headers_no_tracked_shards() {
    heavy_test(|| {
        ...
        run_actix(async {
            ...
            let mut near1 = load_test_config("test1", port1, genesis.clone());
            near1.client_config.min_num_peers = 0;
            near1.client_config.tracked_shards = vec![0.into()]; // Track all shards, it is a validator.
