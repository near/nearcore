...
fn ultra_slow_test_sync_state_dump() {
    heavy_test(|| {
        ...
        run_actix(async move {
            ...
            near1.client_config.tracked_config = TrackedShardsConfig::AllShards;
                    ...
                    match view_client1.send(GetBlock::latest().with_span_context()).await {
                        ...
                        Ok(Ok(b)) if b.header.height >= genesis.config.epoch_length + 2 => {
                            ...
                            if view_client2_holder2.is_none() {
                                ...
                                near2.client_config.tracked_config = TrackedShardsConfig::AllShards;
...
fn slow_test_state_sync_headers_no_tracked_shards() {
    heavy_test(|| {
        ...
        run_actix(async {
            ...
            near1.client_config.tracked_config = TrackedShardsConfig::AllShards; // Track all shards, it is a validator.
            ...
            near2.client_config.tracked_config = TrackedShardsConfig::NoShards;
