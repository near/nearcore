...
async fn ultra_slow_test_sync_state_nodes_multishard() {
    ...
    near3.client_config.min_block_production_delay = near1.client_config.min_block_production_delay;
    near3.client_config.max_block_production_delay = near1.client_config.max_block_production_delay;
    ...
    near4.client_config.min_block_production_delay = near1.client_config.min_block_production_delay;
                ...
                match &view_client1.send_async(GetBlock::latest()).await {
                    Ok(Ok(b)) if b.header.height >= 101 => {
                        ...
                        if view_client2_holder2.is_none() {
                            ...
                            near2.network_config.peer_store.boot_nodes = convert_boot_nodes(vec![
                                ("test1", *port1),
                                ("test3", *port3),
                                ("test4", *port4),
                            ]);
...
async fn ultra_slow_test_dump_epoch_missing_chunk_in_last_block() {
    init_test_logger();
    let epoch_length = 12;
