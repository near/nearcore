...
pub(crate) fn create_account(env: &mut TestEnv, old_account_id: AccountId, new_account_id: AccountId, epoch_length: u64, height: BlockHeight, protocol_version: ProtocolVersion) -> CryptoHash {
    let block = env.clients[0].chain.get_block_by_height(height - 1).unwrap();
    let signer =
        InMemorySigner::from_seed(old_account_id.clone(), KeyType::ED25519, old_account_id.as_ref());
