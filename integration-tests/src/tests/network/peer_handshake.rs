...
fn peer_handshake() {
    ...
    run_actix(async {
        let addr1 = tcp::ListenerAddr::new_localhost();
        let addr2 = tcp::ListenerAddr::new_localhost();
        let pm1 =
            make_peer_manager("test1", addr1.clone(), vec![("test2", addr2.as_ref().port())], 10);
        let _pm2 =
            make_peer_manager("test2", addr2.clone(), vec![("test1", addr1.as_ref().port())], 10);
...
fn peers_connect_all() {
    ...
    run_actix(async {
        let addr = tcp::ListenerAddr::new_localhost();
        ...
        for i in 0..num_peers {
            let pm = make_peer_manager(
                &format!("test{}", i),
                tcp::ListenerAddr::new_localhost(),
                vec![("test", addr.as_ref().port())],
                10,
            );
            peers.push(pm);
        }
