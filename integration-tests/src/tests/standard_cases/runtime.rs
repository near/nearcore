...
fn create_runtime_with_expensive_storage() -> RuntimeNode {
    ...
    fees.storage_usage_config.storage_amount_per_byte = TESTING_INIT_BALANCE.checked_div(1000).unwrap();
    let records = genesis.force_read_records().as_mut();
    match &mut records[0] {
        StateRecord::Account { account, .. } => account.set_amount(TESTING_INIT_BALANCE.checked_mul(10000).unwrap()),
        _ => {
