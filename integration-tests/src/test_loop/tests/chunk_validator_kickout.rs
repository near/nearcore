...
/// Runs test for chunk validator kickout testing.
///
/// First, we choose an account as the subject of the test. If `select_chunk_validator_only` is true,
/// we select a chunk-validator only account, otherwise choose a block/chunk producer account.
/// Second, if `drop_chunks` is true, we drop the chunks that would be validated by the selected account,
/// otherwise, we drop the chunk endorsements generated by the selected account.
