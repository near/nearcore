...
// Deploy the test contract from the given account to itself. Use the node data
// of the provided rpc account id to send the transaction. Do not wait for the
// response. Return the transaction hash.
pub fn deploy_contract(
    test_loop: &mut TestLoopV2,
    node_datas: &[TestData],
    rpc_account_id: &AccountId,
    account_id: &AccountId,
) -> CryptoHash {
    let block_hash = get_shared_block_hash(node_datas, test_loop);

    let contract = near_test_contracts::rs_contract();
    let code = contract.to_vec();
    let signer = create_user_test_signer(&account_id).into();

    // TOOD make nonce an argument
    let nonce = 1;

    let tx = SignedTransaction::deploy_contract(account_id, &signer, nonce, code, block_hash);
    let tx_hash = tx.get_hash();
    let process_tx_request =
        ProcessTxRequest { transaction: tx, is_forwarded: false, check_only: false };

    let rpc_node_data = get_node_data(node_datas, rpc_account_id);
    let rpc_node_data_sender = &rpc_node_data.client_sender.clone();
    let future = rpc_node_data_sender.send_async(process_tx_request);
    drop(future);

    tracing::info!(target: "test", ?account_id, ?tx_hash, "deployed contract");
    tx_hash
}
...
pub fn call_contract(
    ...
    // TOOD make nonce an argument
    let nonce = 2;
    let signer = create_user_test_signer(&sender_account_id);
