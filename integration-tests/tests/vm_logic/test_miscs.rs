mod tests {
    ...
    use crate::vm_logic::vm_logic_builder::VMLogicBuilder;
    ...
    fn test_valid_log_utf16_max_log_len_not_even() {
        ...
        logic
            .log_utf16(std::u64::MAX, utf16_bytes.as_ptr() as _)
            .expect("Valid utf-16 string_bytes");
    ...
    fn test_sha256() {
        ...
        assert_eq!(
            res,
            &[
                18, 176, 115, 156, 45, 100, 241, 132, 180, 134, 77, 42, 105, 111, 199, 127, 118,
                112, 92, 255, 88, 43, 83, 147, 122, 55, 26, 36, 42, 156, 160, 158,
            ]
        );
    ...
    fn test_keccak256() {
        ...
        assert_eq!(
            res.as_slice(),
            &[
                104, 110, 58, 122, 230, 181, 215, 145, 231, 229, 49, 162, 123, 167, 177, 58, 26,
                142, 129, 173, 7, 37, 9, 26, 233, 115, 64, 102, 61, 85, 10, 159
            ]
        );
        let len = data.len() as u64;
        assert_costs(map! {
    ...
    fn test_keccak512() {
        ...
        assert_eq!(
            res,
            &[
                55, 134, 96, 137, 168, 122, 187, 95, 67, 76, 18, 122, 146, 11, 225, 106, 117, 194,
                154, 157, 48, 160, 90, 146, 104, 209, 118, 126, 222, 230, 200, 125, 48, 73, 197,
                236, 123, 173, 192, 197, 90, 153, 167, 121, 100, 88, 209, 240, 137, 86, 239, 41,
                87, 128, 219, 249, 136, 203, 220, 109, 46, 168, 234, 190
            ]
            .to_vec()
        );
        let len = data.len() as u64;
    ...
    fn test_ripemd160() {
        ...
        assert_eq!(
            res,
            &[
                21, 102, 156, 115, 232, 3, 58, 215, 35, 84, 129, 30, 143, 86, 212, 104, 70, 97, 14,
                225,
            ]
        );
        let len = data.len() as u64;
    ...
    fn test_ecrecover() {
        ...
        let signature: [u8; 64] = [
            ...
            0x75, 0xc3, 0x2b, 0x39, 0x64, 0x3b, 0xe8, 0x92,
        ];
        let signer: [u8; 64] = [
            0xb3, 0x68, 0x70, 0xea, 0xab, 0x31, 0xcb, 0xeb, 0x1a, 0x5c, 0x07, 0x46, 0x7b, 0x42,
            0x97, 0x40, 0x7c, 0x62, 0x11, 0x7a, 0x31, 0x15, 0x47, 0xc4, 0x30, 0x5e, 0x14, 0x71,
            0x52, 0x1b, 0x53, 0x01, 0xc2, 0x59, 0x9d, 0x4e, 0xad, 0xdf, 0xd3, 0x84, 0x9d, 0xf9,
            0xd5, 0x99, 0x38, 0xfd, 0x8f, 0x16, 0x56, 0x47, 0x77, 0x32, 0x66, 0x80, 0x66, 0xff,
            0xa1, 0x2e, 0xb3, 0x47, 0xea, 0xb4, 0x7b, 0x9c,
        ];

        let b =
            logic.ecrecover(32, hash.as_ptr() as _, 64, signature.as_ptr() as _, 0, 0, 1).unwrap();
        assert_ne!(b, 0);
