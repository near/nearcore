mod tests {
    ...
    fn minimal_repro(epoch_length: u64, num_heights: u64, max_seed: u64) {
        ...
        if !std::panic::catch_unwind(move || {
            run_with_seed_range(epoch_length, num_heights, max_seed)
        })
        .is_err()
        {
            println!("PASS");
    ...
    fn run_with_seed_range(epoch_length: u64, num_heights: u64, max_seed: u64) {
        for test_seed in 0..max_seed {
            if let Err(e) = std::panic::catch_unwind(move || {
                run_with_seed(epoch_length, num_heights, test_seed)
            }) {
    ...
    fn do_random_test<RngImpl: Rng>(
        ...
        let heights_to_pick = (0u64..1u64)
            .chain(
                (1u64..num_heights).filter(|_i| rng.gen_range(0.0, 1.0) >= skip_height_probability),
            )
            .collect::<Vec<_>>();
    ...
    fn verify_block_stats(
        ...
        for i in 1..block_infos.len() {
            ...
            let blocks_in_epoch =
                (i - heights.binary_search(&prev_epoch_end_height).unwrap()) as u64;
