mod tests {
    ...
    use testlib::client_test_utils::{create_chunk, create_chunk_with_transactions, TestEnv};
    ...
    fn test_verify_chunk_invalid_proofs_challenge() {
        ...
        env.produce_block(0, 1);
        let (chunk, _merkle_paths, _receipts, block) =
    ...
    fn test_verify_chunk_invalid_proofs_challenge_decoded_chunk() {
        ...
        let chunk = encoded_chunk
            .decode_chunk(env.clients[0].chain.runtime_adapter.num_data_parts())
            .unwrap();
    ...
    fn test_verify_chunk_proofs_malicious_challenge_no_changes() {
        ...
        // Valid chunk
        let (chunk, _merkle_paths, _receipts, block) =
            create_chunk(&mut env.clients[0], None, None);
    ...
    fn test_verify_chunk_invalid_state_challenge() {
        ...
        };
        {
            let prev_merkle_proofs =
                Block::compute_chunk_headers_root(last_block.chunks().iter()).1;
            ...
            assert_eq!(
                ...
                vec![
                    vec![
                        1, 5, 0, 10, 178, 228, 151, 124, 13, 70, 6, 146, 31, 193, 111, 108, 60,
                        102, 227, 106, 220, 133, 45, 144, 104, 255, 30, 155, 129, 215, 15, 43, 202,
                        26, 122, 171, 30, 7, 228, 175, 99, 17, 113, 5, 94, 136, 200, 39, 136, 37,
                        110, 166, 241, 148, 128, 55, 131, 173, 97, 98, 201, 68, 82, 244, 223, 70,
                        86, 161, 5, 0, 0, 0, 0, 0, 0
                    ],
                    vec![
                        3, 1, 0, 0, 0, 16, 49, 233, 115, 11, 86, 10, 193, 50, 45, 253, 137, 126,
                        230, 236, 254, 86, 230, 148, 94, 141, 44, 46, 130, 154, 189, 73, 179, 223,
                        178, 17, 133, 232, 213, 5, 0, 0, 0, 0, 0, 0
                    ]
                ],
            );
        }
    ...
    fn test_receive_invalid_chunk_as_chunk_producer() {
        ...
        let (chunk, merkle_paths, receipts, block) =
            create_invalid_proofs_chunk(&mut env.clients[0]);
        let client = &mut env.clients[0];
        assert!(client
            .shards_mgr
    ...
    fn test_fishermen_challenge() {
        ...
        assert_eq!(
            env.clients[0].chain.get_block_by_height(12).unwrap().challenges(),
            &[challenge]
        );
        assert!(env.clients[0].chain.mut_store().is_block_challenged(&block.hash()).unwrap());
    }
